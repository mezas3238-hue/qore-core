...
SCHEMA = "qore.core_stack_v3.vt31.regime_transfer_stability.v8"
IDENTITY = "VT31_NAS100_SHARED_REGIME_TRANSFER_STABILITY_V8"
...
class StableDecisionPolicy:
    ...
    winner_veto_required_eras: int = 2

    def payload(self) -> dict[str, object]:
        return {
            **self.base.payload(),
            "winner_veto_required_eras": self.winner_veto_required_eras,
            "aggregate_positive_ev_veto_enabled": False,
            "mandatory_negative_era_vote": False,
        }
...
def _stable_decision(
    *,
    ...
    winner_protected_eras = sum(
        str(item["action"]) == "PASS_WINNER_ARCHETYPE_PROTECTED"
        for item in era_views
    )

    if winner_protected_eras >= policy.winner_veto_required_eras:
        return {
            "action": "PASS",
            "reason": "TEMPORALLY_INVARIANT_WINNER_ARCHETYPE_VETO",
            "pooled": pooled,
            "era_views": era_views,
            "winner_protected_eras": winner_protected_eras,
        }

    return {
        "action": "ABSTAIN_SHADOW",
        "reason": "POOLED_NEGATIVE_NO_INVARIANT_WINNER_VETO",
        "pooled": pooled,
        "era_views": era_views,
        "winner_protected_eras": winner_protected_eras,
    }
...
def _governance() -> dict[str, bool]:
    return {
        ...
        "winner_veto_requires_two_independent_eras": True,
