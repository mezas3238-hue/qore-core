...
SCHEMA = "qore.core_stack_v3.vt31.regime_transfer_stability.v7"
IDENTITY = "VT31_NAS100_SHARED_REGIME_TRANSFER_STABILITY_V7"
...
DECISION_POLICIES = tuple(
    StableDecisionPolicy(
        base=decision.Policy(
            ...
            maximum_positive_views=maximum_positive_views,
            winner_archetype_ev_floor=winner_ev_floor,
            winner_archetype_payoff_floor=winner_payoff_floor,
            tail_mean_win_r_floor=Decimal("4.5"),
            tail_payoff_floor=Decimal("3"),
            tail_min_win_rate=Decimal("0.12"),
        ),
        positive_era_veto_ev_r=veto_ev,
        minimum_negative_era_support=0,
    )
    for confidence in (2500, 4000)
    for maximum_positive_views in (0, 1)
    for winner_ev_floor in (Decimal("0.20"), Decimal("0.30"))
    for winner_payoff_floor in (Decimal("2.5"), Decimal("3.5"))
    for veto_ev in (
        Decimal("0.15"),
        Decimal("0.30"),
        Decimal("0.50"),
