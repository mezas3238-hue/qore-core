"""Persistent-state causal routing frontier on VT31 NAS100 ALLOC_G.

Consumed evidence only. Root-cause forensics found two decision-time states
that were negative across R5, R6, R8, and the consumed 2022-2024 interval:

- fair-value-gap while premarket state is rotation,
- order-block with risk_ref below 0.30.

The frontier tests narrow structural renewal of those states. CORE breaker is
untouched. Renewal requires a new raid, confirmation, and decision after the
rejected fallback. ALLOC_G weights remain unchanged.
"""
...
SCHEMA = "qore.vt31.nas100.r5.persistent_state_routing_frontier.v1"
MARKET = "NAS100"
PROFILE_NAME = "ALLOC_G_CORE_FAMILY_050"
ROUTE_POLICIES = (
    "ORIGINAL",
    "RENEW_FVG_PREMARKET_ROTATION",
    "RENEW_OB_LOW_RISK_REF",
...
def replay(path: Path, *, partition: str) -> dict[str, object]:
    ...
    if not series or series[0].instrument.symbol != MARKET:
        raise ValueError("persistent-state routing frontier requires NAS100")
    ...
    for route_policy in ROUTE_POLICIES:
        ...
        mc = engine._monte_carlo(
            adjusted,
            variant=f"PERSISTENT_STATE_ROUTING:{route_policy}:{partition}",
        )
    ...
    return {
        ...
        "governance": {
            ...
            "persistent_states_predeclared_from_cross_period_forensics": True,
