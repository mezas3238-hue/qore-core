"""Replacement one-shot collector for WP-04 V3B HOLDOUT_E.

HOLDOUT_A and HOLDOUT_C were consumed by technical acquisition defects after irreversible
markers. REPLICATION_B remains sealed and is not repurposed. HOLDOUT_E is a new
independent recent window preregistered before acquisition.

This collector has no model/probe/economic access. It acquires only raw M1
market evidence for the exact frozen window and requires >=60 days of actual
...
HOLDOUT_START = datetime(2026, 7, 13, tzinfo=UTC)
HOLDOUT_END_EXCLUSIVE = datetime(2026, 9, 25, tzinfo=UTC)
MINIMUM_COVERAGE_DAYS = 60
IDENTITY = "QORE_SHARED_WP04_PREDICTIVE_HOLDOUT_E_V3B_001"
PURPOSE = "WP04_V3B_REPLACEMENT_ONE_SHOT_HOLDOUT_E"
...
def main() -> None:
    ...
    if market not in _RESEARCH_MARKETS:
        raise CTraderDemoLabProbeError(
            "WP04 HOLDOUT_E market must be NAS100, SP500 or US30"
        )
    ...
    try:
        ...
        if not isinstance(coverage, dict):
            raise CTraderDemoLabProbeError("HOLDOUT_E coverage payload is invalid")
        ...
        if first < HOLDOUT_START:
            raise CTraderDemoLabProbeError(
                "HOLDOUT_E evidence begins before preregistered boundary"
            )
        if last > HOLDOUT_END_EXCLUSIVE:
            raise CTraderDemoLabProbeError(
                "HOLDOUT_E evidence crosses preregistered end boundary"
            )
        if (last - first).days < MINIMUM_COVERAGE_DAYS:
            raise CTraderDemoLabProbeError(
                "HOLDOUT_E evidence does not cover sufficient calendar span"
            )
        ...
        payload.update(
            {
                ...
                "holdout_partition": "REPLACEMENT_ONE_SHOT_HOLDOUT_E",
