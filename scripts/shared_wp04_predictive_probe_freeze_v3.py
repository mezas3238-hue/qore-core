"""WP-04 V3 consumed target diagnostics and final predictive probe freeze.

The temporal representation is first learned from generic source->future market
transitions on consumed R8/R6/R5 evidence without named evaluation targets.
This stage then measures whether those frozen temporal concepts add information
beyond the existing ontology in leave-one-partition-out consumed diagnostics.

These diagnostics are development evidence, not fresh holdout evidence. If all
development gates pass, exact target probes are fit on all consumed partitions
and fingerprinted for later one-shot evaluation. No fresh holdout is opened.
"""
...
SCHEMA = "qore.shared.wp04.predictive_probe_freeze_v3.v1"
IDENTITY = "QORE_SHARED_WP04_PREDICTIVE_PROBE_FREEZE_V3_001"
...
def _pooled_summary(
    ...
    if not evaluations:
        raise ValueError("predictive probe diagnostics require evaluations")
...
def run(*, evidence: dict[str, dict[str, Path]]) -> dict[str, Any]:
    ...
    if not model.concepts:
        return {
            "schema": SCHEMA,
            "identity": IDENTITY,
            "status": "WP04_V3_NO_TEMPORAL_CONCEPTS_FOR_PROBE",
            "protocol_pass": False,
    ...
    if development_gate_pass:
        all_episodes = _concat_episodes(evaluation_episodes, PARTITIONS)
    ...
    return {
        ...
        "identity": IDENTITY,
        "status": (
            "WP04_V3_PROBE_FROZEN_FOR_ONE_SHOT_HOLDOUT"
            if probe_frozen
            else "WP04_V3_TARGET_DIAGNOSTIC_FALSIFIED_BEFORE_HOLDOUT"
        ),
        ...
        "representation": {
            ...
            "named_evaluation_target_used": model.named_evaluation_target_used,
        ...
        "governance": {
            ...
            "representation_frozen_before_named_targets": True,
