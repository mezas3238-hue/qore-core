...
if (-not $Shadow.ok -or $Shadow.git_sha -ne $GitSha -or $Shadow.order_send_called) {
...
$Activation = [ordered]@{
    git_sha = $GitSha
    account_identity_fingerprint = [string]$Shadow.account_identity_fingerprint
...
if ([string]$State.account_identity_fingerprint -ne [string]$Shadow.account_identity_fingerprint) {
...
$PreReboot = [ordered]@{
    ...
    activate_live_after_reboot = [bool]$ActivateLive
...
Write-Host "QORE runtime installed in SHADOW with fresh heartbeat."
Write-Host "Post-reboot verifier installed."
if ($RebootNow) {
    Write-Host "Rebooting VPS to prove boot recovery..."
    Restart-Computer -Force
} else {
    Write-Host "Reboot is the only remaining local evidence step."
    Write-Host "Run this installer once with -RebootNow; include -ActivateLive to arm LIVE after a clean reboot proof."
}
