"""Economic replay of corrected resident Shared Core against VT08 Index.

V5 is the post-rejection correction of V4. VT08 contributes the methodology-valid
...
Every closed M15 bar recomputes Shared resident convergence across trajectory,
environment, multi-axis future geometry, competing futures, causal memory,
drawdown phenotype/stability, position path, journey and instinct. Management
directives become active only after the bar that produced them, so same-bar
favorable ordering is never credited. Sizing remains frozen at 1R.
"""
# ruff: noqa: E501
...
SCHEMA = "qore.shared.vt08_index.full_stack_economic_replay.v5"
IDENTITY = "QORE_SHARED_VT08_INDEX_RESIDENT_CONVERGENCE_ECONOMIC_NO_SIZING_V5"
ZERO = Decimal("0")
ONE = Decimal("1")
...
def _directive_at_close(
    ...
    *,
    ...
    analog, phenotype = v3._memory(
        records=closed_memory,
        market=symbol,
        as_of=as_of,
        signature=entry_context["signature"],
    )
    stability = v3._stability(
        records=closed_memory,
        as_of=as_of,
        situation=entry_context["situation"],
        latest=latest,
        trajectory=trajectory,
    )
    convergence = assess_resident_convergence(
        environment,
        trajectory,
        geometry,
        futures,
...
def _simulate_trade(
    ...
    *,
    ...
    if risk <= ZERO:
        raise ValueError("VT08 Shared V5 invalid initial risk")
...
def _window(
    *,
    ...
    if len(control) != expected:
        raise ValueError(f"VT08 Shared V5 {window_id} density drift")
    ...
    *,
    ...
    return {
        ...
        "challenge": {
            ...
            "resident_convergence_recomputed_each_closed_bar": True,
            "resident_convergence_heads": (
                "ENVIRONMENT",
                "TRAJECTORY",
                "MULTI_AXIS_FUTURE_GEOMETRY",
                "MULTI_HORIZON_COMPETING_FUTURES",
                "CAUSAL_ANALOG_MEMORY",
                "DRAWDOWN_PHENOTYPE_MEMORY",
                "DRAWDOWN_STABILITY",
                "POSITION_PATH",
                "JOURNEY",
                "INSTINCT",
