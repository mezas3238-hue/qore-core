"""Five-year validation for frozen VT31 NAS100 execution binding V4.

The strategy remains VT31_NAS100_STRUCTURAL_TARGET_V1. V4 changes no strategy
parameter; it freezes physically causal subleg execution for the certified
target architecture.

Window: [2017-07-01, 2022-07-01), consumed extended validation.
No search, comparison or retuning is permitted.
"""
# ruff: noqa: E402, I001
...
SCHEMA = "qore.vt31.nas100.structural_target_execution_binding_5y.v4"
...
def _annual(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for index, (start, end) in enumerate(YEAR_BLOCKS, start=1):
        selected = [
            row for row in rows
            if start <= date.fromisoformat(cast(str, row["local_date"])) < end
        ]
...
def validate(*, r8_path: Path, r6_path: Path, r5_path: Path) -> dict[str, object]:
    (
        series, account, fingerprint, checked_at, software_sha, provider, records
    ...
    mc = engine._monte_carlo(
        selected,
        variant="VT31_NAS100_STRUCTURAL_TARGET_V1:FIVE_YEAR_CONSUMED",
    )
    ...
    gates = {
        ...
        "binding_fingerprint_stable": len(frozen.binding_fingerprint()) == 64,
    }
    return {
        "schema": SCHEMA,
...
def main() -> None:
    ...
    print(json.dumps({
        ...
        "execution_binding_fingerprint": payload["execution_binding_fingerprint"],
