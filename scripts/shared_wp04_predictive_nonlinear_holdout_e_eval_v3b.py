"""Replacement one-shot HOLDOUT_E evaluator for WP-04 V3B nonlinear probes.
...
then evaluates the preregistered replacement HOLDOUT_E window.

Fresh gate is immutable:
- pooled incremental information >= 100 bps;
- >= 4/8 targets positive.

A HOLDOUT_E pass requires an independent replication window before WP-04
can close. A HOLDOUT_E failure falsifies V3B and leaves replication sealed.
"""
...
SCHEMA = "qore.shared.wp04.predictive_nonlinear_holdout_e_v3b.v1"
IDENTITY = "QORE_SHARED_WP04_PREDICTIVE_HOLDOUT_E_V3B_001"
HOLDOUT_PARTITION = "holdout_e"
HOLDOUT_START = datetime.fromisoformat("2026-07-13T00:00:00+00:00")
HOLDOUT_END_EXCLUSIVE = datetime.fromisoformat("2026-09-25T00:00:00+00:00")
...
def run(
    *,
    ...
    if not holdout_episodes:
        raise ValueError("HOLDOUT_E contains no evaluation episodes")
    if min(item.as_of for item in holdout_episodes) < HOLDOUT_START:
        raise ValueError("HOLDOUT_E begins before preregistered boundary")
    if max(item.as_of for item in holdout_episodes) >= HOLDOUT_END_EXCLUSIVE:
        raise ValueError("HOLDOUT_E crosses preregistered source boundary")
    ...
    holdout_targets = {
        target_name: tuple(
            item
            for item in rows
            if HOLDOUT_START <= item.observed_at < HOLDOUT_END_EXCLUSIVE
        )
        for target_name, rows in holdout_targets.items()
    }
    if any(not rows for rows in holdout_targets.values()):
        raise ValueError("HOLDOUT_E has an empty target after boundary filtering")
    ...
    if frozen_probe_fingerprints != evaluated_probe_fingerprints:
        raise ValueError("HOLDOUT_E V3B probe fingerprint drift")
    ...
    return {
        ...
        "status": (
            ...
            if summary["pass"]
            else "WP04_V3B_HOLDOUT_E_FALSIFIED"
        ),
        "protocol_pass": True,
        "holdout_e_pass": summary["pass"],
        "replication_required": summary["pass"],
        "replacement_for_consumed_holdout_a": True,
        "replacement_for_consumed_holdout_c": True,
        ...
        "governance": {
            ...
            "replication_d_triggered_only_after_holdout_e_pass": True,
        ...
        json.dumps(
            {
                ...
                "holdout_e_pass": payload["holdout_e_pass"],
