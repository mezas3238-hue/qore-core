"""WP-05 V3 consumed-development hierarchy absorption falsification.

V1 used a current snapshot and V2 used continuous trajectory prototypes. V3
changes representation: adversity is a front moving across ordered scales,
with advance, dwell, recession, absorption and re-propagation states.

The source-only trajectory is:
    t-90m -> t-60m -> t-30m -> t-15m -> t

R8 fits duration/state terminal hazards and the declaration threshold. R6/R5
remain consumed later falsification partitions only. No fresh holdout is opened.
"""
...
SCHEMA = "qore.shared.wp05.temporal_hierarchy_absorption_consumed_v3"
IDENTITY = "QORE_SHARED_WP05_TEMPORAL_HIERARCHY_ABSORPTION_V3_001"
TRAJECTORY_OFFSETS_MINUTES = (90, 60, 30, 15, 0)
...
def _evaluation_payload(
    evaluation: TemporalHierarchyAbsorptionEvaluation,
) -> dict[str, int | str]:
    return {
    ...
    *,
    ...
    if not sample_gate:
        return {
            "schema": SCHEMA,
            "identity": IDENTITY,
            "status": "WP05_V3_SAMPLE_GATE_FAILED",
            "protocol_pass": False,
    ...
    return {
        ...
        "identity": IDENTITY,
        "status": (
            "WP05_V3_ABSORPTION_MODEL_FROZEN_FOR_FRESH_HOLDOUT"
            if protocol_pass and development_gate_pass
            else "WP05_V3_ABSORPTION_MODEL_FALSIFIED"
        ),
        ...
        "model": {
            ...
            "representation": "SEMI_MARKOV_HIERARCHY_FRONT",
            ...
            "hazard_cells": [
                {
                    ...
                    "terminal_count": cell.terminal_count,
                    "hazard_micros": cell.hazard_micros,
                }
                for cell in model.hazard_cells
            ],
        },
        "evaluations": {
        ...
        "governance": {
            ...
            "v3_representation": "SEMI_MARKOV_HIERARCHY_FRONT",
            "front_advance_dwell_recession_modelled": True,
            "absorption_modelled": True,
            "repropagation_modelled": True,
