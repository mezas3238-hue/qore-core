...
def main()->None:
    ...
    groups=f["diagnostic_groups"]
    report_groups={name:{k:compact(v) for k,v in groups[name].items()} for name in GROUP_ORDER}
    # Predeclared descriptive contrasts only; none is a candidate gate.
    ps=groups["protected_swing_like_at_signal"]
    ps_contrast=None
    if "True" in ps and "False" in ps:
        ps_contrast={
            "true":compact(ps["True"]),"false":compact(ps["False"]),
            "mean_difference_true_minus_false":format(D(ps["True"]["mean_stressed_r"])-D(ps["False"]["mean_stressed_r"]),"f"),
        }
    prior=groups["prior_day_alignment"]
    prior_contrast=None
    if "True" in prior and "False" in prior:
        prior_contrast={
            "aligned":compact(prior["True"]),"opposed":compact(prior["False"]),
            "mean_difference_aligned_minus_opposed":format(D(prior["True"]["mean_stressed_r"])-D(prior["False"]["mean_stressed_r"]),"f"),
        }
    ...
    payload={
        "schema":"qore.vt31.definitive_deep_forensics_report.v1","research_only":True,"opens_new_holdout":False,"candidate_status":"NO_R9_NOT_CERTIFIED","selection_prohibited":True,
        ...
        "interpretation_constraints":[
            "group contrasts are descriptive diagnostics, not candidate-selection rules",
            "post-entry terminal_r was used only as label",
            "no threshold may be promoted without a finite predeclared family and leakage-free walk-forward",
        ],
    }
    a.output.write_text(json.dumps(payload,sort_keys=True,indent=2)+"\n")
    print(json.dumps({"source_target_structure":payload["source_target_structure"],"protected_swing_like_contrast":ps_contrast,"prior_day_alignment_contrast":prior_contrast,"groups":report_groups,"top_losing_streaks":streak_summary[:5]},sort_keys=True))
