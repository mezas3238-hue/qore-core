"""Post-equilibrium structural protection V2 for VT31_NAS100.

Corrects V1 temporal/accounting leakage.

Key invariants:
- equilibrium must be touched on a CLOSED M1 strictly before the original
  terminal time;
- post-EQ protection may only improve/hold the current journey, never revive a
  trade after its original terminal;
- before DOL1, if no new structural stop or DOL1 occurs before the original
  terminal, the original terminal outcome is preserved;
- DOL1 may stage a runner only if reached before the original terminal, or on
  the original terminal bar when the original exit itself was a structural
  target/boundary event;
- after valid DOL1 staging, the residual runner may continue with existing PS2
  logic;
- same-bar ambiguity remains conservative.

No future label is a runtime input.
"""
# ruff: noqa: B009
...
SCHEMA = (
    "qore.vt31.nas100."
    "post_equilibrium_structural_protection_frontier.v2"
)
MARKET = "NAS100"
...
TARGET_TERMINAL_REASONS = {
    "structural-target",
    "partial-plus-structural-boundary",
}
...
def _eq_remainder_to_dol1(
    ...
    *,
    ...
    for index in range(1, len(path)):
        ...
        at_or_after_terminal = closed >= original_terminal_at
        terminal_is_target = (
            original_exit_reason in TARGET_TERMINAL_REASONS
        )

        if at_or_after_terminal:
            if (
                closed == original_terminal_at
                and terminal_is_target
                and _touches_target(
                    bar,
                    side=side,
                    level=dol1,
    ...
    for row in rows:
        ...
        eq_index = next(
            (
                ...
                if cast(
                    datetime,
                    getattr(day_bars[index], "closed_at"),
                )
                < terminal_at
                and _wall(
                    getattr(day_bars[index], "opened_at")
                )
                < (16, 0, 0)
                and _touches_target(
                    day_bars[index],
        ...
        status, remainder_r, dol1_at, eq_diag = (
            _eq_remainder_to_dol1(
                day_bars,
                eq_index=eq_index,
                original_terminal_at=terminal_at,
                original_exit_reason=original_exit_reason,
                original_gross_r=original_gross,
                side=side,
                entry=entry,
        ...
        if dol1_at is None:
            ...
            updated["post_eq_status_v2"] = status
            updated[
                "post_eq_structural_protection_armed_v2"
            ] = bool(
                eq_diag.get(
            ...
            updated["exit_reason"] = (
                "eq-bank-post-eq-structural-protection-v2"
            )
            adjusted.append(updated)
            continue
        ...
        runner_r, runner_status, runner_diag = (
            post_dol1._runner_outcome(
                day_bars,
        ...
        updated["runner_terminal_status"] = (
            runner_status
        )
        updated[
            "runner_structural_protection_armed"
        ] = bool(
            runner_diag.get(
                "structural_protection_armed"
            )
        ...
        updated["exit_reason"] = (
            "eq-protected-dol1-runner-ladder-v2"
        )
        adjusted.append(updated)

    return adjusted, {
        "equilibrium_bank_applied_count": eq_applied,
        "post_eq_structural_protection_armed_count": (
            eq_protected
        ),
        "post_eq_status_counts": dict(
            sorted(eq_statuses.items())
        ),
        "dol1_staged_count": dol1_staged,
        "runner_applied_count": runner_applied,
        "runner_structural_protection_armed_count": (
            runner_protected
        ),
        "runner_status_counts": dict(
            sorted(runner_statuses.items())
        ),
...
def replay(
    ...
    *,
    ...
    rows, evidence, diagnostics, source_stats = (
        alt._current_rows(path)
    )
    (
        series,
        account,
    ...
    if (
        not series
        or getattr(series[0], "instrument").symbol
        != MARKET
    ):
        raise ValueError(
            "post-equilibrium protection v2 requires NAS100"
        )

    raw: dict[date, list[object]] = defaultdict(list)
    for bar in series:
        raw[_day(getattr(bar, "opened_at"))].append(
            bar
        )
    by_day = {
        local_day: tuple(
    ...
    for name, spec in VARIANTS.items():
        ...
        mc = engine._monte_carlo(
            adjusted,
            variant=(
                f"POST_EQ_STRUCTURAL_PROTECT_V2:"
                f"{name}:{partition}"
            ),
        )
        annual = (
        ...
        variants[name] = {
            ...
            "passes_economic_objectives": all(
                objectives.values()
            ),
        }
    ...
    return {
        ...
        "governance": {
            ...
            "equilibrium_strictly_before_original_terminal": True,
            "original_terminal_preserved_unless_earlier_improvement": True,
            "no_trade_resurrection_after_original_terminal": True,
            ...
            "runner_only_after_valid_dol1": True,
