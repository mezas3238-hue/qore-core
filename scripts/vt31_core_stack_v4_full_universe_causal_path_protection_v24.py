"""VT31 Shared full-universe causal path protection laboratory V24.

V24 combines the full-universe reconstruction of V23 with the generic Shared
Position Path Intelligence. Every methodology-valid R8/R6 opportunity remains
present. No entry abstention, sizing, risk taper, terminal outcome signal or
future bar is allowed.

Hypothesis:
- adverse market environment alone is insufficient for defense;
- Shared must distinguish terminal path failure from a recoverable pullback;
- established favorable journey activates winner protection;
- defense is permitted only when causal path evidence confirms failure risk
  or persistent adverse dominance.

R8/R6 are consumed falsification folds. R5 and fresh holdouts remain closed.
"""
# ruff: noqa: E501
from __future__ import annotations

import argparse
import json
from collections import Counter
from dataclasses import dataclass
from decimal import Decimal
from pathlib import Path
from typing import cast

import vt31_core_stack_v4_full_universe_asymmetric_journey_v23 as v23

from qore.infrastructure.core_stack_v2.environment_intelligence import (
    MarketEnvironmentState,
)
from qore.infrastructure.core_stack_v2.path_intelligence import (
    PositionPathObservation,
    PositionPathState,
    assess_position_path,
)

SCHEMA = "qore.core_stack_v4.vt31.full_universe_causal_path_protection.v24"
IDENTITY = "VT31_NAS100_SHARED_FULL_UNIVERSE_CAUSAL_PATH_PROTECTION_V24"
ZERO = Decimal("0")
DD_ACCEPTABLE_MAX_R = Decimal("6")
DD_EXCEPTIONAL_R = Decimal("4")
PATH_WINDOW = 7
v22 = v23.v22
v21 = v22.v21
v19 = v21.v20.v19


@dataclass(frozen=True, slots=True)
class PathJourneyPolicy:
    name: str
    environment_states: frozenset[MarketEnvironmentState]
    path_states: frozenset[PositionPathState]

    def payload(self) -> dict[str, object]:
        return {
            "name": self.name,
            "environment_states": tuple(
                sorted(state.value for state in self.environment_states)
            ),
            "path_states": tuple(sorted(state.value for state in self.path_states)),
            "entry_abstention_allowed": False,
            "numeric_threshold_grid_search": False,
            "winner_protection_required": True,
        }


POLICIES = (
    PathJourneyPolicy(
        "DEFENSIVE_PATH_FAILURE_ONLY",
        frozenset({MarketEnvironmentState.DEFENSIVE}),
        frozenset({PositionPathState.FAILURE_RISK}),
    ),
    PathJourneyPolicy(
        "ADVERSE_PATH_FAILURE_ONLY",
        frozenset(
            {
                MarketEnvironmentState.ADVERSE_FORMING,
                MarketEnvironmentState.DEFENSIVE,
            }
        ),
        frozenset({PositionPathState.FAILURE_RISK}),
    ),
    PathJourneyPolicy(
        "DEFENSIVE_PATH_FAILURE_OR_DOMINANCE",
        frozenset({MarketEnvironmentState.DEFENSIVE}),
        frozenset(
            {
                PositionPathState.FAILURE_RISK,
                PositionPathState.ADVERSE_DOMINANCE,
            }
        ),
    ),
    PathJourneyPolicy(
        "ADVERSE_PATH_FAILURE_OR_DOMINANCE",
        frozenset(
            {
                MarketEnvironmentState.ADVERSE_FORMING,
                MarketEnvironmentState.DEFENSIVE,
            }
        ),
        frozenset(
            {
                PositionPathState.FAILURE_RISK,
                PositionPathState.ADVERSE_DOMINANCE,
            }
        ),
    ),
)


def _d(value: object) -> Decimal:
    return Decimal(str(value))


def _clamp_bps(value: Decimal) -> int:
    return max(0, min(10_000, int(value)))


def _path_observation(
    event: dict[str, object],
    *,
    peak_progress_bps: int,
) -> PositionPathObservation:
    progress = max(ZERO, min(Decimal("1"), _d(event["progress_fraction"])))
    close_r = _d(event["close_r"])
    efficiency = _d(event["efficiency"])
    signed_body = _d(event["signed_body_r"])

    return PositionPathObservation(
        as_of=v19._to_observation({}, event).as_of
        if False
        else cast(object, event["path_as_of"]),
        data_integrity_bps=int(event["path_data_integrity_bps"]),
        journey_progress_bps=_clamp_bps(progress * Decimal("10000")),
        close_support_bps=v19._signed_bps(close_r),
        directional_efficiency_bps=v19._signed_bps(efficiency),
        favorable_excursion_bps=peak_progress_bps,
        adverse_excursion_bps=max(
            v19._adverse_bps(close_r),
            v19._adverse_bps(efficiency),
        ),
        favorable_body_bps=v19._signed_bps(signed_body),
        adverse_body_bps=v19._adverse_bps(signed_body),
        market_support_bps=int(event["trajectory_support_bps"]),
        environment_adverse_bps=int(event["environment_adverse_bps"]),
        recovery_evidence_bps=max(
            int(event["trajectory_recovery_velocity_bps"]),
            int(event["environment_recovery_velocity_bps"]),
        ),
    )


def _annotate_path(
    prepared: list[dict[str, object]],
) -> tuple[list[dict[str, object]], dict[str, object]]:
    annotated: list[dict[str, object]] = []
    histogram: Counter[str] = Counter()
    transition_histogram: Counter[str] = Counter()

    for item in prepared:
        history: list[PositionPathObservation] = []
        events: list[dict[str, object]] = []
        peak_progress_bps = 0
        previous: str | None = None

        for raw in cast(list[dict[str, object]], item["events"]):
            progress = max(
                ZERO,
                min(Decimal("1"), _d(raw["progress_fraction"])),
            )
            peak_progress_bps = max(
                peak_progress_bps,
                _clamp_bps(progress * Decimal("10000")),
            )
            event = {
                **raw,
                "path_as_of": v19._to_observation(
                    cast(dict[str, object], item["row"]),
                    raw,
                ).as_of,
                "path_data_integrity_bps": v19._to_observation(
                    cast(dict[str, object], item["row"]),
                    raw,
                ).data_integrity_bps,
            }
            observation = _path_observation(
                event,
                peak_progress_bps=peak_progress_bps,
            )
            history.append(observation)
            assessment = assess_position_path(tuple(history[-PATH_WINDOW:]))
            state = assessment.state.value
            histogram[state] += 1
            if previous is not None and previous != state:
                transition_histogram[f"{previous}->{state}"] += 1
            previous = state
            events.append(
                {
                    **raw,
                    "path_state": state,
                    "path_support_bps": assessment.path_support_bps,
                    "path_adverse_bps": assessment.adverse_dominance_bps,
                    "path_adverse_persistence_bps": (
                        assessment.adverse_persistence_bps
                    ),
                    "path_recovery_persistence_bps": (
                        assessment.recovery_persistence_bps
                    ),
                    "path_winner_protection_bps": (
                        assessment.winner_protection_bps
                    ),
                    "path_terminal_failure_risk_bps": (
                        assessment.terminal_failure_risk_bps
                    ),
                    "path_reasons": assessment.reasons,
                }
            )
        annotated.append({**item, "events": events})

    return annotated, {
        "path_window": PATH_WINDOW,
        "state_histogram": dict(sorted(histogram.items())),
        "transition_histogram": dict(sorted(transition_histogram.items())),
        "terminal_outcome_used": False,
        "future_path_used": False,
    }


def _matches(event: dict[str, object], policy: PathJourneyPolicy) -> bool:
    environment = MarketEnvironmentState(str(event["environment_state"]))
    path = PositionPathState(str(event["path_state"]))
    if environment not in policy.environment_states:
        return False
    if path not in policy.path_states:
        return False
    if int(event["path_winner_protection_bps"]) >= 6200:
        return False
    return True


def _metrics(values: list[Decimal]) -> dict[str, object]:
    return v22._metrics(values)


def _evaluate(
    prepared: list[dict[str, object]],
    *,
    policy: PathJourneyPolicy,
) -> dict[str, object]:
    baseline_values: list[Decimal] = []
    shared_values: list[Decimal] = []
    defended: list[dict[str, object]] = []

    for item in prepared:
        row = cast(dict[str, object], item["row"])
        baseline_r = cast(Decimal, item["baseline_r"])
        baseline_values.append(baseline_r)
        shared_r = baseline_r
        for event in cast(list[dict[str, object]], item["events"]):
            if not _matches(event, policy):
                continue
            shared_r = _d(event["defense_r_after_friction"])
            defended.append(
                {
                    "signal_at": row["signal_at"],
                    "baseline_r": format(baseline_r, "f"),
                    "defense_r": format(shared_r, "f"),
                    "uplift_r": format(shared_r - baseline_r, "f"),
                    "minutes_since_fill": event["minutes_since_fill"],
                    "environment_state": event["environment_state"],
                    "path_state": event["path_state"],
                    "path_winner_protection_bps": event[
                        "path_winner_protection_bps"
                    ],
                    "path_terminal_failure_risk_bps": event[
                        "path_terminal_failure_risk_bps"
                    ],
                    "path_reasons": event["path_reasons"],
                }
            )
            break
        shared_values.append(shared_r)

    baseline = _metrics(baseline_values)
    shared = _metrics(shared_values)
    losers = [x for x in defended if _d(x["baseline_r"]) < ZERO]
    winners = [x for x in defended if _d(x["baseline_r"]) > ZERO]
    baseline_winner_r = _d(baseline["gross_winner_r"])
    shared_winner_r = _d(shared["gross_winner_r"])
    winner_retention = (
        Decimal("1")
        if baseline_winner_r == ZERO
        else shared_winner_r / baseline_winner_r
    )
    loss_count = int(baseline["losses"])
    return {
        "baseline": baseline,
        "shared_v24": shared,
        "density": {
            "input": len(baseline_values),
            "kept": len(shared_values),
            "retained": "1",
            "entry_abstentions": 0,
            "trades_with_journey_events": sum(
                bool(cast(list[dict[str, object]], item["events"]))
                for item in prepared
            ),
        },
        "journey": {
            "defended_trades": len(defended),
            "defended_baseline_losses": len(losers),
            "defended_baseline_winners": len(winners),
            "baseline_loss_defense_recall": (
                "0"
                if loss_count == 0
                else format(Decimal(len(losers)) / Decimal(loss_count), "f")
            ),
            "loser_uplift_r": format(
                sum((_d(x["uplift_r"]) for x in losers), ZERO),
                "f",
            ),
            "winner_damage_r": format(
                -min(ZERO, sum((_d(x["uplift_r"]) for x in winners), ZERO)),
                "f",
            ),
            "winner_r_retention": format(winner_retention, "f"),
            "net_journey_uplift_r": format(
                sum((_d(x["uplift_r"]) for x in defended), ZERO),
                "f",
            ),
            "best_examples": sorted(
                defended, key=lambda x: _d(x["uplift_r"]), reverse=True
            )[:20],
            "worst_examples": sorted(
                defended, key=lambda x: _d(x["uplift_r"])
            )[:20],
        },
    }


def _gates(result: dict[str, object]) -> dict[str, bool]:
    baseline = cast(dict[str, object], result["baseline"])
    shared = cast(dict[str, object], result["shared_v24"])
    journey = cast(dict[str, object], result["journey"])
    density = cast(dict[str, object], result["density"])
    baseline_pf = _d(baseline["profit_factor"])
    shared_pf = _d(shared["profit_factor"])
    dd = _d(shared["max_drawdown_r"])
    return {
        "profit_factor_above_baseline": shared_pf > baseline_pf,
        "profit_factor_plus_25pct": shared_pf >= baseline_pf * Decimal("1.25"),
        "drawdown_at_most_6r": dd <= DD_ACCEPTABLE_MAX_R,
        "drawdown_below_4r_exceptional": dd < DD_EXCEPTIONAL_R,
        "total_r_not_lower": _d(shared["total_r"]) >= _d(baseline["total_r"]),
        "density_exactly_preserved": density["retained"] == "1",
        "entry_abstentions_zero": density["entry_abstentions"] == 0,
        "winner_r_retention_at_least_95pct": (
            _d(journey["winner_r_retention"]) >= Decimal("0.95")
        ),
    }


def _hard_pass(gates: dict[str, bool]) -> bool:
    required = (
        "profit_factor_above_baseline",
        "profit_factor_plus_25pct",
        "drawdown_at_most_6r",
        "total_r_not_lower",
        "density_exactly_preserved",
        "entry_abstentions_zero",
        "winner_r_retention_at_least_95pct",
    )
    return all(gates.get(key, False) for key in required)


def _score(result: dict[str, object]) -> Decimal:
    baseline = cast(dict[str, object], result["baseline"])
    shared = cast(dict[str, object], result["shared_v24"])
    journey = cast(dict[str, object], result["journey"])
    return (
        _d(shared["profit_factor"])
        / max(_d(baseline["profit_factor"]), Decimal("0.000001"))
        * _d(baseline["max_drawdown_r"])
        / max(_d(shared["max_drawdown_r"]), Decimal("0.000001"))
        * _d(journey["winner_r_retention"])
        * (Decimal("1") + _d(journey["baseline_loss_defense_recall"]))
    )


def _governance() -> dict[str, bool]:
    return {
        "vt31_is_falsification_lab_only": True,
        "full_methodology_valid_universe_reconstructed": True,
        "shared_generic_environment_engine_used": True,
        "shared_generic_position_path_engine_used": True,
        "winner_protection_used": True,
        "journey_only_no_entry_abstention": True,
        "density_preservation_is_hard_gate": True,
        "runtime_terminal_trade_outcome_used": False,
        "runtime_prior_trade_pnl_used": False,
        "runtime_prior_loss_streak_used": False,
        "future_m1_used": False,
        "r5_opened": False,
        "new_holdout_opened": False,
        "capital_risk_weighting_used": False,
        "methodology_entry_modified": False,
        "shared_order_authority": False,
        "shared_risk_authority": False,
        "shared_execution_authority": False,
        "stop_widening_used": False,
        "live_authorized": False,
        "production_authorized": False,
        "merge_authorized": False,
    }


def run(
    *,
    r8_trades: Path,
    r6_trades: Path,
    r8_nas: Path,
    r8_sp: Path,
    r8_us: Path,
    r6_nas: Path,
    r6_sp: Path,
    r6_us: Path,
    daily_path: Path,
) -> dict[str, object]:
    daily = v22.v21.v20.v19.v18.v2.v3._load_daily(daily_path)
    r8 = v22.v21.v20.v19.v18._load_cross_rows(
        trades=r8_trades, nas=r8_nas, sp=r8_sp, us=r8_us, daily=daily
    )
    r6 = v22.v21.v20.v19.v18._load_cross_rows(
        trades=r6_trades, nas=r6_nas, sp=r6_sp, us=r6_us, daily=daily
    )
    if len(r8) != 228 or len(r6) != 278:
        raise AssertionError("VT31 R8/R6 challenge-set drift")

    p8 = v23._prepare_full_universe(
        r8, nas_evidence=r8_nas, sp_evidence=r8_sp, us_evidence=r8_us
    )
    p6 = v23._prepare_full_universe(
        r6, nas_evidence=r6_nas, sp_evidence=r6_sp, us_evidence=r6_us
    )
    a8, trajectory_r8 = v19._annotate_trajectory(p8)
    a6, trajectory_r6 = v19._annotate_trajectory(p6)
    h8 = v21._with_state_history(a8)
    h6 = v21._with_state_history(a6)
    e8, environment_r8 = v22._annotate_environment(h8)
    e6, environment_r6 = v22._annotate_environment(h6)
    x8, path_r8 = _annotate_path(e8)
    x6, path_r6 = _annotate_path(e6)

    results: list[dict[str, object]] = []
    passing: list[tuple[Decimal, PathJourneyPolicy]] = []
    for policy in POLICIES:
        r8_result = _evaluate(x8, policy=policy)
        r6_result = _evaluate(x6, policy=policy)
        g8 = _gates(r8_result)
        g6 = _gates(r6_result)
        passed = _hard_pass(g8) and _hard_pass(g6)
        score = (_score(r8_result) + _score(r6_result)) / Decimal("2")
        results.append(
            {
                "policy": policy.payload(),
                "r8": {"evaluation": r8_result, "gates": g8},
                "r6": {"evaluation": r6_result, "gates": g6},
                "passes_both_consumed_folds": passed,
                "diagnostic_score": format(score, "f"),
            }
        )
        if passed:
            passing.append((score, policy))

    results.sort(key=lambda item: _d(item["diagnostic_score"]), reverse=True)
    frozen_policy: dict[str, object] | None = None
    if passing:
        passing.sort(key=lambda item: item[0], reverse=True)
        score, policy = passing[0]
        frozen_policy = {
            "journey_policy": policy.payload(),
            "generic_environment_defaults": "FROZEN_DEFAULTS",
            "generic_path_defaults": "FROZEN_DEFAULTS",
            "joint_score": format(score, "f"),
            "density_retained": "1",
        }

    return {
        "schema": SCHEMA,
        "identity": IDENTITY,
        "economic_status": (
            "CALIBRATION_PASSED_NEW_HOLDOUT_REQUIRED"
            if frozen_policy is not None
            else "FALSIFIED_BEFORE_NEW_HOLDOUT"
        ),
        "owner_objective": {
            "profit_factor_must_increase": True,
            "profit_factor_plus_25pct_reference_gate": True,
            "acceptable_drawdown_r": [4, 6],
            "exceptional_drawdown_r": "<4",
            "density_must_be_preserved": True,
        },
        "challenge_set": {"r8": 228, "r6": 278},
        "r5_opened": False,
        "new_holdout_opened": False,
        "passes_calibration": frozen_policy is not None,
        "frozen_policy": frozen_policy,
        "policy_results": results,
        "best_diagnostic_policy": None if not results else results[0]["policy"],
        "representation": {
            "trajectory_r8": trajectory_r8,
            "trajectory_r6": trajectory_r6,
            "environment_r8": environment_r8,
            "environment_r6": environment_r6,
            "path_r8": path_r8,
            "path_r6": path_r6,
            "entry_filtering_disabled": True,
            "density_preserved_by_design": True,
            "journey_reconstructed_for_full_universe": True,
        },
        "governance": _governance(),
    }


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--r8-trades", type=Path, required=True)
    parser.add_argument("--r6-trades", type=Path, required=True)
    parser.add_argument("--r8-nas", type=Path, required=True)
    parser.add_argument("--r8-sp", type=Path, required=True)
    parser.add_argument("--r8-us", type=Path, required=True)
    parser.add_argument("--r6-nas", type=Path, required=True)
    parser.add_argument("--r6-sp", type=Path, required=True)
    parser.add_argument("--r6-us", type=Path, required=True)
    parser.add_argument("--daily-path", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    payload = run(
        r8_trades=args.r8_trades,
        r6_trades=args.r6_trades,
        r8_nas=args.r8_nas,
        r8_sp=args.r8_sp,
        r8_us=args.r8_us,
        r6_nas=args.r6_nas,
        r6_sp=args.r6_sp,
        r6_us=args.r6_us,
        daily_path=args.daily_path,
    )
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(payload, sort_keys=True, indent=2) + "\n")
    print(
        json.dumps(
            {
                "economic_status": payload["economic_status"],
                "passes_calibration": payload["passes_calibration"],
                "frozen_policy": payload["frozen_policy"],
                "best_diagnostic_policy": payload["best_diagnostic_policy"],
                "policy_results": payload["policy_results"],
            },
            sort_keys=True,
        )
    )


if __name__ == "__main__":
    main()
