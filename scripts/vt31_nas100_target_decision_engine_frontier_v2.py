"""Causal Target Decision Engine V2 frontier for VT31_NAS100.

V2 corrects V1's over-broad SHALLOW classification.

Only stable 4/4 SHALLOW interactions from Entry Destination Intelligence V1
are allowed to suppress a DEEP classification. Broad one-dimensional states
such as risk_ref=low are NOT sufficient by themselves.

SHALLOW interactions:
- breaker entry + confirmation latency 11m+
- last structure breaker + breaker entry
- premarket bearish + cash-open bearish
- expanded reference volatility + current_path high
- SECONDARY tier + breaker entry

DEEP:
- compressed reference volatility
- and no SHALLOW interaction above

NEUTRAL:
- everything else

SHALLOW/NEUTRAL retain full DOL1. DEEP may bank 75% at DOL1 and run 25%
toward DOL2/DOL3 with causal protective-swing management.

No future journey label is used at runtime.
"""
# ruff: noqa: B009
...
SCHEMA = "qore.vt31.nas100.target_decision_engine_frontier.v2"
MARKET = "NAS100"
...
VARIANTS = {
    "BASE": None,
    "DEEP_V2_DOL2_PS1": (Decimal("0.25"), 1),
    "DEEP_V2_DOL2_PS2": (Decimal("0.25"), 2),
    "DEEP_V2_DOL3_PS2": (Decimal("0.50"), 2),
}
...
def _shallow_interactions(
    ...
    if (
        ...
        and row.get("confirmation_latency_bucket") == "11m_plus"
    ):
        reasons.append("BREAKER_LATENCY_11M_PLUS")

    if (
        row.get("last_structure_event_family") == "breaker"
        and row.get("entry_family") == "breaker"
    ):
        reasons.append("LAST_BREAKER_ENTRY_BREAKER")
...
def _destination_state(
    ...
    shallow = _shallow_interactions(row)
    if shallow:
...
def _apply_variant(
    ...
    *,
    ...
    for row in rows:
        ...
        updated["destination_state_v2"] = state
        updated["destination_state_v2_reasons"] = list(reasons)
        ...
        if (
            ...
            updated["target_decision_v2"] = "FULL_DOL1"
            adjusted.append(updated)
            continue

        deep_target_winners += 1
        ...
        if (
            ...
            updated["target_decision_v2"] = "FULL_DOL1_CENSORED_GEOMETRY"
            censored += 1
            adjusted.append(updated)
            continue
        ...
        if risk <= 0:
            updated["target_decision_v2"] = "FULL_DOL1_CENSORED_RISK"
            censored += 1
            adjusted.append(updated)
            continue
        ...
        if not day_bars:
            updated["target_decision_v2"] = "FULL_DOL1_CENSORED_DAY"
            censored += 1
            adjusted.append(updated)
            continue
        ...
        updated["exit_reason"] = "target-decision-engine-v2"
    ...
    return adjusted, {
        ...
        "destination_reason_counts": dict(sorted(reason_counts.items())),
        "deep_target_winner_count": deep_target_winners,
...
def replay(path: Path, *, partition: str) -> dict[str, object]:
    ...
    if not series or getattr(series[0], "instrument").symbol != MARKET:
        raise ValueError("target decision engine v2 requires NAS100")
    ...
    for name, spec in VARIANTS.items():
        if spec is None:
            ...
            diag = {
                ...
                "destination_reason_counts": {},
                "deep_target_winner_count": 0,
        ...
        mc = engine._monte_carlo(
            adjusted,
            variant=f"TARGET_DECISION_ENGINE_V2:{name}:{partition}",
        )
        annual = (
