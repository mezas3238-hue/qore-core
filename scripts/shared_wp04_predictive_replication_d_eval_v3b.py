...
D uses the exact frozen V3 temporal representation and eight V3B
SYMMETRIC_SECOND_ORDER_V1 probes already tested on HOLDOUT_E. No representation,
probe or threshold may be refit on D.

Replication gate is unchanged:
- pooled incremental information >= 100 bps;
- >= 4/8 frozen targets positive.

Only an independent D pass after HOLDOUT_E PASS can satisfy the WP-04 exit gate.
"""
...
SCHEMA = "qore.shared.wp04.predictive_replication_d_v3b.v1"
...
def run(
    *,
    ...
    if freeze["probe_frozen_for_holdout"] is not True:
        raise ValueError("V3B probes are not frozen")
    ...
    if any(probe.basis_id != BASIS_ID for probe in probes.values()):
        raise ValueError("V3B replication probe basis drift")
    if any(
        probe.representation_fingerprint != representation_fingerprint
        for probe in probes.values()
    ):
        raise ValueError("V3B replication representation fingerprint drift")
    if any(
    ...
    if any(probe.holdout_used_for_fit for probe in probes.values()):
        raise ValueError("fresh evidence cannot participate in V3B probe fit")

    episodes, targets, evidence_range = _prepare_partition(
    ...
    frozen_fingerprints = {probe.probe_fingerprint for probe in probes.values()}
    evaluated_fingerprints = {item.probe_fingerprint for item in evaluations}
    if frozen_fingerprints != evaluated_fingerprints:
        raise ValueError("REPLICATION_D probe fingerprint drift")

    return {
        "schema": SCHEMA,
        "identity": IDENTITY,
        "status": (
            "WP04_V3B_REPLICATION_D_PASS_WP04_EXIT_GATE"
            if summary["pass"]
        ...
        "probe_fingerprints": sorted(frozen_fingerprints),
        "replication": {
            "partition": REPLICATION_PARTITION,
            ...
            "evaluation_range": evidence_range,
            "episode_count": len(episodes),
        },
        "evaluation": summary,
        "governance": {
            "replication_preregistered_before_holdout_e_acquisition": True,
            "holdout_e_pass_required": True,
            ...
            "replication_b_remains_sealed_not_repurposed": True,
