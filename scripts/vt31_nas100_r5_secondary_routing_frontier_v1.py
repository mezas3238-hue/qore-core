"""Causal secondary routing correction frontier for VT31_NAS100.

Consumed development evidence only.

The rejected R5 holdout and cross-period forensics identified contextual
routing failures, not a global breaker failure. This lab keeps the original
entry grammar, source stop, target/lifecycle, ACTIVITY_L rearm budget and
global 0.60 risk scalar. It changes only what happens when a SECONDARY fallback
is authorized in a pre-entry context that was stably negative across R5/R6/R8
and the consumed 2022-2024 interval.

Renewal requires a genuinely new raid + confirmation + decision after the
rejected fallback decision. No cooldown, date label, fold identity, or future
PnL is used.
"""
...
SCHEMA = "qore.vt31.nas100.r5.secondary_routing_frontier.v1"
MARKET = "NAS100"
GLOBAL_SCALAR = Decimal("0.60")
ROUTE_POLICIES = (
    "ORIGINAL",
    "RENEW_BREAKER_CONTEXT",
    "RENEW_CASH_BULLISH",
...
def replay(path: Path, *, partition: str) -> dict[str, object]:
    ...
    if not series or series[0].instrument.symbol != MARKET:
        raise ValueError("routing frontier requires NAS100")
    ...
    for route_policy in ROUTE_POLICIES:
        ...
        mc = engine._monte_carlo(
            scaled,
            variant=f"ROUTING:{route_policy}:{partition}",
        )
