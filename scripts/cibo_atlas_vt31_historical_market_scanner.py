"""Independent consumed-only market scanner for CIBO Atlas / VT-31.

The scanner studies NAS100, SP500 and US30 market behavior independently of
trader acceptance. It consumes only already-opened R8/R6/R5 M1 evidence and the
definitive tick-corrected VT-31 ledger as a diagnostic overlay. It never opens
fresh evidence, chooses a market, changes a trader, or grants execution authority.
"""
...
def fraction(numerator: int, denominator: int) -> str | None:
    if denominator == 0:
        return None
    return fmt(Decimal(numerator) / Decimal(denominator))
...
def body_fraction(bar: OhlcSnapshot) -> Decimal | None:
    ...
    if span <= 0:
        return None
    return abs(D(bar.close) - D(bar.open)) / span
...
def _directional_path(
    *,
    ...
    if side == "high":
        ...
        raise ValueError("directional path requires high or low side")
    return max(Decimal(0), continuation) / ref_width, max(Decimal(0), reversal) / ref_width, opposite


def analyze_day(
    *, partition: str, market: str, provider: str, local_day: date, day_bars: tuple[OhlcSnapshot, ...]
) -> MarketDay | None:
    ...
    if len(reference) != 60 or len(session) != 60:
        return None
    if any(cur.opened_at != prev.closed_at for prev, cur in zip(reference, reference[1:])):
        return None
    if any(cur.opened_at != prev.closed_at for prev, cur in zip(session, session[1:])):
        return None
    ...
    first_side, first_index, first_bar, both_sides = _first_breach(session, ref_high, ref_low)
    lifecycle_complete = len(lifecycle) == 360 and all(
        cur.opened_at == prev.closed_at for prev, cur in zip(lifecycle, lifecycle[1:])
    )
    life_range = (
    ...
    reverse_11: Decimal | None = None
    reverse_16: Decimal | None = None
    if first_side in {"high", "low"} and first_index is not None and first_bar is not None:
        depth = (
            (D(first_bar.high) - ref_high) / ref_width
            if first_side == "high"
            else (ref_low - D(first_bar.low)) / ref_width
        )
        breach_body = body_fraction(first_bar)
        after_11 = session[first_index:]
        continuation, reverse_11, opposite_11 = _directional_path(
            side=first_side,
            path=after_11,
            ref_high=ref_high,
            ref_low=ref_low,
        ...
        after_16 = lifecycle[first_index:] if len(lifecycle) > first_index else after_11
        _, reverse_16, opposite_16 = _directional_path(
            side=first_side,
            path=after_16,
            ref_high=ref_high,
            ref_low=ref_low,
        ...
        for relative_index, bar in enumerate(after_11):
            close = D(bar.close)
            reentered = close < ref_high if first_side == "high" else close > ref_low
            if reentered:
    ...
    return MarketDay(
        ...
        first_breach=first_side,
        first_breach_minute=minute_from_1000(first_bar) if first_bar is not None else None,
        ...
        max_reversal_excursion_ref_by_11=reverse_11,
        max_reversal_excursion_ref_by_16=reverse_16,
...
def overlay_trader(rows: list[MarketDay], ledger: dict[str, Any]) -> list[MarketDay]:
    ...
    for root in cast(list[dict[str, Any]], ledger["roots"]):
        by_key[(str(root["market"]), str(root["ny_date"]))].append(root)
    rendered: list[MarketDay] = []
    for row in rows:
            ...
            MarketDay(
                **{
                    **{key: value for key, value in row.__dict__.items()},
                    ...
                    "trader_initial_stop_count": families.get("initial_stop", 0),
                    "trader_protected_stop_count": families.get("protected_stop", 0),
                    "trader_target_count": families.get("target", 0),
...
def summarize(rows: list[MarketDay]) -> dict[str, Any]:
    ...
    breach = Counter(row.first_breach for row in rows)
    directional = [row for row in rows if row.first_breach in {"high", "low"}]
    lifecycle = [row for row in rows if row.lifecycle_complete]
    return {
        ...
        "first_breach_counts": dict(sorted(breach.items())),
        ...
        "lifecycle_complete_rate": fraction(len(lifecycle), len(rows)),
        ...
        "reference_width_p50": fmt(quantile([row.reference_width for row in rows], Decimal("0.50"))),
        "session_range_to_reference_p50": fmt(
            quantile([row.session_range_to_reference for row in rows], Decimal("0.50"))
        ),
        "raid_depth_ref_p50": fmt(
            quantile(
                [row.first_breach_depth_ref for row in directional if row.first_breach_depth_ref is not None],
                Decimal("0.50"),
            )
        ),
        "reversal_excursion_ref_by_11_p50": fmt(
            quantile(
                [
                    row.max_reversal_excursion_ref_by_11
        ...
        "reversal_excursion_ref_by_16_p50": fmt(
            quantile(
                [
                    row.max_reversal_excursion_ref_by_16
                    for row in directional
...
def outcome_contrasts(rows: list[MarketDay]) -> dict[str, Any]:
    groups = {
        "initial_stop": [row for row in rows if row.trader_initial_stop_count > 0],
        "protected_stop": [row for row in rows if row.trader_protected_stop_count > 0],
        "target": [row for row in rows if row.trader_target_count > 0],
    }
    result: dict[str, Any] = {}
...
def cross_index(rows: list[MarketDay]) -> dict[str, Any]:
    ...
    complete = {day: markets for day, markets in by_day.items() if set(markets) == set(MARKETS)}
    cohorts: list[dict[str, Any]] = []
    for day, markets in sorted(complete.items()):
        sides = [markets[market].first_breach for market in MARKETS]
        directional = [side for side in sides if side in {"high", "low"}]
    ...
    return {
        ...
        "direction_state_counts": dict(sorted(Counter(row["direction_state"] for row in cohorts).items())),
        "cohorts": cohorts,
    }
...
def write_csv(path: Path, rows: list[MarketDay]) -> None:
    ...
    if not records:
        raise ValueError("cannot write empty Atlas day matrix")
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    all_rows: list[MarketDay] = []
    for partition in PARTITIONS:
        for market in MARKETS:
            path = cast(Path, getattr(args, f"{partition}_{market.lower()}"))
            all_rows.extend(load_partition(partition, market, path))
    ...
    for row in all_rows:
        ...
        unique[key] = row
    rows = overlay_trader(sorted(unique.values(), key=lambda r: (r.ny_date, r.market)), ledger)
    ...
    payload = {
        ...
        "interpretation_constraints": [
            ...
            "trader outcomes are post-outcome diagnostic labels only",
            "no positive cell or contrast is a candidate rule",
            "no fresh holdout was opened",
            "specialist changes require causal hypothesis plus leakage-free walk-forward",
        ],
    }
    args.output_dir.mkdir(parents=True, exist_ok=True)
    (args.output_dir / "cibo-atlas-vt31-historical-market-scanner.json").write_text(
        json.dumps(payload, sort_keys=True, indent=2) + "\n", encoding="utf-8"
    )
    write_csv(args.output_dir / "cibo-atlas-vt31-market-day-matrix.csv", rows)
    return payload


def self_test() -> None:
    assert status_family("terminal-initial-stop") == "initial_stop"
...
def main() -> None:
    ...
    payload = build(args)
    print(json.dumps({"market_summary": payload["market_summary"], "cross_index": payload["cross_index"]}, sort_keys=True))
