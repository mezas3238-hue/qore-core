...
_CORRELATION = CorrelationId(UUID("62000000-0000-0000-0000-000000000001"))
...
def _surface(platform: ExecutiveClientPlatform) -> ExecutiveClientSurface:
    result = build_executive_client_surface(
        ...
        platform=platform,
        client_version=ExecutiveClientVersion("android.v1"),
...
def _binding(surface: ExecutiveClientSurface) -> ExecutiveClientSessionBinding:
    assertion = AuthenticatedExecutivePrincipal(
        ...
        identity_boundary_ref=ExecutiveIdentityBoundaryRef("identity:android"),
        issued_at=_NOW,
    ...
    session = ExecutiveClientSession(
        ...
        device_ref=ExecutiveClientDeviceRef("device:android-reference"),
        principal_id=_PRINCIPAL,
...
def _client() -> ExecutiveAndroidReferenceClient:
    ...
    result = build_executive_android_reference_client(
        reference_id=ExecutiveAndroidReferenceId(UUID(int=6)),
        surface=surface,
...
def test_android_reference_lives_outside_core_and_uses_mobile_surface() -> None:
    client = _client()

    assert ExecutiveAndroidReferenceClient.__module__.startswith("qore_clients.")
    assert client.surface.platform is ExecutiveClientPlatform.ANDROID
...
def test_non_android_surface_fails_closed() -> None:
    ...
    result = build_executive_android_reference_client(
        reference_id=ExecutiveAndroidReferenceId(UUID(int=9)),
        surface=surface,
        session_binding=binding,
        command_center=view_result.value,
        environment=ExecutiveClientGatewayEnvironment.TEST,
...
def test_session_surface_value_mismatch_fails_closed() -> None:
    ...
    changed_surface = replace(
        surface,
        client_version=ExecutiveClientVersion("android.v2"),
    )
    view_result = build_executive_command_center_view_model(
    ...
    result = build_executive_android_reference_client(
        reference_id=ExecutiveAndroidReferenceId(UUID(int=11)),
...
def test_android_composition_cannot_outlive_bound_session() -> None:
    surface = _surface(ExecutiveClientPlatform.ANDROID)
    binding = _binding(surface)
    view_result = build_executive_command_center_view_model(
    ...
    result = build_executive_android_reference_client(
        reference_id=ExecutiveAndroidReferenceId(UUID(int=13)),
        surface=surface,
        session_binding=binding,
        command_center=view_result.value,
        environment=ExecutiveClientGatewayEnvironment.TEST,
...
def test_android_request_delegates_to_canonical_client_gateway() -> None:
    client = _client()
    result = normalize_executive_android_gateway_ingress(
        client,
        _read(),
...
def test_android_reference_exposes_no_native_or_execution_surface() -> None:
    client = _client()

    assert not hasattr(client, "activity")
    assert not hasattr(client, "compose_view")
    assert not hasattr(client, "android_context")
