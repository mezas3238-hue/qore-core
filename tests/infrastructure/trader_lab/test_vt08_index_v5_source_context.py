...
def _bar(open_: str, high: str, low: str, close: str, *, day: int = 1) -> Vt08IndexC2R1Bar:
...
def test_classifies_four_preregistered_source_closures() -> None:
    ...
    assert classify_closed_source_day(ref, _bar("102", "115", "98", "112", day=2)) is SourceClosureContext.BULLISH_CONTINUATION
    assert classify_closed_source_day(ref, _bar("102", "105", "85", "88", day=2)) is SourceClosureContext.BEARISH_CONTINUATION
    assert classify_closed_source_day(ref, _bar("102", "108", "85", "96", day=2)) is SourceClosureContext.BULLISH_REVERSAL
    assert classify_closed_source_day(ref, _bar("102", "115", "95", "105", day=2)) is SourceClosureContext.BEARISH_REVERSAL
...
def test_two_sided_source_day_fails_closed() -> None:
    ...
    result = resolve_source_context(reference=ref, subject=subject, side=DemoTradingSetupSide.LONG)
...
def test_context_direction_qualifies_only_matching_side() -> None:
    ...
    long_result = resolve_source_context(reference=ref, subject=bullish, side=DemoTradingSetupSide.LONG)
    short_result = resolve_source_context(reference=ref, subject=bullish, side=DemoTradingSetupSide.SHORT)
