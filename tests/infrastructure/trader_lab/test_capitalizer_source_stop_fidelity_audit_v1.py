...
def test_source_stop_fidelity_audit_reopens_exact_stop_placement() -> None:
    ...
    assert audit.source_stop_relation == "BEYOND_PROTECTED_SWING"
    assert audit.source_universal_numeric_padding_defined is False
    assert audit.current_trade_plan_relation == "EXACTLY_AT_PROTECTED_SWING"
    assert audit.current_trade_plan_source_faithful is False
    ...
    assert audit.source_strategy_status == "REOPEN_STOP_PLACEMENT_FIDELITY"
