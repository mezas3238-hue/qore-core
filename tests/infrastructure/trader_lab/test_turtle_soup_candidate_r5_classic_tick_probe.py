...
def test_tick_page_decodes_provider_signed_negative_deltas() -> None:
...
def test_tick_page_preserves_positive_magnitude_and_equal_timestamp_encodings() -> None:
    page = _decode_tick_page_with_signed_deltas(
        (
            SimpleNamespace(timestamp=1_700_000_000_500, tick=123456),
            SimpleNamespace(timestamp=125, tick=123450),
            SimpleNamespace(timestamp=0, tick=123449),
        ),
        digits=5,
    )
    assert [item.timestamp_ms for item in page] == [
