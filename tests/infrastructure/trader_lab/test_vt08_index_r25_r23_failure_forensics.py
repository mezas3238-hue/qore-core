...
def test_r25_quality_tiers_are_diagnostic_labels_not_new_weights() -> None:
    assert r24.BASE_QUALITY.base_weight == Decimal("0.005")
    assert r24.BASE_QUALITY.tier_a_weight == Decimal("1.50")
    assert r24.BASE_QUALITY.tier_b_weight == Decimal("0.75")
    assert r24.BASE_QUALITY.tier_c_weight == Decimal("0.10")


def test_r25_keeps_r23_realized_governor_fixed() -> None:
    assert r24.BASE_RISK.rolling_trades == 60
    assert r24.BASE_RISK.warn_dd_r == Decimal("1.25")
    assert r24.BASE_RISK.hard_dd_r == Decimal("1.50")
    assert r24.BASE_RISK.portfolio_risk_budget_r == Decimal("0.75")
