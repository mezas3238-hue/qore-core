...
from qore.infrastructure.trader_lab import (
    turtle_soup_audjpy_r35_confidence_tier_ensemble as r35,
)
...
def test_r35_direction_is_first_authority_layer() -> None:
    assert r35.CORE == "R34_DIRECTION_REGIME_ROUTE_TYPES"
    for layers in r35.ENSEMBLES.values():
        assert layers[0][0] == r35.CORE
        assert set(layers[0][1]) == {r35.ROBUST, r35.MAJORITY}


def test_r35_expansion_is_predeclared_and_audjpy_memory_only() -> None:
    allowed = {r35.CORE, r35.MINIMAL, r35.TIMEFRAME}
    for layers in r35.ENSEMBLES.values():
        schemes = {scheme for scheme, _classes in layers}
        assert schemes <= allowed
        assert r35.RANGE_FALSIFIED not in schemes
