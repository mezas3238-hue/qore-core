...
def _activation(*, ready: bool = True) -> ProviderActivationDecisionSnapshot:
    provider = _provider()
    source = _source()
    ...
    assert result.value.can_activate is ready
    return result.value
...
def _transport(
    payload: bytes,
) -> tuple[ConcreteBearerHttpsExternalTransport, FakeConnection]:
...
def test_oanda_practice_quote_flows_through_canonical_ingestion() -> None:
    ...
    assert connection.calls[0][1] == (
        "/v3/accounts/practice-001/pricing?instruments=EUR_USD"
    )
...
def test_blocked_activation_fails_before_provider_network_composition() -> None:
    transport, connection = _transport(b'{"prices":[]}')

    result = compose_oanda_practice_read_only_provider(
        configuration=_configuration(),
        credential=_credential(),
