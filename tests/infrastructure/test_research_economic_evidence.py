...
def test_gross_result_requires_bound_execution_model() -> None:
    built = _gross(_run(execution_model=False))
    assert isinstance(built, Failure)
...
def test_cost_coverage_is_explicit_and_unknown_is_not_zero() -> None:
    run = _run()
    ...
    assert complete.value.complete
...
def test_unknown_required_cost_blocks_net_result() -> None:
    run = _run()
    gross = _gross(run)
...
def test_net_pnl_uses_cash_adjustments_without_spread_double_counting() -> None:
    run = _run()
...
def test_signed_cash_adjustment_allows_evidence_backed_rebate() -> None:
    run = _run()
    gross = _gross(run, pnl="100")
    assert isinstance(gross, Success)
    coverage = build_research_cash_cost_coverage(
...
def test_return_requires_capital_basis_and_rejects_tampering() -> None:
    gross = _gross(_run(), pnl="100")
    assert isinstance(gross, Success)
    returned = build_research_return_observation(
        observation_id=ResearchReturnObservationId(_uuid(650)),
    ...
    zero = build_research_return_observation(
        observation_id=ResearchReturnObservationId(_uuid(651)),
        source_result=gross.value,
