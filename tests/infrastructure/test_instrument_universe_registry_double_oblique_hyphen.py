...
_DOUBLE_OBLIQUE_HYPHEN = "\u2e17"


@pytest.mark.parametrize("label", ("api", "private"))
@pytest.mark.parametrize("delimiter", ("=", ":"))
def test_reason_constructor_rejects_double_oblique_hyphen_sensitive_assignment(
    label: str,
    delimiter: str,
) -> None:
    value = f"{label}{_DOUBLE_OBLIQUE_HYPHEN}key{delimiter}PLAINTEXT-SECRET"
...
@pytest.mark.parametrize("label", ("api", "private"))
def test_reason_revalidation_rejects_double_oblique_hyphen_sensitive_assignment(
    label: str,
) -> None:
    value = f"{label}{_DOUBLE_OBLIQUE_HYPHEN}key=PLAINTEXT-SECRET"
...
def test_evidence_revalidation_rejects_double_oblique_hyphen_sensitive_assignment(
    field_name: str,
) -> None:
    ...
    object.__setattr__(
        record,
        field_name,
        f"private{_DOUBLE_OBLIQUE_HYPHEN}key=PLAINTEXT-SECRET",
    )
...
def test_benign_double_oblique_hyphen_text_is_retained_byte_for_byte() -> None:
    value = f"Evidence{_DOUBLE_OBLIQUE_HYPHEN}reference"
