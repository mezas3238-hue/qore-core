"""Frozen incremental-information probe for WP-04 invariant representation V2.

The invariant representation is learned without future targets. Only after that
representation is frozen may consumed matured targets be used to fit a simple
incremental-information probe. The probe compares:

    existing ontology
    versus
    existing ontology + frozen invariant latent concepts

The probe itself is not Shared runtime cognition and cannot promote knowledge.
An independent one-shot holdout must be evaluated without refitting before
WP-04 can satisfy its exit gate.
"""
...
class InvariantIncrementalProbe:
    fitted_at: datetime
    ...
    def __post_init__(self) -> None:
        ...
        if len(set(self.calibration_partitions)) != len(
            self.calibration_partitions
        ):
        ...
        if not self.representation_target_blind or self.holdout_used_for_fit:
            raise ValueError("probe must preserve frozen target-blind representation")
        if (
            self.identity_used
            ...
            raise ValueError("invariant probe cannot carry trading authority")


@dataclass(frozen=True, slots=True)
class InvariantIncrementalEvaluation:
    partition: str
    target_name: str
    sample_count: int
...
def invariant_representation_fingerprint(
    ...
    payload = (
        ...
        tuple(
            (
                item.feature_name,
                item.ontology_coefficients_micros,
            )
        ...
        tuple(
            (
                concept.concept_id,
                concept.loading_micros,
                concept.discovery_explained_residual_variance_bps,
                concept.source_invariance_bps,
            )
...
def _solve(matrix: list[list[float]], vector: list[float]) -> list[float]:
    ...
    for column in range(size):
        ...
        if abs(augmented[pivot][column]) < 1e-12:
            raise ValueError("invariant probe linear system is singular")
...
def _design(
    ...
    *,
    ...
    if not augmented:
        return baseline
    latent = project_invariant_representation(
        model=model,
        episode=episode,
    )
    activations = {
...
def fit_invariant_incremental_probe(
    *,
    model: InvariantRepresentationModel,
    fitted_at: datetime,
    calibration_partitions: tuple[str, ...],
    ...
    ridge: float = 1.0,
) -> InvariantIncrementalProbe:
    """Fit a frozen ontology-vs-latent probe on consumed target evidence."""
    ...
    if len(rows) < minimum:
        raise ValueError("insufficient calibration rows for invariant probe")
    ...
    baseline_float = _ridge_fit(
        baseline_design,
        response,
        ridge=ridge,
    )
    augmented_float = _ridge_fit(
        augmented_design,
        response,
        ridge=ridge,
    )
    baseline = tuple(
        ...
        for value in baseline_float
    )
    augmented = tuple(
        int(round(value * 1_000_000))
        for value in augmented_float
    )
    ...
    return InvariantIncrementalProbe(
        fitted_at=fitted_at,
        calibration_partitions=calibration_partitions,
        target_name=target_name,
        representation_fingerprint=representation_fingerprint,
...
def _predict(
    ...
    if len(coefficients_micros) != len(row):
        raise ValueError("invariant probe coefficient shape mismatch")
    prediction = sum(
    ...
    if not isfinite(prediction):
        raise ValueError("non-finite invariant probe prediction")
    return prediction


def evaluate_invariant_incremental_probe(
    *,
    model: InvariantRepresentationModel,
    probe: InvariantIncrementalProbe,
    partition: str,
    episodes: tuple[RepresentationEpisode, ...],
    targets: tuple[RepresentationEvaluationTarget, ...],
) -> InvariantIncrementalEvaluation:
    """Evaluate the exact frozen probe without refitting."""
    ...
    if probe.representation_fingerprint != invariant_representation_fingerprint(
        model
    ):
    ...
    if probe.concept_ids != tuple(
        item.concept_id for item in model.concepts
    ):
    ...
    return InvariantIncrementalEvaluation(
        partition=partition,
