"""One-shot read-only cTrader DEMO acquisition for the VT-08 Index V6 holdout.

The request is deliberately bounded at the sealed holdout end.  No bar after
2022-09-15 00:00 America/New_York is requested from cTrader.
"""
...
ACQUISITION_START_NY = datetime(2020, 8, 1, 0, 0, tzinfo=_NY)
HOLDOUT_START_NY = datetime(2020, 9, 15, 0, 0, tzinfo=_NY)
HOLDOUT_END_EXCLUSIVE_NY = datetime(2022, 9, 15, 0, 0, tzinfo=_NY)


def fixed_acquisition_window_utc() -> tuple[datetime, datetime]:
    """Return the immutable request interval, including pre-holdout context."""
...
def main() -> None:
    ...
    try:
        ...
        payload["v6_holdout_acquisition"] = {
            ...
            "freshness_claim": "unseen-before-v6-freeze",
        }
        print(
            json.dumps(
                payload,
