...
deployed twice. It models capital capacity only; it performs no broker action.
"""
...
class CapitalSourceAccount:
    ...
    def __post_init__(self) -> None:
        ...
        if self.reserved_usd + self.deployed_usd > self.proven_amount_usd:
            raise CiboCapitalManagementError("capital source is over-allocated")

    @property
    def available_usd(self) -> Decimal:
        return self.proven_amount_usd - self.reserved_usd - self.deployed_usd
...
class CapitalReservation:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.amount_usd, Decimal) or not self.amount_usd.is_finite():
            raise CiboCapitalManagementError("reservation amount must be finite Decimal")
...
class CapitalSourceLedger:
    ...
    def release(self, reservation_id: str) -> CapitalSourceLedger:
        reservation = self._reservation(reservation_id)
        if reservation.state is ReservationState.RELEASED:
            raise CiboCapitalManagementError("reservation already released")
        account = self._account(reservation.source_id)
