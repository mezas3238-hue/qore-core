...
This bootstrap is read-only. It authenticates the Open API application, queries the
account list attached to the supplied access token, refreshes the token in memory if
necessary, and returns an account id only when exactly one account is explicitly DEMO.
"""
...
def _ensure_reactor_running(reactor: object) -> None:
    ...
    with _REACTOR_LOCK:
        ...
        while (
            getattr(reactor, "running", None) is not True
    ...
    *,
    ...
    for name, value in (
        ...
        if not isinstance(value, str) or not value:
            raise CTraderDemoAccountDiscoveryError(
                f"{name} must be a non-empty secret"
            )
    ...
    accounts_req_type = bindings.messages.get(
        "ProtoOAGetAccountListByAccessTokenReq"
    )
    accounts_res_type = bindings.messages.get(
    ...
    _method(client, "setDisconnectedCallback")(
        lambda received, reason: disconnected.set()
        if received is client
        else None
    )
    ...
    def request(message: object, client_msg_id: str) -> object:
        ...
        if not completed.wait(timeout_seconds + 0.5):
            raise CTraderDemoAccountDiscoveryError(
                "cTrader discovery request timed out"
            )
        if failures or not responses:
            raise CTraderDemoAccountDiscoveryError(
                "cTrader discovery request failed"
            )
        try:
    ...
    try:
        ...
        app_response = request(
            app_req, "qore-demo-account-discovery-app-auth"
        )
        ...
        def account_list(token: str, suffix: str) -> object:
            ...
            return request(
                account_req, f"qore-demo-account-discovery-{suffix}"
            )

        accounts_response = account_list(
            current_access_token, "account-list"
        )
        if not isinstance(accounts_response, accounts_res_type):
            ...
            refreshed = request(
                refresh_req, "qore-demo-account-discovery-refresh"
            )
        ...
        return select_single_demo_account_id(
            cast(Iterable[object], entries)
        )
    finally:
