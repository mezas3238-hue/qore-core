"""Five-year validation of the exact frozen GBPUSD R37 candidate.

This is consumed robustness validation, not a fresh holdout.
Window: 2021-09-17 <= entry < 2026-09-17 UTC.
No signal, target, stop, lifecycle, family or risk rule is re-optimized here.
"""
...
IDENTITY = "TURTLE_SOUP_GBPUSD_R39_FROZEN_R37_5Y_VALIDATION_V1"
...
def run(
    ...
    trades_path = output / "r39-5y-scaled-trades.jsonl"
    with trades_path.open("w", encoding="utf-8") as handle:
        for row in rows:
            handle.write(json.dumps(asdict(row), sort_keys=True) + "\n")

    report: dict[str, Any] = {
        "schema": "qore.turtle_soup_gbpusd.r39_frozen_r37_5y_validation.v1",
        "identity": IDENTITY,
    ...
    (output / "r39-5y-validation-report.json").write_text(
