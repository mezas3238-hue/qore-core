...
_EXECUTION_MODEL = (
    "nested-crt-market-entry-1130-ny-h4-target-stop-first-censor-v1"
)
...
class Vt08Crt159V3Report:
    ...
    def payload(self) -> dict[str, object]:
        terminal = tuple(
            item for item in self.setups if item.r_multiple is not None
        )
        values = tuple(
            cast(Decimal, item.r_multiple) for item in terminal
        )
        wins = sum(item.outcome == "target" for item in terminal)
        win_rate = (
            Decimal(wins) / Decimal(len(terminal))
            if terminal
            else Decimal(0)
        )
        return {
            ...
            "target_count": sum(
                item.outcome == "target" for item in terminal
            ),
            "stop_count": sum(
                item.outcome == "stop" for item in terminal
            ),
            "h4_close_censored_count": sum(
                item.outcome == "h4_close_censored"
                for item in self.setups
            ),
            "gap_censored_count": sum(
                item.outcome == "gap_censored" for item in self.setups
            ),
            "long_trade_count": sum(
                item.side == "long" for item in self.setups
            ),
            "short_trade_count": sum(
                item.side == "short" for item in self.setups
            ),
            ...
            "daily_target_touch_count": sum(
                item.daily_target_touched for item in self.setups
            ),
...
def _text(value: object, *, field: str) -> str:
    if type(value) is not str or not value:
        raise Vt08Crt159V3BacktestError(
...
def _decimal(value: object, *, field: str) -> Decimal:
    ...
    if not result.is_finite() or result <= 0:
        raise Vt08Crt159V3BacktestError(
            f"{field} must be positive finite"
        )
    return result
...
def _load(
    ...
    if re.fullmatch(r"[0-9a-f]{64}", account) is None:
        raise Vt08Crt159V3BacktestError(
            "account_fingerprint must be SHA-256"
        )
    software_sha = _text(
        payload.get("software_sha"),
        field="software_sha",
    )
    if re.fullmatch(r"[0-9a-f]{40}", software_sha) is None:
        raise Vt08Crt159V3BacktestError(
            "software_sha must be exact Git SHA"
        )
    periods = _object(payload.get("periods"), field="periods")
    if set(periods) != {"M15", "D1"}:
        raise Vt08Crt159V3BacktestError(
            "VT-08 V3 evidence requires exact M15 and D1"
        )
    m15 = tuple(
        _bar(item)
        for item in _array(periods.get("M15"), field="M15")
    )
    d1 = tuple(
        _bar(item)
        for item in _array(periods.get("D1"), field="D1")
    )
    if m15 != tuple(sorted(m15, key=lambda item: item.opened_at)):
        raise Vt08Crt159V3BacktestError(
            "M15 evidence must be chronological"
        )
    if d1 != tuple(sorted(d1, key=lambda item: item.opened_at)):
        raise Vt08Crt159V3BacktestError(
            "D1 evidence must be chronological"
        )
    material = json.dumps(
        payload,
        sort_keys=True,
...
def _terminal_trade(
    ...
    if risk <= 0:
        raise Vt08Crt159V3BacktestError(
            "risk geometry must be positive"
        )
    expected_path = 6
    ...
    for bar in path:
        if side is DemoTradingSetupSide.LONG:
            ...
            daily_touched = (
                daily_touched or bar.high >= setup.daily_target
            )
            ...
            daily_touched = (
                daily_touched or bar.low <= setup.daily_target
            )
...
def run_vt08_v3_backtest(path: Path) -> Vt08Crt159V3Report:
    ...
    by_local_open = {
        item.opened_at.astimezone(_NY): item for item in m15
    }
    local_days = sorted(
        {item.opened_at.astimezone(_NY).date() for item in m15}
    )
    decision_days = 0
    daily_pass = 0
    h4_pass = 0
    ...
    for local_day in local_days:
        ...
        prior_daily = tuple(
            item for item in d1 if item.closed_at <= h4_ref.opened_at
        )
        ...
        evaluation = evaluate_vt08_crt_159_v3(
            Vt08Crt159V3Input(
                ...
                h4_distribution_closes_at=(
                    expiry_local.astimezone(UTC)
                ),
            )
        )
        if (
            evaluation.decision is not DemoTradingDecision.SETUP
            or evaluation.setup is None
        ):
            continue
        path_bars = _exact_window(
            by_local_open,
            local_day,
            11,
            30,
...
def main(argv: list[str] | None = None) -> None:
    ...
    if len(args) != 1:
        raise SystemExit(
            "usage: vt08_crt_159_v3_backtest <market-evidence.json>"
        )
