"""Turtle Soup AUDJPY Specialist Memory V1.

Creates a derived, trader-specific copy of the immutable CIBO AUDJPY 10Y
master memory.
...
1. SPECIALIST RAW MEMORY
   - one snapshot row for every CIBO AUDJPY behavior event;
...
IDENTITY = "TURTLE_SOUP_AUDJPY_SPECIALIST_MEMORY_V1"
RAW_IDENTITY = "TURTLE_SOUP_AUDJPY_SPECIALIST_RAW_MEMORY_V1"
COGNITIVE_IDENTITY = "TURTLE_SOUP_AUDJPY_SPECIALIST_COGNITIVE_MEMORY_V1"

MASTER_RAW_RUN_ID = 35166210458
MASTER_RAW_ARTIFACT_ID = 10476530915
MASTER_JOURNEY_RUN_ID = 35175979474
MASTER_TARGET_RUN_ID = 35204892665
MASTER_TARGET_ARTIFACT_ID = 10489761448
...
def build(
    ...
    for event in events:
        ...
        raw_rows.append(
            {
                "schema": "qore.turtle_soup_audjpy.specialist_raw_memory.v1",
    ...
    for episode_id, setup in sorted(
        ...
        for target in ladder:
            ...
            row: dict[str, Any] = {
                "schema": "qore.turtle_soup_audjpy.specialist_decision_observation.v1",
    ...
    raw_path = output / "turtle-soup-audjpy-specialist-raw-memory.jsonl"
    observation_path = output / "turtle-soup-audjpy-specialist-observations.jsonl"
    _write_jsonl(raw_path, raw_rows)
    _write_jsonl(observation_path, observations)
    (output / "turtle-soup-audjpy-specialist-cognitive-memory.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )

    report = {
        "schema": "qore.turtle_soup_audjpy.specialist_memory_report.v1",
        "identity": IDENTITY,
    ...
    (output / "turtle-soup-audjpy-specialist-memory-report.json").write_text(
