"""VT08 Index R20 — static formation-quality portfolio, dual-window screen.

R20 preserves the complete R8 2.5R execution stream in both consumed
development windows. It removes PnL-state governors entirely and allocates
non-zero risk only from the three pre-entry FVG formation-quality cohorts that
R17 found positive in BOTH windows.

The same static quality scheme must pass 5Y and recent-2Y simultaneously.
No trade is skipped. No outcome or future information is used in weighting.
"""
...
SCHEMA = "qore.trader_lab.vt08_index_r20_static_quality_portfolio.v1"
IDENTITY = "VT08_INDEX_R20_STATIC_QUALITY_PORTFOLIO_001"
...
def _schemes() -> tuple[QualityScheme, ...]:
    return tuple(
        QualityScheme(base, tier_b, tier_c)
        for base, tier_b, tier_c in itertools.product(
            (
                Decimal("0.01"),
                Decimal("0.025"),
                Decimal("0.05"),
