"""VT08 Index R34 — hybrid formation-tier + POI-family causal health.

R31: static structural hierarchy + formation-tier health kept PF high and DD low,
but 2019 remained negative.
R33: POI-family health repaired 2019, but replacing the structural hierarchy
collapsed aggregate PF and made 2023 negative.

R34 combines the two causal layers without changing the 2,448-trade signal
population:
1) preserve R31/R29 structural quality weights and frozen A/B formation-tier
   health;
2) overlay a rolling FVG/CISD/relevant-swing family-health multiplier based
   only on completed prior trades.

Because all signals retain the 0.005 floor, the overlay can reduce requested
risk but cannot suppress a valid structural signal.
"""
...
SCHEMA = "qore.trader_lab.vt08_index_r34_hybrid_formation_poi_health.v1"
IDENTITY = "VT08_INDEX_R34_HYBRID_FORMATION_POI_HEALTH_001"
...
def _profiles() -> tuple[PoiOverlayProfile, ...]:
    return tuple(
        PoiOverlayProfile(window, minimum, cold, weak, threshold)
...
def _assign(
    ...
    *,
    ...
    exit_heap: list[
        tuple[datetime, int, Decimal, str, str, Decimal]
    ] = []
    ...
    hard_allocations = 0
    loss_allocations = 0
    poi_weak_allocations = 0
    poi_healthy_allocations = 0
    poi_cold_allocations = 0
    tier_weak_allocations = 0
    tier_healthy_allocations = 0
    tier_cold_allocations = 0
    ...
    while cursor < len(stream):
        ...
        for opportunity, _outcome in batch:
            ...
            if family not in poi_histories:
            ...
            if tier in tier_histories:
                ...
                weight *= tier_mult
                tier_cold_allocations += int(tier_state == "COLD")
                tier_weak_allocations += int(tier_state == "WEAK")
                tier_healthy_allocations += int(tier_state == "HEALTHY")

            poi_mult, poi_state = _health(
            ...
            weight *= poi_mult
            poi_cold_allocations += int(poi_state == "COLD")
            poi_weak_allocations += int(poi_state == "WEAK")
            poi_healthy_allocations += int(poi_state == "HEALTHY")
            ...
            metadata.append(
                (tier, family, realized_warn, realized_hard, realized_loss)
            )
        ...
        for ((opportunity, outcome), weight, meta) in zip(
            ...
            tier, family, realized_warn, realized_hard, realized_loss = meta
    ...
    if len(assigned) != len(stream):
        raise ValueError("R34 assignment changed source-complete trade count")
    ...
    return tuple(assigned), {
        ...
        "poi_cold_allocations": poi_cold_allocations,
        "poi_weak_allocations": poi_weak_allocations,
        "poi_healthy_allocations": poi_healthy_allocations,
        "tier_cold_allocations": tier_cold_allocations,
        "tier_weak_allocations": tier_weak_allocations,
        "tier_healthy_allocations": tier_healthy_allocations,
...
def _row(
    ...
    *,
    ...
    assigned, diagnostics = _assign(stream, overlay=overlay)
...
def build_report(
    *,
    ...
    if not contract.validates_trade_count(years=5, sample=len(stream)):
        raise ValueError(f"R34 density drift: {len(stream)}")
    ...
    search = [_row(stream, overlay=profile) for profile in _profiles()]
    ...
    return {
        ...
        "identity": IDENTITY,
        "sample": len(stream),
        "overlay_profile_count": len(_profiles()),
        ...
        "frozen_formation_health": r29.FORMATION_HEALTH,
        "frozen_structural_quality": r29.QUALITY,
        "contract": {
        ...
        "governance": {
            ...
            "r29_structural_hierarchy_preserved": True,
            "poi_overlay_is_causal_completed_trade_memory": True,
        ...
        json.dumps(
            {
                ...
                "overlay_profile_count": report["overlay_profile_count"],
