"""VT08 CRT PURE — common-window canonical M5 market consumer.

Research-only. Reads cTrader DEMO M5 evidence for the frozen three-market CRT PURE
...
IDENTITY = "VT08_CRT_PURE_M5_CONSUMPTION_V1"
PARENT_IDENTITY = "VT08_CRT_PURE_MARKET_EVIDENCE_PROBE_V1"
SCHEMA = "qore.vt08.crt_pure.m5_consumption.v1"
RAW_SCHEMA = "qore.vt08.crt_pure.raw_m5.v1"
QUALITY_SCHEMA = "qore.vt08.crt_pure.data_quality.v1"
PERIOD_M5 = 5
REQUEST_COUNT = 5000
REQUEST_PAUSE_SECONDS = 0.12
TARGET_START = datetime(2017, 1, 1, 0, 0, tzinfo=UTC)
TARGET_END_EXCLUSIVE = datetime(2026, 9, 21, 0, 0, tzinfo=UTC)
CHUNK_DAYS = 7
    ...
    *,
    ...
    for index, (opened_at, closed_at) in enumerate(chunk_grid(partition)):
        native_bars = _read_chunk(
            ...
            closed_at=closed_at,
            client_msg_id=(
                f"vt08-crt-pure-m5:{provider_symbol_id}:{partition.label}:{index}"
            ),
        )
...
def _selected_symbol(
    ...
    *,
    ...
    listed = client.request(
        ...
        client_msg_id="vt08-crt-pure-m5-symbol-list",
    ...
    details = client.request(
        ...
        client_msg_id=f"vt08-crt-pure-m5-symbol:{symbol_id}",
        timeout_seconds=30.0,
    )
...
def consume_symbol(canonical_symbol: str, output: Path) -> SymbolConsumptionManifest:
    if canonical_symbol not in TARGET_SYMBOLS:
        raise ValueError(f"symbol outside frozen CRT PURE scope: {canonical_symbol}")
