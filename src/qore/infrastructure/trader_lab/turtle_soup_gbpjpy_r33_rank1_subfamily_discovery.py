"""R33 GBPJPY rank-1 structural subfamily discovery.
...
IDENTITY = "TURTLE_SOUP_GBPJPY_R33_RANK1_SUBFAMILY_DISCOVERY_V1"
...
def _load_rank1(root: Path) -> list[dict[str, Any]]:
    ...
    path = _single(root, "turtle-soup-gbpjpy-specialist-observations-v2.jsonl")
    with path.open(encoding="utf-8") as handle:
        for line in handle:
            ...
            if int(row["target_rank"]) == 1:
                rows.append(row)
    if not rows:
        raise ValueError("no GBPJPY rank-1 observations")
    return rows
...
def build(v2_root: Path, output: Path) -> dict[str, Any]:
    ...
    payload = {
        ...
        "identity": IDENTITY,
        "source": {
            "cognitive_memory": "TURTLE_SOUP_GBPJPY_SPECIALIST_COGNITIVE_MEMORY_V2",
            "rank": 1,
            "lifecycle": "STATIC",
    ...
    (output / "r33-rank1-subfamily-freeze.json").write_text(
        json.dumps(
            {
                "identity": "TURTLE_SOUP_GBPJPY_R33_SUBFAMILY_FREEZE_V1",
