"""R35 EURUSD loss-sequence capital-protection lab.
...
IDENTITY = "TURTLE_SOUP_EURUSD_R35_LOSS_SEQUENCE_GOVERNOR_LAB_V1"
...
LOSS_SEQUENCE_GOVERNORS: dict[str, tuple[Decimal, ...]] = {
    # Scale is chosen only from the number of consecutive prior losing trades.
    # A non-losing trade resets the sequence immediately.
    "FIXED_1R": (Decimal("1"),),
    "L1_075_L2_050_L3_025": (
        Decimal("1"),
        Decimal("0.75"),
        Decimal("0.50"),
        Decimal("0.25"),
...
def _loss_sequence_scale(
    prior_losing_streak: int,
    policy: tuple[Decimal, ...],
) -> Decimal:
    index = min(prior_losing_streak, len(policy) - 1)
    return policy[index]


def _loss_sequence_stat(
    trades: Sequence[r26.RuntimeTrade],
    governor_name: str,
) -> dict[str, Any]:
    policy = LOSS_SEQUENCE_GOVERNORS[governor_name]
    equity = Decimal(0)
    peak = Decimal(0)
    max_dd = Decimal(0)
    gains = Decimal(0)
    losses = Decimal(0)
    prior_losing_streak = 0
    max_losing_streak = 0
    scales: Counter[str] = Counter()

    for trade in trades:
        scale = _loss_sequence_scale(prior_losing_streak, policy)
        scales[str(scale)] += 1
        contribution = trade.net_010_r * scale
        equity += contribution
        peak = max(peak, equity)
        max_dd = max(max_dd, peak - equity)
        if trade.net_010_r < 0:
            losses += -contribution
            prior_losing_streak += 1
            max_losing_streak = max(max_losing_streak, prior_losing_streak)
        else:
            if contribution > 0:
                gains += contribution
            prior_losing_streak = 0
...
def _run_family_set(
    *,
    ...
    for governor_name in LOSS_SEQUENCE_GOVERNORS:
        stat = _loss_sequence_stat(trades, governor_name)
...
def run(
    ...
    payload = {
        "schema": "qore.turtle_soup_eurusd.r35_loss_sequence_governor.v1",
        "identity": IDENTITY,
        "window": {
        ...
        "experiment_contract": {
            ...
            "loss_sequence_governors_predeclared": {
                name: [str(value) for value in rule]
                for name, rule in LOSS_SEQUENCE_GOVERNORS.items()
            },
            "governor_uses_only_prior_trade_outcomes": True,
            "governor_resets_after_non_loss": True,
    ...
    (output / "r35-loss-sequence-governor-report.json").write_text(
