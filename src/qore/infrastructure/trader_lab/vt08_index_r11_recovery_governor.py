"""VT08 Index R11 — causal recovery governor on fixed R8 execution stream.

R11 preserves every R8 2.5R execution. It combines bounded pre-entry context
weights with a loss-cluster governor that can recover immediately; unlike R6,
there is no all-time-peak drawdown state that can trap the system permanently.

All state is observable before the next trade. No trade receives zero risk.
The 2018-09-15..2023-09-15 window is consumed development evidence.
"""
...
class RecoveryScheme:
    ...
    short_cap: Decimal
    loss_trigger: int
    defensive_multiplier: Decimal
    hard_trigger: int
    hard_multiplier: Decimal
    strong_context_bypass: bool
    ...
    def scheme_id(self) -> str:
        return (
            ...
            f"L{self.loss_trigger}x{self.defensive_multiplier}-"
            f"H{self.hard_trigger}x{self.hard_multiplier}-"
            f"B{int(self.strong_context_bypass)}"
        )
    ...
    def payload(self) -> dict[str, Any]:
        return {
            ...
            "defensive_multiplier": str(self.defensive_multiplier),
            "hard_trigger": self.hard_trigger,
            "hard_multiplier": str(self.hard_multiplier),
            "strong_context_bypass": self.strong_context_bypass,
        }


def _schemes() -> tuple[RecoveryScheme, ...]:
    rows: list[RecoveryScheme] = []
    for (
        aligned_weight,
        c2_cap,
        short_cap,
        ...
        rows.append(
            RecoveryScheme(
                ...
                strong_context_bypass=bypass,
            )
        )
    return tuple(rows)


def _base_weight(context: r10.Context, scheme: RecoveryScheme) -> Decimal:
    weight = (
        Decimal("1")
    ...
    if weight <= 0:
        raise ValueError("R11 base weight may not be zero")
    return weight
...
def _weighted_values(
    ...
    *,
    ...
    for outcome, context in zip(outcomes, contexts, strict=True):
        ...
        strong = context.previous_source_day_body_opposed or context.rearm

        if loss_streak >= scheme.hard_trigger:
            if scheme.strong_context_bypass and strong:
                bypass_trades += 1
            else:
                weight *= scheme.hard_multiplier
                hard_trades += 1
        elif loss_streak >= scheme.loss_trigger:
            if scheme.strong_context_bypass and strong:
                bypass_trades += 1
            else:
                weight *= scheme.defensive_multiplier
                defensive_trades += 1

        if weight <= 0:
            raise ValueError("R11 may not skip a trade")

        value = (outcome.r_multiple - stress) * weight
        values.append(value)
        weights.append(weight)

        # State for the NEXT trade uses only this completed outcome.
    ...
    return tuple(values), {
        ...
        "strong_context_bypass_trades": bypass_trades,
    }
    ...
    *,
    ...
    return {
        ...
        "governance": {
            ...
            "strong_pre_entry_context_can_restore_risk": True,
