"""Five-year validation of the exact frozen AUDJPY R39/R38 candidate.

This is consumed robustness validation, not a fresh holdout.
Window: 2021-09-17 <= entry < 2026-09-17 UTC.

The signal, entry, DOL, Protected Swing, authority-layer ordering and structural confidence and fragility
risk policy are frozen before this replay. The five annual blocks must all be
positive for the candidate to pass the Owner's 5Y gate.
"""
...
IDENTITY = "TURTLE_SOUP_AUDJPY_R40_FROZEN_R39_5Y_VALIDATION_V1"
...
FROZEN_ENSEMBLE = "R38_FROZEN_SIGNAL_BASELINE"
FROZEN_POLICY = "AUDJPY_CONFIDENCE_100_075_025"
FROZEN_LAYERS = r38.ENSEMBLES[FROZEN_ENSEMBLE]
...
def _verify_freeze(root: Path) -> dict[str, Any]:
    freeze = _load_json(root, "r39-candidate-freeze-manifest.json")
    ...
    expected_policy = r38.RISK_POLICIES[FROZEN_POLICY]
    ...
    required_true = (
        ...
        "fragility_uses_pre_entry_context_only",
    )
    if any(contract[key] is not True for key in required_true):
        raise ValueError("frozen structural contract drift")
    if contract["r34_range_used_for_authority"] is not False:
        raise ValueError("falsified RANGE authority drift")
    if tuple(contract["fragility_flags"]) != r38.FRAGILITY_FLAGS:
        raise ValueError("fragility flag drift")
    if tuple(contract["fragility_policy_0_1_2_3plus"]) != tuple(
        str(value) for value in r38.FRAGILITY_POLICY
    ):
...
def run(
    ...
    for setup in setups:
        ...
        runtime = r26._simulate(
            setup,
            decision=r38._runtime_decision(decision),
            ladder=ladder,
            entry_at=entry_at,
            entry=entry,
            evidence=evidence,
        ...
        scale = r38._risk_scale(decision, FROZEN_POLICY)
        ...
        rows.append(
            ScaledTrade(
                ...
                fragility_flags=decision.fragility_flags,
                fragility_flag_count=len(decision.fragility_flags),
    ...
    with (output / "r40-5y-scaled-trades.jsonl").open(
        "w",
        encoding="utf-8",
    ) as handle:
        for row in rows:
    ...
    report: dict[str, Any] = {
        "schema": "qore.turtle_soup_audjpy.r40_frozen_r39_5y_validation.v1",
        "identity": IDENTITY,
        "candidate": {
            ...
            "fragility_policy_0_1_2_3plus": freeze["frozen_contract"][
        ...
        "frozen_contract_verification": {
            ...
            "fragility_policy_changed": False,
    ...
    (output / "r40-5y-validation-report.json").write_text(
