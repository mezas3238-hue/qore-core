"""R38 EURUSD five-year drawdown-forensics correction.

The R36 signal/entry/stop/DOL contract remains intact.  R38 adds only two
pre-entry structural risk overlays identified after consuming the failed R37
5Y robustness result.  Therefore this is consumed corrective research, not an
independent holdout.
"""
...
IDENTITY = "TURTLE_SOUP_EURUSD_R38_5Y_STRUCTURAL_RISK_CORRECTION_V1"
...
def run(
    ...
    for setup in setups:
        ...
        base_scale = r36._fragility_scale(len(flags), GOVERNOR_RULE)
        overlay_scale = Decimal("1")
        if (
            decision.family == r36.F5
            and runtime.side == "short"
        ):
            overlay_scale *= F5_SHORT_OVERLAY_SCALE
        if (
            runtime.side == "long"
            and runtime.target_route == UNSTABLE_LONG_ROUTE
        ):
            overlay_scale *= UNSTABLE_LONG_ROUTE_OVERLAY_SCALE
        scale = base_scale * overlay_scale
        ...
        rows.append(
            ScaledTrade(
                ...
                base_fragility_scale=str(base_scale),
                structural_overlay_scale=str(overlay_scale),
        ...
        counts[f"BASE_SCALE_{base_scale}"] += 1
        counts[f"OVERLAY_SCALE_{overlay_scale}"] += 1
    ...
    trade_path = output / "r38-5y-corrected-trades.jsonl"
    with trade_path.open("w", encoding="utf-8") as handle:
        for row in rows:
            handle.write(json.dumps(asdict(row), sort_keys=True) + "\n")

    report: dict[str, Any] = {
        "schema": "qore.turtle_soup_eurusd.r38_5y_structural_risk_correction.v1",
        "identity": IDENTITY,
        "candidate": {
        ...
        "window": {
            ...
            "years": 5,
            "fresh_holdout": False,
            "historical_status": "CONSUMED_FORENSIC_CORRECTION",
        },
        "correction_contract": {
            ...
            "base_fragility_governor_changed": False,
            ...
            "f5_short_overlay_scale": str(F5_SHORT_OVERLAY_SCALE),
            "unstable_long_route": UNSTABLE_LONG_ROUTE,
            "unstable_long_route_overlay_scale": str(
                UNSTABLE_LONG_ROUTE_OVERLAY_SCALE
            ),
            "forensic_basis": {
                "f5_short": (
                    "REGIME_UNSTABLE_ACROSS_5Y_AND_HIGH_INTERNAL_DRAWDOWN"
                ),
                "unstable_long_route": (
                    "AGGREGATE_NEGATIVE_AND_NEGATIVE_IN_3_OF_5_ANNUAL_BLOCKS"
                ),
            },
        },
        ...
        "governance": {
            "r36_freeze_used_as_signal_baseline": True,
            "r37_5y_consumed_forensics": True,
            "r38_corrected_candidate_not_yet_frozen": True,
            "fresh_holdout_consumed": False,
            "candidate_certified": False,
            "freeze_authorized_next_only_if_acceptance_pass": pass_gate,
            "demo_eligible": False,
    ...
    (output / "r38-5y-correction-report.json").write_text(
