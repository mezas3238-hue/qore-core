"""QORE Capitalizer V6C: V6C OTE + M3 quality-filter replay.

V6C is a separate experiment. V3/V4/V6C are untouched.
...
No M1 FVG gate.
M3 quality filter: SWING_AND_BODY060_AND_ATR1P2_NO_CISD.
...
IDENTITY = "QORE_CAPITALIZER_CRT_H1_M3_M1_OTE_1Y_V6C"
MATRIX_IDENTITY = "QORE_CAPITALIZER_NINE_MARKET_CRT_H1_M3_M1_OTE_1Y_V6C"
ENTRY_IDENTITY = (
    "H1_N_N1_CRT__M5_CLOSEBACK__M3_FILTER_C__"
...
class V6CTrade:
    ...
    m3_filter_logic: str = "SWING_AND_BODY060_AND_ATR1P2_NO_CISD"
...
def _metrics(trades: tuple[V6CTrade, ...]) -> V6CMetrics | None:
    if not trades:
        return None
...
def build_market_report(
    ...
    *,
    session: CapitalizerSession,
) -> tuple[V6CMarketReport, tuple[V6CTrade, ...]]:
    all_bars = tuple(
        bar
    ...
    if not all_bars:
        raise ValueError("V6C replay found no native M1")
    symbol = all_bars[0].symbol
    if any(bar.symbol != symbol for bar in all_bars):
        raise ValueError("V6C requires one symbol per M1 root")
...
def write_market(
    report: V6CMarketReport,
    trades: tuple[V6CTrade, ...],
    output: Path,
) -> None:
    output.mkdir(parents=True, exist_ok=True)
    stem = f"capitalizer-{report.symbol.lower()}-crt-h1-m3-m1-ote-1y-v6c"
...
def _load_reports(root: Path) -> list[dict[str, Any]]:
    paths = sorted(root.rglob("capitalizer-*-crt-h1-m3-m1-ote-1y-v6c.json"))
    if len(paths) != 9:
        raise ValueError(f"V6C matrix requires 9 reports, got {len(paths)}")
    reports: list[dict[str, Any]] = []
    for path in paths:
        raw: Any = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(raw, dict) or raw.get("identity") != IDENTITY:
            raise ValueError("unexpected V6C market report")
        reports.append(dict(raw))
    if {str(item["symbol"]) for item in reports} != EXPECTED_SYMBOLS:
        raise ValueError("V6C universe mismatch")
    return sorted(reports, key=lambda item: str(item["symbol"]))


def _load_trades(root: Path) -> tuple[V6CTrade, ...]:
    rows: list[V6CTrade] = []
    for path in sorted(
        root.rglob("capitalizer-*-crt-h1-m3-m1-ote-1y-v6c-trades.jsonl")
    ):
        with path.open(encoding="utf-8") as handle:
            for line in handle:
                if line.strip():
                    rows.append(V6CTrade(**json.loads(line)))
    return tuple(
        sorted(rows, key=lambda item: (_aware(item.entry_at), item.symbol))
    )


def _portfolio_max3(trades: tuple[V6CTrade, ...]) -> tuple[V6CTrade, ...]:
    grouped: dict[str, list[V6CTrade]] = defaultdict(list)
    for trade in trades:
        grouped[f"{trade.session}:{trade.operating_date}"].append(trade)
    selected: list[V6CTrade] = []
...
def _monthly_counts(trades: tuple[V6CTrade, ...]) -> dict[str, int]:
...
def build_matrix(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "m3_filter_logic": "SWING_AND_BODY060_AND_ATR1P2_NO_CISD",
...
def write_matrix(report: dict[str, Any], output: Path) -> None:
    output.mkdir(parents=True, exist_ok=True)
    (output / "capitalizer-nine-market-crt-h1-m3-m1-ote-1y-v6c.json").write_text(
