...
def _validate_story_pack(payload: dict[str, object]) -> None:
    ...
    if _text(renderer.get("market_data_source"), field_name="market data source") != (
...
def _episode(payload: dict[str, object], *, episode_id: str) -> dict[str, object]:
    matches = [
        _object(item, field_name="episode")
        for item in _array(payload.get("episodes"), field_name="episodes")
        if _text(_object(item, field_name="episode").get("episode_id"), field_name="episode id")
        == episode_id
    ]
    if len(matches) != 1:
    ...
    if _text(chart.get("renderer_version"), field_name="episode renderer version") != (
        _RENDERER_VERSION
    ):
        raise StoryForensicsRendererError("episode renderer version changed")
    if _text(chart.get("source_of_truth"), field_name="chart source of truth") != (
        "qore-retained-evidence"
    ):
...
def render_story_html(payload: dict[str, object], *, episode_id: str) -> str:
    ...
    title = html.escape(
        f"{_text(episode.get('trader_code'), field_name='trader code').upper()} · "
        f"{_text(episode.get('symbol'), field_name='symbol')} · {episode_id}"
    )
...
button {{ border: 1px solid #343946; background: #1a1e27; color: #f4f6fa; padding: 9px 13px;
  border-radius: 8px; cursor: pointer; font: inherit; }}
button:disabled {{ opacity: .45; cursor: default; }}
button:focus-visible {{ outline: 2px solid #8ab4ff; outline-offset: 2px; }}
#chart {{ width: 100%; height: 620px; border: 1px solid #2b303b; border-radius: 10px;
  overflow: hidden; background: #11141a; }}
.grid {{ display: grid; grid-template-columns: minmax(0, 1.2fr) minmax(280px, .8fr); gap: 12px;
  margin-top: 12px; }}
.panel {{ border: 1px solid #2b303b; border-radius: 10px; padding: 12px; background: #151922; }}
...
.kv dt {{ color: #969dab; }} .kv dd {{ margin: 0; overflow-wrap: anywhere; }}
.badge {{ display: inline-block; padding: 3px 7px; border: 1px solid #404757; border-radius: 999px;
  font-size: 12px; }}
footer {{ margin-top: 14px; font-size: 12px; color: #949ba9; }}
footer a {{ color: #a9c8ff; }}
@media (max-width: 760px) {{ #chart {{ height: 480px; }} .grid {{ grid-template-columns: 1fr; }} }}
</style>
</head>
<body>
<main>
<header>
<h1>{title}</h1>
<div class=\"meta\"><span id=\"stageBadge\" class=\"badge\">Replay</span> ·
<span id=\"frameLabel\">initializing</span></div>
</header>
...
<div class=\"grid\">
<section class=\"panel\"><h2>Current narrative</h2><p id=\"narrative\"></p></section>
<section class=\"panel\"><h2>Episode facts</h2><dl class=\"kv\" id=\"facts\"></dl></section>
</div>
<footer>
QORE retained evidence is the source of truth. TradingView is visualization only; this page grants no
trading authority. Charts powered by
...
<script type=\"module\">
const LIBRARY_URL = {_safe_json(_LIBRARY_URL)};
...
function asMarker(row) {{
  ...
  const rules = {{
    ...
    entry: {{ position: isLong ? 'belowBar' : 'aboveBar', shape: isLong ? 'arrowUp' : 'arrowDown', color: '#56ccf2' }},
    mfe: {{ position: 'aboveBar', shape: 'circle', color: '#6fcf97' }},
    mae: {{ position: 'belowBar', shape: 'circle', color: '#eb5757' }},
    exit: {{ position: isLong ? 'aboveBar' : 'belowBar', shape: 'square', color: '#bb6bd9' }},
  }};
  return {{ time: row.time, text: row.label, id: `${{row.kind}}-${{row.time}}`, size: 1,
...
function paintPriceLines() {{
  ...
  for (const row of chartData.price_lines) {{
    ...
    candleSeries.createPriceLine({{
      price: Number(row.price), color: rule.color, lineWidth: 2, lineStyle: 2,
      axisLabelVisible: true, title: rule.title,
    }});
  }}
}}
...
function narrativeFor(stage) {{
  if (stage === 'context' || stage === 'signal' || stage === 'entry') return decision.narrative;
  if (stage === 'mfe') return `Peak favorable excursion observed at ${{post.close_path_mfe_r}}R on closed-bar evidence.`;
  if (stage === 'mae') return `Peak adverse excursion observed at ${{post.close_path_mae_r}}R on closed-bar evidence.`;
...
function renderFrame() {{
  ...
  const visibleMarkers = chartData.markers.filter(row => row.time <= through).map(asMarker);
  candleSeries.setData(visibleBars);
  markerApi.setMarkers(visibleMarkers);
  chart.timeScale().fitContent();
  const frame = fullView ? {{ stage: 'forensic-full', visible_through: 'all retained story bars' }} : frames[frameIndex];
  frameLabel.textContent = `${{frame.stage}} · ${{frame.visible_through}}`;
  stageBadge.textContent = fullView ? 'Oracle / post-outcome permitted' : 'Chronological replay';
...
function togglePlay() {{
  ...
  timer = window.setInterval(() => {{
    if (frameIndex >= frames.length - 1) {{ stopPlay(); return; }}
    frameIndex += 1;
    renderFrame();
  }}, 1200);
}}
...
function fillFacts() {{
  const rows = [
    ['Trader', episode.trader_code], ['Market', episode.symbol], ['Period', episode.execution_period],
    ['Side', decision.side], ['Entry', decision.entry_price], ['SL', decision.stop_loss], ['TP', decision.take_profit],
    ['Session', decision.session], ['Trend', decision.trend_regime], ['Volatility', decision.volatility_regime],
    ['Exit', post.exit_reason], ['MFE', `${{post.close_path_mfe_r}}R`], ['MAE', `${{post.close_path_mae_r}}R`],
...
try {{
  ...
  chart = createChart(root, {{
    autoSize: true,
    layout: {{ background: {{ type: 'solid', color: '#11141a' }}, textColor: '#cbd1dc', attributionLogo: true }},
    grid: {{ vertLines: {{ color: '#232833' }}, horzLines: {{ color: '#232833' }} }},
    timeScale: {{ timeVisible: true, secondsVisible: false }},
  }});
  candleSeries = chart.addSeries(CandlestickSeries, {{ priceLineVisible: false }});
  markerApi = createSeriesMarkers(candleSeries, [], {{ autoScale: true }});
  paintPriceLines();
  fillFacts();
  renderFrame();
  prevBtn.addEventListener('click', () => {{ stopPlay(); fullView = false; frameIndex = Math.max(0, frameIndex - 1); renderFrame(); }});
  nextBtn.addEventListener('click', () => {{ stopPlay(); fullView = false; frameIndex = Math.min(frames.length - 1, frameIndex + 1); renderFrame(); }});
  playBtn.addEventListener('click', togglePlay);
  fullBtn.addEventListener('click', () => {{ stopPlay(); fullView = !fullView; renderFrame(); }});
  shotBtn.addEventListener('click', saveScreenshot);
}} catch (error) {{
  root.textContent = `Renderer initialization failed: ${{error instanceof Error ? error.message : String(error)}}`;
  for (const button of [prevBtn, nextBtn, playBtn, fullBtn, shotBtn]) button.disabled = true;
}}
</script>
</body>
</html>
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        render_story_file(
            Path(arguments[0]), episode_id=arguments[1], output_path=Path(arguments[2])
        )
