"""R2-CA AUDUSD cognitive WAIT first-M15 journey atlas.

Attribution only. No rule is promoted.

For every R2-BY OOS opportunity whose initial sovereign cognitive action is WAIT,
observe exactly one additional completed M15 bar from the planned entry open.

The observation is causal at the end of that M15:
- INVALIDATED: structural stop touched before reassessment;
- DESTINATION_CONSUMED: target touched without stop;
- otherwise the completed close is expressed in original planned R and bucketed.

Same-M15 STOP_FIRST is preserved. Final trade outcome is used only for
retrospective attribution by journey state and is never exposed to the decision
state.
"""
...
IDENTITY = "VT08_CRT_PURE_R2CA_AUDUSD_WAIT_JOURNEY_ATLAS_001"
SCHEMA = "qore.vt08.crt_pure.r2ca_audusd_wait_journey_atlas.v1"
...
def run_atlas() -> tuple[tuple[WaitJourneyRecord, ...], dict[str, Any]]:
    ...
    for oos_year in range(
        ...
        for row in oos:
            decision = r2by._decision(model=model, record=row)
    ...
    report: dict[str, Any] = {
        ...
        "source_cognitive_identity": r2by.IDENTITY,
...
def main() -> None:
    ...
    print(
        "CRT_R2CA_AUDUSD_WAIT_JOURNEY_JSON="
