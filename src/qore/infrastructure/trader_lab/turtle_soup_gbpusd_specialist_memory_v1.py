"""Turtle Soup GBPUSD Specialist Memory V1.

Creates a derived, trader-specific copy of the immutable CIBO GBPUSD 10Y
master memory.
...
1. SPECIALIST RAW MEMORY
   - one snapshot row for every CIBO GBPUSD behavior event;
...
IDENTITY = "TURTLE_SOUP_GBPUSD_SPECIALIST_MEMORY_V1"
RAW_IDENTITY = "TURTLE_SOUP_GBPUSD_SPECIALIST_RAW_MEMORY_V1"
COGNITIVE_IDENTITY = "TURTLE_SOUP_GBPUSD_SPECIALIST_COGNITIVE_MEMORY_V1"

MASTER_RAW_RUN_ID = 35166210458
MASTER_RAW_ARTIFACT_ID = 10475449182
MASTER_JOURNEY_RUN_ID = 35175979474
MASTER_TARGET_RUN_ID = 35204892665
MASTER_TARGET_ARTIFACT_ID = 10489089220
...
def build(
    ...
    for event in events:
        ...
        raw_rows.append(
            {
                "schema": "qore.turtle_soup_gbpusd.specialist_raw_memory.v1",
    ...
    for episode_id, setup in sorted(
        ...
        for target in ladder:
            ...
            row: dict[str, Any] = {
                "schema": "qore.turtle_soup_gbpusd.specialist_decision_observation.v1",
    ...
    raw_path = output / "turtle-soup-gbpusd-specialist-raw-memory.jsonl"
    observation_path = output / "turtle-soup-gbpusd-specialist-observations.jsonl"
    _write_jsonl(raw_path, raw_rows)
    _write_jsonl(observation_path, observations)
    (output / "turtle-soup-gbpusd-specialist-cognitive-memory.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )

    report = {
        "schema": "qore.turtle_soup_gbpusd.specialist_memory_report.v1",
        "identity": IDENTITY,
    ...
    (output / "turtle-soup-gbpusd-specialist-memory-report.json").write_text(
