"""R26 Turtle Soup GBPUSD driven by the dedicated Specialist Cognitive Memory.

The strategy supplies its setup, actual NEXT_SOURCE_OPEN entry and exact
Protected Swing.  The dedicated Turtle Soup GBPUSD specialist memory supplies:
...
from qore.infrastructure.trader_lab import (
    turtle_soup_gbpusd_specialist_memory_v1 as specialist,
)
...
IDENTITY = "TURTLE_SOUP_GBPUSD_R26_SPECIALIST_MEMORY_BRAIN_V1"
...
def _load_cognitive(root: Path) -> dict[str, Any]:
    payload = json.loads(
        _single(
            root,
            "turtle-soup-gbpusd-specialist-cognitive-memory.json",
        ).read_text()
    )
...
def run(
    ...
    report = json.loads(
        _single(
            specialist_root,
            "turtle-soup-gbpusd-specialist-memory-report.json",
        ).read_text()
    )
    ...
    payload = {
        "schema": "qore.turtle_soup_gbpusd_r26.specialist_memory_brain.v1",
        "identity": IDENTITY,
        "window": {
