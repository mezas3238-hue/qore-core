"""Reaction-structure atlas for VT-08 Index consumed evidence.

Research-only diagnostics. This module explains where price arrived before a
favorable expansion and what it did around that reaction. It detects source-bound
mechanical FVG, liquidity-sweep, CISD-confirmed order-block and breaker candidates,
then joins those observations to the frozen V7 Market Journey Atlas.

Nothing here changes V7 admission, entry, stop, target, market, session or risk.
Observed structure != causal trading rule.
"""
...
def _opposing_series(bars: Sequence[Bar], before_index: int, *, bullish: bool) -> tuple[Bar, ...]:
    selected: list[Bar] = []
...
def _order_block_zones(bars: Sequence[Bar]) -> list[StructureZone]:
    """CISD-confirmed opposing-candle series, descriptive only."""
    zones: list[StructureZone] = []
    for index in range(1, len(bars)):
        current = bars[index]
        if current.close > current.open:
            series = _opposing_series(bars, index, bullish=True)
            if series and current.close > max(bar.open for bar in series):
                zones.append(
                    StructureZone(
                        kind="order-block",
                        side="bullish",
            ...
            series = _opposing_series(bars, index, bullish=False)
            if series and current.close < min(bar.open for bar in series):
                zones.append(
                    StructureZone(
                        kind="order-block",
                        side="bearish",
...
def _pivot_points(bars: Sequence[Bar], radius: int = 1) -> list[tuple[str, int, Decimal]]:
...
def _breaker_zones(bars: Sequence[Bar]) -> list[StructureZone]:
    """TTrades breaker sequence candidates with a later retestable candle zone."""
    ...
    for first, second, third, fourth in zip(
        ...
        if kinds == "LHLH" and third[2] < first[2] and fourth[2] > second[2]:
            source = [
                bar
            ...
            if source:
                zones.append(
                    StructureZone(
                        ...
                        definition="ttrades-low-high-lower-low-higher-high",
                    )
                )
        elif kinds == "HLHL" and third[2] > first[2] and fourth[2] < second[2]:
            source = [
                bar
                for bar in bars[first[1] : second[1] + 1]
                if bar.close < bar.open
            ...
            if source:
                zones.append(
                    StructureZone(
                        ...
                        low=min(bar.low for bar in source),
                        high=max(bar.high for bar in source),
                        definition="ttrades-high-low-higher-high-lower-low",
                    )
                )
    return zones
...
def _zone_retests(zones: Sequence[StructureZone], bars: Sequence[Bar], signal_at: datetime) -> list[dict[str, object]]:
    events: list[dict[str, object]] = []
    for zone in zones:
        for bar in reversed(bars):
            if bar.closed_at > signal_at or bar.opened_at < zone.confirmed_at:
                continue
            if _overlaps(zone, bar):
                events.append(
                    {
                        "kind": zone.kind,
                        "side": zone.side,
...
def _liquidity_events(bars: Sequence[Bar], signal_at: datetime) -> list[dict[str, object]]:
            ...
            result.append(
                {
                    ...
                    "side": "bearish" if str(event["kind"]).startswith("high") else "bullish",
...
def _segment_behavior(bars: Sequence[Bar]) -> dict[str, object]:
    ...
    directions = [1 if bar.close > bar.open else -1 if bar.close < bar.open else 0 for bar in bars]
    flips = sum(
...
def _episode(
    ...
    window_start = signal_at - timedelta(hours=24)
    window = tuple(bar for bar in bars if window_start <= bar.opened_at < signal_at)
    structures = [
    ...
    if structures:
        ...
        reaction_structures = [
            item for item in structures if _dt(item["observed_at"]) == reaction_at
        ]
    segment = (
        _bars_between(bars, tuple(bar.opened_at for bar in bars), reaction_at, signal_at)
    ...
    return {
        ...
        "reaction_hour_new_york": reaction_at.astimezone(_NY).hour if reaction_at else None,
        ...
        "minutes_reaction_to_1r": (
            ...
            if reaction_at is not None and one_r_at is not None and one_r_at >= reaction_at
            else None
        ),
        "minutes_reaction_to_2r": (
            int((two_r_at - reaction_at).total_seconds() // 60)
            if reaction_at is not None and two_r_at is not None and two_r_at >= reaction_at
            else None
        ),
        "hit_1r": _hit(row, "1"),
        "hit_2r": _hit(row, "2"),
...
def _summary(episodes: Sequence[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "sample": samples,
        "stop_rate": _fmt(Decimal(sum(bool(item["stopped"]) for item in episodes)) / Decimal(samples)),
        "hit_1r_rate": _fmt(Decimal(sum(bool(item["hit_1r"]) for item in episodes)) / Decimal(samples)),
        "hit_2r_rate": _fmt(Decimal(sum(bool(item["hit_2r"]) for item in episodes)) / Decimal(samples)),
        "hit_3r_rate": _fmt(Decimal(sum(bool(item["hit_3r"]) for item in episodes)) / Decimal(samples)),
...
def _group(episodes: Sequence[dict[str, object]], keys: tuple[str, ...]) -> dict[str, object]:
    grouped: dict[str, list[dict[str, object]]] = defaultdict(list)
    for episode in episodes:
        structures = cast(list[dict[str, object]], episode["reaction_structures"])
        structure_labels = [str(item["kind"]) for item in structures] or ["none-detected"]
        for structure in structure_labels:
            values: list[str] = []
            for key in keys:
                values.append(structure if key == "reaction_structure" else str(episode.get(key, "unknown")))
            grouped["|".join(values)].append(episode)
    return {label: _summary(items) for label, items in sorted(grouped.items())}
...
def analyze_window(
    *,
    ...
    payload = cast(dict[str, object], json.loads(journey.read_text(encoding="utf-8")))
    trades = cast(list[dict[str, object]], payload["trades"])
    paths = {"NAS100": nas100, "SP500": sp500, "US30": us30}
    bars = {symbol: _load_bars(path, symbol) for symbol, path in paths.items()}
    ...
    return {
        ...
        "episodes": episodes,
        "by_market_structure": _group(episodes, ("symbol", "reaction_structure")),
        "by_market_weekday_structure": _group(
            episodes, ("symbol", "weekday_new_york", "reaction_structure")
        ),
        "by_market_hour_structure": _group(
            episodes, ("symbol", "reaction_hour_new_york", "reaction_structure")
        ),
        "by_market_behavior": _group(
            episodes, ("symbol", "reaction_behavior_to_signal", "reaction_structure")
        ),
        "definition_status": {
            "fvg": "SOURCE_BOUND_DIAGNOSTIC: TTrades three-candle imbalance",
            "liquidity_sweep": "SOURCE_BOUND_DIAGNOSTIC: short-term extreme run with close-back",
            "order_block": "SOURCE_BOUND_DIAGNOSTIC: opposing candle series confirmed by closure/CISD",
            "breaker_block": "SOURCE_BOUND_DIAGNOSTIC: L-H-LL-HH or H-L-HH-LL sequence plus retest",
            "mitigation_block": "UNRESOLVED_FOR_THIS_ATLAS: source definition exists but separate formalization required",
            "behavior_labels": "DESCRIPTIVE_HEURISTIC_ONLY: fixed non-economic efficiency/body-flip descriptors",
        },
        "governance": {
...
def main() -> None:
    ...
    print(json.dumps({"schema": result["schema"], "episode_count": result["episode_count"]}, sort_keys=True))
