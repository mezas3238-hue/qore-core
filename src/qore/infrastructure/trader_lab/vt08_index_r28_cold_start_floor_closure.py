"""VT08 Index R28 — final bounded cold-start floor closure.

R27 left the partial 2018 segment at -0.004375R while every other primary
calendar segment was positive and conservative portfolio DD stayed below 3R.

R28 does not reopen the model. It fixes the R27 winner in every dimension and
tests only whether cold-start confidence for A/B formations should begin closer
to the already-governed non-zero execution floor until five own completed
observations exist.

No signal is removed. No calendar label is used. No target, stop, rearm,
formation definition, rolling health window, weak-state behavior, portfolio
governor, or concurrency rule changes.
"""
...
SCHEMA = "qore.trader_lab.vt08_index_r28_cold_start_floor_closure.v1"
IDENTITY = "VT08_INDEX_R28_COLD_START_FLOOR_CLOSURE_001"
...
def build_report(
    *,
    ...
    if not contract.validates_trade_count(years=5, sample=len(stream)):
        raise ValueError(f"R28 density drift: {len(stream)}")
    ...
    return {
        ...
        "governance": {
            ...
            "bounded_single_dimension_refinement": (
                "formation_cold_start_confidence_to_existing_nonzero_floor"
            ),
