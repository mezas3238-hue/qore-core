"""QORE Capitalizer CRT H1 -> M3 MSS -> M1 OTE replay V6.

V6 is a separate experiment. V3/V4 are untouched.
...
Frozen chain:
    ...
    -> stop one minimum price quantum beyond OTE 0.79
    -> TP1 H1 N equilibrium closes 50% and moves remainder to BE
    -> TP2 opposite H1 N extreme
    -> same-session / portfolio MAX3 ceiling.

No M1 FVG gate. No M3 CISD/body-ratio/ATR gate.
M3 MSS is valid only when the M3 candle BODY CLOSES beyond the last
pre-sweep 3-candle pivot high/low.
...
IDENTITY = "QORE_CAPITALIZER_CRT_H1_M3_M1_OTE_1Y_V6"
MATRIX_IDENTITY = "QORE_CAPITALIZER_NINE_MARKET_CRT_H1_M3_M1_OTE_1Y_V6"
ENTRY_IDENTITY = (
    "H1_N_N1_CRT__M5_CLOSEBACK__M3_LAST_PRE_SWEEP_PIVOT_BREAK__"
    "M1_CAUSAL_OB__BODY_FIB__OTE_062_079_TWO_CANDLE_ABSORPTION"
)
STOP_IDENTITY = "OTE_079_PLUS_ONE_MIN_PRICE_QUANTUM"
...
class V6MarketReport:
    ...
    m3_body_threshold_required: bool = False
    m3_atr_threshold_required: bool = False
    stop_anchor: str = "OTE_0.79"
    stop_offset_model: str = "ONE_MIN_PRICE_QUANTUM"
    stop_uses_m1_ob: bool = False
...
def _metrics(trades: tuple[V6Trade, ...]) -> V6Metrics | None:
    if not trades:
        return None
...
def _stop_from_079(
    ...
    *,
    ...
    if offset <= 0:
        raise ValueError("V6 stop offset must be positive")
    return (
        zone.level_079 - offset
        if side is CapitalizerSide.LONG
        else zone.level_079 + offset
    )
...
def build_market_report(
    ...
    *,
    session: CapitalizerSession,
) -> tuple[V6MarketReport, tuple[V6Trade, ...]]:
    all_bars = tuple(
        bar
    ...
    if not all_bars:
        raise ValueError("V6 replay found no native M1")
    symbol = all_bars[0].symbol
    if any(bar.symbol != symbol for bar in all_bars):
        raise ValueError("V6 requires one symbol per M1 root")
    ...
    buffer_price = _price_quantum(all_bars)
...
def write_market(
    report: V6MarketReport,
    trades: tuple[V6Trade, ...],
    output: Path,
) -> None:
    output.mkdir(parents=True, exist_ok=True)
    stem = f"capitalizer-{report.symbol.lower()}-crt-h1-m3-m1-ote-1y-v6"
...
def _load_reports(root: Path) -> list[dict[str, Any]]:
    paths = sorted(root.rglob("capitalizer-*-crt-h1-m3-m1-ote-1y-v6.json"))
    if len(paths) != 9:
        raise ValueError(f"V6 matrix requires 9 reports, got {len(paths)}")
    reports: list[dict[str, Any]] = []
    for path in paths:
        raw: Any = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(raw, dict) or raw.get("identity") != IDENTITY:
            raise ValueError("unexpected V6 market report")
        reports.append(dict(raw))
    if {str(item["symbol"]) for item in reports} != EXPECTED_SYMBOLS:
        raise ValueError("V6 universe mismatch")
    return sorted(reports, key=lambda item: str(item["symbol"]))


def _load_trades(root: Path) -> tuple[V6Trade, ...]:
    rows: list[V6Trade] = []
    for path in sorted(
        root.rglob("capitalizer-*-crt-h1-m3-m1-ote-1y-v6-trades.jsonl")
    ):
        with path.open(encoding="utf-8") as handle:
            for line in handle:
                if line.strip():
                    rows.append(V6Trade(**json.loads(line)))
    return tuple(
        sorted(rows, key=lambda item: (_aware(item.entry_at), item.symbol))
    )


def _portfolio_max3(trades: tuple[V6Trade, ...]) -> tuple[V6Trade, ...]:
    grouped: dict[str, list[V6Trade]] = defaultdict(list)
    for trade in trades:
        grouped[f"{trade.session}:{trade.operating_date}"].append(trade)
    selected: list[V6Trade] = []
...
def _monthly_counts(trades: tuple[V6Trade, ...]) -> dict[str, int]:
...
def build_matrix(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "architecture": (
            ...
            "TO_M1_OB_BODY_FIB_OTE_TWO_CANDLE_ABSORPTION_STOP_079"
        ),
        "window_start": WINDOW_START.isoformat(),
        "window_end_exclusive": WINDOW_END.isoformat(),
        "market_count": 9,
        ...
        "m3_body_close_break_required": True,
        "stop_uses_m1_ob": False,
        "stop_anchor": "OTE_0.79",
        "stop_offset_model": "ONE_MIN_PRICE_QUANTUM",
        "tp1_fraction": "0.5",
...
def write_matrix(report: dict[str, Any], output: Path) -> None:
    output.mkdir(parents=True, exist_ok=True)
    (output / "capitalizer-nine-market-crt-h1-m3-m1-ote-1y-v6.json").write_text(
