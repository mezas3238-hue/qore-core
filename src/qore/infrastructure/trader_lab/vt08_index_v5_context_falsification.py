...
def _mean(rows: list[dict[str,str]]) -> str | None:
    ...
    return str(sum((Decimal(r["outcome_r"])-STRESS_R for r in rows),Decimal())/len(rows))

def _group(rows: list[dict[str,str]], key: str) -> dict[str,dict[str,Any]]:
    grouped: dict[str,list[dict[str,str]]] = defaultdict(list)
    for row in rows: grouped[row[key]].append(row)
    return {name:{"sample":len(items),"stressed_mean_r":_mean(items)} for name,items in sorted(grouped.items())}
...
def analyze(rows: list[dict[str,str]]) -> dict[str,Any]:
    ...
    for source in rows:
        ...
        row["v5_context_side_relation"]="inconclusive" if direction is None else "with_side" if direction==row["side"] else "against_side"
...
def main() -> None:
    ...
    with args.census_csv.open(newline="",encoding="utf-8") as handle: rows=list(csv.DictReader(handle))
    report=analyze(rows)
