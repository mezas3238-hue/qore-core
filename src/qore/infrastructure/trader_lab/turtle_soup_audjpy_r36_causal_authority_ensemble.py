"""R36 AUDJPY causal-authority coverage ensemble.

R36 uses the strongest positive AUDJPY pooled causal memory from R32 CORE as
the high-authority layer, then expands only when that core abstains using
positive AUDJPY R34 memories. The negative R34 RANGE memory remains falsified
for authority. No foreign-market family or fragility parameter is imported.
...
IDENTITY = "TURTLE_SOUP_AUDJPY_R36_CAUSAL_AUTHORITY_ENSEMBLE_LAB_V1"
...
ENSEMBLES: dict[str, tuple[tuple[str, tuple[str, ...]], ...]] = {
    "R36_R32CORE_DIRECTION": (
        (CORE, (ROBUST, MAJORITY)),
        (DIRECTION, (ROBUST, MAJORITY)),
    ),
    "R36_R32CORE_DIRECTION_TIMEFRAME": (
        (CORE, (ROBUST, MAJORITY)),
        (DIRECTION, (ROBUST, MAJORITY)),
        (TIMEFRAME, (ROBUST, MAJORITY)),
    ),
    "R36_R32CORE_DIRECTION_TIMEFRAME_MINIMAL_ROBUST": (
        (CORE, (ROBUST, MAJORITY)),
        (DIRECTION, (ROBUST, MAJORITY)),
        (TIMEFRAME, (ROBUST, MAJORITY)),
        (MINIMAL, (ROBUST,)),
...
def run(
    ...
    payload = {
        "schema": "qore.turtle_soup_audjpy.r36_causal_authority_ensemble.v1",
        "identity": IDENTITY,
        "window": {
    ...
    (output / "r36-causal-authority-ensemble-report.json").write_text(
