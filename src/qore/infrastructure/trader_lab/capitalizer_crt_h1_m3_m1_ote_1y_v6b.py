"""QORE Capitalizer V6B: V6B OTE + M3 quality-filter replay.

V6B is a separate experiment. V3/V4/V6B are untouched.
...
No M1 FVG gate.
M3 quality filter: SWING_OR_CISD_AND_BODY060_AND_ATR1P2.
...
ENTRY_IDENTITY = (
    "H1_N_N1_CRT__M5_CLOSEBACK__M3_FILTER_B__"
...
def _metrics(trades: tuple[V6BTrade, ...]) -> V6BMetrics | None:
    if not trades:
        return None
...
def build_market_report(
    ...
    *,
    session: CapitalizerSession,
) -> tuple[V6BMarketReport, tuple[V6BTrade, ...]]:
    all_bars = tuple(
        bar
    ...
    if not all_bars:
        raise ValueError("V6B replay found no native M1")
    symbol = all_bars[0].symbol
    if any(bar.symbol != symbol for bar in all_bars):
        raise ValueError("V6B requires one symbol per M1 root")
...
def write_market(
    report: V6BMarketReport,
    trades: tuple[V6BTrade, ...],
    output: Path,
) -> None:
    output.mkdir(parents=True, exist_ok=True)
    stem = f"capitalizer-{report.symbol.lower()}-crt-h1-m3-m1-ote-1y-v6b"
...
def _load_reports(root: Path) -> list[dict[str, Any]]:
    paths = sorted(root.rglob("capitalizer-*-crt-h1-m3-m1-ote-1y-v6b.json"))
    if len(paths) != 9:
        raise ValueError(f"V6B matrix requires 9 reports, got {len(paths)}")
    reports: list[dict[str, Any]] = []
    for path in paths:
        raw: Any = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(raw, dict) or raw.get("identity") != IDENTITY:
            raise ValueError("unexpected V6B market report")
        reports.append(dict(raw))
    if {str(item["symbol"]) for item in reports} != EXPECTED_SYMBOLS:
        raise ValueError("V6B universe mismatch")
    return sorted(reports, key=lambda item: str(item["symbol"]))


def _load_trades(root: Path) -> tuple[V6BTrade, ...]:
    rows: list[V6BTrade] = []
    for path in sorted(
        root.rglob("capitalizer-*-crt-h1-m3-m1-ote-1y-v6b-trades.jsonl")
    ):
        with path.open(encoding="utf-8") as handle:
            for line in handle:
                if line.strip():
                    rows.append(V6BTrade(**json.loads(line)))
    return tuple(
        sorted(rows, key=lambda item: (_aware(item.entry_at), item.symbol))
    )


def _portfolio_max3(trades: tuple[V6BTrade, ...]) -> tuple[V6BTrade, ...]:
    grouped: dict[str, list[V6BTrade]] = defaultdict(list)
    for trade in trades:
        grouped[f"{trade.session}:{trade.operating_date}"].append(trade)
    selected: list[V6BTrade] = []
...
def _monthly_counts(trades: tuple[V6BTrade, ...]) -> dict[str, int]:
...
def build_matrix(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "m3_filter_logic": "SWING_OR_CISD_AND_BODY060_AND_ATR1P2",
        "m3_swing_break_required": False,
...
def write_matrix(report: dict[str, Any], output: Path) -> None:
    output.mkdir(parents=True, exist_ok=True)
    (output / "capitalizer-nine-market-crt-h1-m3-m1-ote-1y-v6b.json").write_text(
