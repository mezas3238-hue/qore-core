"""R35 AUDJPY confidence-tier coverage ensemble.

R35 preserves the strongest positive AUDJPY R34 DIRECTION_REGIME memory as
the high-authority core and expands only when that core abstains. The negative
R34 RANGE memory is explicitly falsified for authority. Expansion uses only
AUDJPY causal memories already declared and evaluated in R34; no foreign-market
...
IDENTITY = "TURTLE_SOUP_AUDJPY_R35_CONFIDENCE_TIER_ENSEMBLE_LAB_V1"
...
CORE = "R34_DIRECTION_REGIME_ROUTE_TYPES"
...
# AUDJPY R34 forensics: DIRECTION_REGIME is the strongest positive coarse
# authority. RANGE is negative at net@0.10 and is therefore excluded from
# authority rather than rescued with a threshold. Coverage expansion is
# predeclared using only positive AUDJPY causal memories.
ENSEMBLES: dict[str, tuple[tuple[str, tuple[str, ...]], ...]] = {
    "R35_DIRECTION_TIMEFRAME": (
        (CORE, (ROBUST, MAJORITY)),
        (TIMEFRAME, (ROBUST, MAJORITY)),
    ),
    "R35_DIRECTION_TIMEFRAME_MINIMAL_ROBUST": (
        (CORE, (ROBUST, MAJORITY)),
        (TIMEFRAME, (ROBUST, MAJORITY)),
        (MINIMAL, (ROBUST,)),
    ),
    "R35_DIRECTION_MINIMAL_TIMEFRAME_ROBUST": (
        (CORE, (ROBUST, MAJORITY)),
        (MINIMAL, (ROBUST, MAJORITY)),
        (TIMEFRAME, (ROBUST,)),
    ),
}
...
def run(
    ...
    payload = {
        "schema": "qore.turtle_soup_audjpy.r35_confidence_tier_ensemble.v1",
        "identity": IDENTITY,
        "window": {
        ...
        "experiment_contract": {
            ...
            "r34_direction_core_preserved": True,
