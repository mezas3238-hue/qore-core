"""R28 Turtle Soup GBPUSD — structurally defined, economically validated brain."""
...
from qore.infrastructure.trader_lab import (
    turtle_soup_gbpusd_r27_structural_specialist_brain as r27,
)
from qore.infrastructure.trader_lab import (
    turtle_soup_gbpusd_specialist_cognitive_memory_v2 as v2,
)
from qore.infrastructure.trader_lab import (
    turtle_soup_gbpusd_specialist_cognitive_memory_v3 as v3,
)
from qore.infrastructure.trader_lab import (
...
IDENTITY = "TURTLE_SOUP_GBPUSD_R28_VALIDATED_SPECIALIST_BRAIN_V1"
...
def _load_cognitive(root: Path) -> dict[str, Any]:
    payload = json.loads(
        _single(root, "turtle-soup-gbpusd-specialist-cognitive-memory-v3.json").read_text()
    )
...
def run(
    ...
    memory_report = json.loads(
        _single(
            cognitive_root,
            "turtle-soup-gbpusd-specialist-cognitive-memory-v3-report.json",
        ).read_text()
    )
    ...
    payload = {
        "schema": "qore.turtle_soup_gbpusd_r28.validated_specialist_brain.v1",
        "identity": IDENTITY,
        "window": {
