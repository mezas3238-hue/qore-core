"""R2-Y final fresh BTCUSD validation.

Frozen before consuming the preserved 2017-09-21 -> 2018-09-21 BTC window.
...
The primary was selected from already-consumed 2018-2026 evidence because both
single-factor candidates survived four sequential 2Y regimes, while REF2_PLUS
had the stronger eight-year risk-adjusted profile:
- higher PF;
- lower observed max DD;
- shorter losing streak;
- positive contribution in every consumed 2Y block.

BTC_BEARISH remains a challenger because it has higher density and also survived
all consumed regimes. Challenger survival cannot replace primary failure.

CRT execution methodology and the R2-G competition policy remain unchanged.
Research only. No certification or capital authority is granted by this lab.
"""
...
IDENTITY = "VT08_CRT_PURE_R2Y_BTCUSD_FINAL_FRESH_001"
SCHEMA = "qore.vt08.crt_pure.r2y_btcusd_final_fresh.v1"
MARKET = CrtPureMarket.BTCUSD

START = datetime(2017, 9, 21, 0, 0, tzinfo=UTC)
FOLD = datetime(2018, 3, 21, 0, 0, tzinfo=UTC)
...
class Candidate(StrEnum):
    BTC_REF2_PLUS_PRIMARY = "BTC_REF2_PLUS_PRIMARY"
    BTC_BEARISH_CHALLENGER = "BTC_BEARISH_CHALLENGER"
...
BINDING: dict[Candidate, CandidateId] = {
    Candidate.BTC_REF2_PLUS_PRIMARY: CandidateId.BTC_REF2_PLUS,
    Candidate.BTC_BEARISH_CHALLENGER: CandidateId.BTC_BEARISH,
}
PRIMARY = Candidate.BTC_REF2_PLUS_PRIMARY
CHALLENGER = Candidate.BTC_BEARISH_CHALLENGER
FAMILY_MANIFEST = "|".join(
    (
        f"PRIMARY:{PRIMARY.value}:{BINDING[PRIMARY].value}",
        f"CHALLENGER:{CHALLENGER.value}:{BINDING[CHALLENGER].value}",
        f"WINDOW:{START.isoformat()}:{FOLD.isoformat()}:{END.isoformat()}",
        f"GATE:{MIN_TRADES}:{MIN_PF}:{MAX_DD_R}:POSITIVE_BOTH_HALVES",
    )
)
...
def _gate(
    ...
    if float(half_1["total_r"]) <= 0:
        failures.append("HALF1_R")
    if float(half_2["total_r"]) <= 0:
        failures.append("HALF2_R")
...
def run_validation() -> tuple[
    ...
    for candidate in FAMILY:
        ...
        half_1 = _summary(_window(trades, START, FOLD))
        half_2 = _summary(_window(trades, FOLD, END))
        survived, failures = _gate(full, half_1, half_2)
        if survived:
            survivors.append(candidate.value)
        arms[candidate.value] = {
            "role": "PRIMARY" if candidate is PRIMARY else "CHALLENGER",
            "binding": BINDING[candidate].value,
            "full_1y": full,
            "half_1": half_1,
            "half_2": half_2,
    ...
    primary_survived = bool(arms[PRIMARY.value]["survived_research_gate"])
    challenger_survived = bool(arms[CHALLENGER.value]["survived_research_gate"])
    report: dict[str, Any] = {
        "schema": SCHEMA,
        "identity": IDENTITY,
        "market": MARKET.value,
        ...
        "family_frozen_before_fresh_results": True,
        "primary_candidate": PRIMARY.value,
        "challenger_candidate": CHALLENGER.value,
        "primary_survived": primary_survived,
        "challenger_survived": challenger_survived,
        "challenger_cannot_replace_primary_failure": True,
        "research_gate": {
...
def main() -> None:
    ...
    print("CRT_R2Y_BTC_FINAL_FRESH_JSON=" + json.dumps(report, sort_keys=True))
