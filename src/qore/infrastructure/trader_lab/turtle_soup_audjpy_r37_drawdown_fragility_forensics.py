"""R37 AUDJPY drawdown and fragility forensics.

R37 replays the consumed R36 causal-authority signal without changing its
mechanics and emits a per-trade pre-entry ledger for structural drawdown
forensics. It does not promote, suppress, or freeze a candidate.
...
IDENTITY = "TURTLE_SOUP_AUDJPY_R37_DRAWDOWN_FRAGILITY_FORENSICS_V1"
        ...
        ledger.append(
            {
                ...
                "gross_r": str(runtime.gross_r),
                "net_010_r": str(runtime.net_010_r),
...
def run(
    ...
    payload = {
        "schema": "qore.turtle_soup_audjpy.r37_drawdown_fragility_forensics.v1",
        "identity": IDENTITY,
        "window": {
        ...
        "governance": {
            "research_only": True,
            "candidate_replacement_allowed_only_if_acceptance_pass": False,
    ...
    (output / "r37-drawdown-fragility-forensics-report.json").write_text(
        json.dumps(payload, indent=2, sort_keys=True) + "\n"
    )
    with (output / "r37-forensic-trade-ledger.jsonl").open("w", encoding="utf-8") as handle:
        for row in results[0]["trade_ledger"]:
