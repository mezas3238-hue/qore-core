"""QORE Capitalizer V6A: V6A OTE + M3 quality-filter replay.

V6A is a separate experiment. V3/V4/V6A are untouched.
...
Frozen chain:
    ...
    -> two-candle M1 absorption may occur anywhere inside OTE 0.62-0.79
    -> 0.705 is a reporting sweet-spot boundary, not a mandatory trigger
    -> adverse M1 close through 0.79 invalidates pre-entry
    -> M1 entry at the close of candle 2 of the first valid absorption
...
No M1 FVG gate.
M3 quality filter: SWING_AND_CISD_AND_BODY060_AND_ATR1P2.
...
IDENTITY = "QORE_CAPITALIZER_CRT_H1_M3_M1_OTE_1Y_V6A"
MATRIX_IDENTITY = "QORE_CAPITALIZER_NINE_MARKET_CRT_H1_M3_M1_OTE_1Y_V6A"
ENTRY_IDENTITY = (
    "H1_N_N1_CRT__M5_CLOSEBACK__M3_FILTER_A__"
    "M1_CAUSAL_OB__BODY_FIB__OTE_062_079_TWO_CANDLE_ABSORPTION"
)
...
class OTEReaction:
    ...
    absorption_retracement: Decimal
...
class V6ATrade:
    ...
    m3_filter_logic: str = "SWING_AND_CISD_AND_BODY060_AND_ATR1P2"
    m3_swing_break_required: bool = True
...
class V6AMarketReport:
    ...
    ote_79_touches: int
    absorptions_confirmed_zone: int
    absorptions_062_0705: int
    absorptions_0705_079: int
    invalidated_close_through_79: int
    entries_062_0705: int
    entries_0705_079: int
    entries_executed: int
    target_geometry_invalid: int
    raw_metrics: V6AMetrics | None
    ...
    entry_level: str = "ANYWHERE_0.62_TO_0.79"
    level_079_role: str = "PRE_ENTRY_INVALIDATION_BY_CLOSE_ONLY"
    reaction_rule: str = "M1_TWO_CANDLE_ABSORPTION_ANYWHERE_OTE"
    pre_entry_invalidation_rule: str = "ANY_M1_CLOSE_THROUGH_079_BEFORE_ABSORPTION"
...
def _metrics(trades: tuple[V6ATrade, ...]) -> V6AMetrics | None:
    if not trades:
        return None
...
def _variant_structure_logic(
    *,
    ...
    return structure_break and cisd_break
...
def _find_m3_filtered(
    *,
    ...
    if swing is None:
        stages["M3_FILTER_NO_PRE_SWEEP_SWING"] += 1
        return None

    start = bisect.bisect_right(m3_closes, crt.m5_closeback_at)
    end = bisect.bisect_right(m3_closes, crt.n1_deadline)
    ...
    for index in range(start, end):
        ...
        structure_break = (
            source.close > swing.price
            if crt.side is CapitalizerSide.LONG
            else source.close < swing.price
        )
        cisd_break = False
        ...
        if structure_break:
            stages["M3_FILTER_SWING_BREAK_PASS"] += 1
        if cisd_break:
            stages["M3_FILTER_CISD_PASS"] += 1
        stages["M3_FILTER_BODY_PASS"] += 1
        stages["M3_FILTER_ATR_PASS"] += 1
...
def _is_absorption_candle1(
    ...
    *,
    ...
    if side is CapitalizerSide.LONG:
        return bar.close < bar.open and bar.close >= zone.level_079
    return bar.close > bar.open and bar.close <= zone.level_079
...
def _is_absorption_candle2(
    ...
    *,
    ...
    if side is CapitalizerSide.LONG:
        return bar.close > bar.open and bar.close > previous.close
    return bar.close < bar.open and bar.close < previous.close
...
def _absorption_depth(
    ...
    *,
    ...
    if span <= 0:
        raise ValueError("OTE absorption depth requires positive impulse span")
    raw = (
...
def _absorption_bucket(depth: Decimal) -> str:
    if depth <= FIB_0705:
        return "0.62_TO_0.705"
    return "0.705_TO_0.79"
...
def _find_ote_reaction(
    ...
    *,
    ...
    for index, bar in enumerate(execution):
        ...
        if candidate_index is not None:
            ...
            if index == candidate_index + 1 and _is_absorption_candle2(
                ...
                return "ABSORPTION_IN_OTE", OTEReaction(
                    index=index,
                    confirmed_at=bar.closed_at,
                    entry_price=bar.close,
                    reaction_type="TWO_CANDLE_ABSORPTION_IN_OTE",
                    level_079_touched=(
                        candle1.low <= zone.level_079 <= candle1.high
                    ),
                    absorption_candle1_at=candle1.opened_at,
                    absorption_candle2_at=bar.opened_at,
                    absorption_retracement=depth,
                    absorption_bucket=_absorption_bucket(depth),
                )
    ...
    if zone_interacted:
        return "OTE_INTERACTION_NO_ABSORPTION", None
...
def _scan_day(
    *,
    ...
    for n1_open, n1_deadline, hour_bars in _h1_windows(execution):
        ...
        } or (reaction is not None and reaction.level_079_touched):
        ...
        if reaction is None:
            ...
            continue
        stages["OTE_ABSORPTION_CONFIRMED_ZONE"] += 1
        stages[f"OTE_ABSORPTION_{reaction.absorption_bucket}"] += 1
        ...
        stages[f"ENTRY_{reaction.absorption_bucket}"] += 1
        if tp1_hit:
            stages["TP1_HIT"] += 1
        if tp2_hit:
            stages["TP2_HIT"] += 1
        ...
        results.append(
            V6ATrade(
                ...
                absorption_retracement=str(reaction.absorption_retracement),
                absorption_bucket=reaction.absorption_bucket,
...
def build_market_report(
    ...
    *,
    session: CapitalizerSession,
) -> tuple[V6AMarketReport, tuple[V6ATrade, ...]]:
    all_bars = tuple(
        bar
    ...
    if not all_bars:
        raise ValueError("V6A replay found no native M1")
    symbol = all_bars[0].symbol
    if any(bar.symbol != symbol for bar in all_bars):
        raise ValueError("V6A requires one symbol per M1 root")
    ...
    report = V6AMarketReport(
        ...
        absorptions_confirmed_zone=stages["OTE_ABSORPTION_CONFIRMED_ZONE"],
        absorptions_062_0705=stages["OTE_ABSORPTION_0.62_TO_0.705"],
        absorptions_0705_079=stages["OTE_ABSORPTION_0.705_TO_0.79"],
        invalidated_close_through_79=stages["OTE_INVALIDATED_CLOSE_THROUGH_079"],
        entries_062_0705=stages["ENTRY_0.62_TO_0.705"],
        entries_0705_079=stages["ENTRY_0.705_TO_0.79"],
...
def write_market(
    report: V6AMarketReport,
    trades: tuple[V6ATrade, ...],
    output: Path,
) -> None:
    output.mkdir(parents=True, exist_ok=True)
    stem = f"capitalizer-{report.symbol.lower()}-crt-h1-m3-m1-ote-1y-v6a"
...
def _load_reports(root: Path) -> list[dict[str, Any]]:
    paths = sorted(root.rglob("capitalizer-*-crt-h1-m3-m1-ote-1y-v6a.json"))
    if len(paths) != 9:
        raise ValueError(f"V6A matrix requires 9 reports, got {len(paths)}")
    reports: list[dict[str, Any]] = []
    for path in paths:
        raw: Any = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(raw, dict) or raw.get("identity") != IDENTITY:
            raise ValueError("unexpected V6A market report")
        reports.append(dict(raw))
    if {str(item["symbol"]) for item in reports} != EXPECTED_SYMBOLS:
        raise ValueError("V6A universe mismatch")
    return sorted(reports, key=lambda item: str(item["symbol"]))


def _load_trades(root: Path) -> tuple[V6ATrade, ...]:
    rows: list[V6ATrade] = []
    for path in sorted(
        root.rglob("capitalizer-*-crt-h1-m3-m1-ote-1y-v6a-trades.jsonl")
    ):
        with path.open(encoding="utf-8") as handle:
            for line in handle:
                if line.strip():
                    rows.append(V6ATrade(**json.loads(line)))
    return tuple(
        sorted(rows, key=lambda item: (_aware(item.entry_at), item.symbol))
    )


def _portfolio_max3(trades: tuple[V6ATrade, ...]) -> tuple[V6ATrade, ...]:
    grouped: dict[str, list[V6ATrade]] = defaultdict(list)
    for trade in trades:
        grouped[f"{trade.session}:{trade.operating_date}"].append(trade)
    selected: list[V6ATrade] = []
...
def _monthly_counts(trades: tuple[V6ATrade, ...]) -> dict[str, int]:
...
def build_matrix(root: Path) -> dict[str, Any]:
    ...
    per_ote_band: dict[str, dict[str, Any]] = {}
    for bucket in ("0.62_TO_0.705", "0.705_TO_0.79"):
        br = tuple(x for x in raw if x.absorption_bucket == bucket)
        bm = tuple(x for x in max3 if x.absorption_bucket == bucket)
        brm = _metrics(br)
        bmm = _metrics(bm)
        per_ote_band[bucket] = {
            "raw_trades": len(br),
            "raw_metrics": None if brm is None else asdict(brm),
            "max3_trades": len(bm),
            "max3_metrics": None if bmm is None else asdict(bmm),
        }
    ...
    return {
        ...
        "architecture": (
            ...
            "TO_M1_OB_BODY_FIB_OTE_TWO_CANDLE_ABSORPTION"
        ),
        "window_start": WINDOW_START.isoformat(),
        "window_end_exclusive": WINDOW_END.isoformat(),
        "market_count": 9,
        ...
        "absorptions_confirmed_zone": sum(
            int(x["absorptions_confirmed_zone"]) for x in reports
        ),
        "absorptions_062_0705": sum(
            int(x["absorptions_062_0705"]) for x in reports
        ),
        "absorptions_0705_079": sum(
            int(x["absorptions_0705_079"]) for x in reports
        ),
        "invalidated_close_through_79": sum(
            int(x["invalidated_close_through_79"])
            for x in reports
        ),
        "entries_062_0705_raw": sum(
            x.absorption_bucket == "0.62_TO_0.705" for x in raw
        ),
        "entries_0705_079_raw": sum(
            x.absorption_bucket == "0.705_TO_0.79" for x in raw
        ),
        "raw_trades": len(raw),
        ...
        "max3_entries_062_0705": sum(
            x.absorption_bucket == "0.62_TO_0.705" for x in max3
        ),
        "max3_entries_0705_079": sum(
        ...
        "ote_sweet_spot": "0.705_REPORTING_BOUNDARY",
        "entry_level": "ANYWHERE_0.62_TO_0.79",
        "level_079_role": "PRE_ENTRY_INVALIDATION_BY_CLOSE_ONLY",
        "reaction_rule": "M1_TWO_CANDLE_ABSORPTION_ANYWHERE_OTE",
        "pre_entry_invalidation_rule": "ANY_M1_CLOSE_THROUGH_079_BEFORE_ABSORPTION",
        "micro_cisd_required": False,
        "m3_filter_logic": "SWING_AND_CISD_AND_BODY060_AND_ATR1P2",
        "m3_swing_break_required": True,
        "m3_cisd_required": True,
        "m3_body_threshold_required": True,
        "m3_atr_threshold_required": True,
...
def write_matrix(report: dict[str, Any], output: Path) -> None:
    output.mkdir(parents=True, exist_ok=True)
    (output / "capitalizer-nine-market-crt-h1-m3-m1-ote-1y-v6a.json").write_text(
