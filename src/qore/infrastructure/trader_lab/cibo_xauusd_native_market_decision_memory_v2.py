...
IDENTITY = "CIBO_XAUUSD_NATIVE_MARKET_DECISION_MEMORY_V2"
SYMBOL = "XAUUSD"
...
def build(
    ...
    if evidence.symbol != SYMBOL or int(provenance["retained_bars"]) != 707716:
        raise ValueError("unexpected XAUUSD raw corpus")
    ...
    (output / "cibo-xauusd-native-memory-v2-report.json").write_text(
        json.dumps(payload, indent=2, sort_keys=True) + "\n"
    )
    (output / "cibo-xauusd-native-memory-v2-observations.json").write_text(
