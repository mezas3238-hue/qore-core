"""V3 density test D2: admit multiple valid M3 MSS events per H1."""
...
IDENTITY = "QORE_CAPITALIZER_V3_DENSITY_MULTI_MSS_1Y_V1"
MATRIX_IDENTITY = "QORE_CAPITALIZER_NINE_MARKET_V3_DENSITY_MULTI_MSS_1Y_V1"
ENTRY_IDENTITY = (
    "H1_SWEEP__M5_CLOSEBACK__MULTI_M3_MSS_CISD_ATR__"
...
class V3MarketReport:
    ...
    m3_body_ratio_min: str = "0.60"
    m3_atr_multiplier: str = "1.2"
    multi_mss_per_h1_allowed: bool = True
    identical_entry_trigger_to_v3: bool = True
...
def _load_reports(root: Path) -> list[dict[str, Any]]:
    paths = sorted(
        root.rglob(
            "capitalizer-*-v3-density-multi-mss-1y-v1.json"
        )
    )
    if len(paths) != 9:
...
def _load_trades(root: Path) -> tuple[V3Trade, ...]:
    rows: list[V3Trade] = []
    pattern = "capitalizer-*-v3-density-multi-mss-1y-v1-trades.jsonl"
...
def build_matrix(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "architecture": (
            "H1_SWEEP_TO_M5_CLOSEBACK_TO_MULTI_M3_MSS_TO_"
            "M1_CAUSAL_OB_FVG"
        ),
...
def write_matrix(report: dict[str, Any], output: Path) -> None:
    output.mkdir(parents=True, exist_ok=True)
    path = output / "capitalizer-nine-market-v3-density-multi-mss-1y-v1.json"
    path.write_text(
