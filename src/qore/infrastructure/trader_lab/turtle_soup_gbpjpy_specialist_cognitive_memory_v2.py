"""Turtle Soup GBPJPY Specialist Cognitive Memory V2.
...
IDENTITY = "TURTLE_SOUP_GBPJPY_SPECIALIST_COGNITIVE_MEMORY_V2"
SOURCE_V1_IDENTITY = v1.IDENTITY
SOURCE_V1_RUN_ID = 35339795013
SOURCE_V1_ARTIFACT_ID = 10544229406
SOURCE_V1_ARTIFACT_DIGEST = (
    "sha256:4bf56e0977fc27a4d41c023db7287dc1da0c47a2d766b6fe2fab26127bc3e6b4"
)
SOURCE_V1_SHA = "76c1a31079787db22634d921b560727db90c2af7"
...
def build(raw_root: Path, v1_root: Path, output: Path) -> dict[str, Any]:
    source_report = json.loads(
        _single(v1_root, "turtle-soup-gbpjpy-specialist-memory-report.json").read_text()
    )
    ...
    if evidence.symbol != "GBPJPY" or int(provenance["retained_bars"]) != 745478:
        raise ValueError("unexpected GBPJPY master corpus")

    observations: list[dict[str, Any]] = []
    source_path = _single(v1_root, "turtle-soup-gbpjpy-specialist-observations.jsonl")
    ...
    v1._write_jsonl(
        output / "turtle-soup-gbpjpy-specialist-observations-v2.jsonl",
        enriched,
    )
    (output / "turtle-soup-gbpjpy-specialist-cognitive-memory-v2.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )

    report = {
        "schema": "qore.turtle_soup_gbpjpy.specialist_cognitive_memory.v2",
        "identity": IDENTITY,
        "source_v1": {
    ...
    (output / "turtle-soup-gbpjpy-specialist-cognitive-memory-v2-report.json").write_text(
