"""Frozen GBPJPY Turtle Soup R1 and one-year sealed-holdout replay.
...
IDENTITY = "TURTLE_SOUP_GBPJPY_R1"
HOLDOUT_ID = "TURTLE_SOUP_GBPJPY_DEVELOPMENT_10Y_CONSUMED"
SYMBOL = "GBPJPY"
...
def _collect_m5() -> Evidence:
    """Read the preregistered GBPJPY interval without probing another window."""
    ...
    try:
        ...
        if not bars:
            raise RuntimeError("no GBPJPY historical M5 evidence in frozen interval")
...
def evidence_payload(evidence: Evidence) -> dict[str, Any]:
    return {
        "schema": "qore.turtle_soup_gbpjpy_r1.holdout_evidence.v1",
        "identity": IDENTITY,
...
def load_evidence(path: Path) -> Evidence:
    ...
    if payload.get("identity") != IDENTITY or payload.get("holdout_id") != HOLDOUT_ID:
        raise ValueError("unexpected GBPJPY R1 evidence identity")
...
def _simulate(evidence: Evidence, signal: Signal) -> Trade:
    ...
    if not path:
        raise ValueError("empty execution path for frozen GBPJPY R1 signal")
...
def report(evidence: Evidence, output: Path) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema": "qore.turtle_soup_gbpjpy_r1.holdout_result.v1",
        "identity": IDENTITY,
        "holdout_id": HOLDOUT_ID,
        "symbol": SYMBOL,
