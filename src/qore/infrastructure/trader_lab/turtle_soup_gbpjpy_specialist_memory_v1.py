"""Turtle Soup GBPJPY Specialist Memory V1.

Creates a derived, trader-specific copy of the immutable CIBO GBPJPY 10Y
master memory.
...
1. SPECIALIST RAW MEMORY
   - one snapshot row for every CIBO GBPJPY behavior event;
...
IDENTITY = "TURTLE_SOUP_GBPJPY_SPECIALIST_MEMORY_V1"
RAW_IDENTITY = "TURTLE_SOUP_GBPJPY_SPECIALIST_RAW_MEMORY_V1"
COGNITIVE_IDENTITY = "TURTLE_SOUP_GBPJPY_SPECIALIST_COGNITIVE_MEMORY_V1"

MASTER_RAW_RUN_ID = 35166210458
MASTER_RAW_ARTIFACT_ID = 10475453293
MASTER_JOURNEY_RUN_ID = 35175979474
MASTER_TARGET_RUN_ID = 35204892665
MASTER_TARGET_ARTIFACT_ID = 10489327009
...
def build(
    ...
    for event in events:
        ...
        raw_rows.append(
            {
                "schema": "qore.turtle_soup_gbpjpy.specialist_raw_memory.v1",
    ...
    for episode_id, setup in sorted(
        ...
        for target in ladder:
            ...
            row: dict[str, Any] = {
                "schema": "qore.turtle_soup_gbpjpy.specialist_decision_observation.v1",
    ...
    raw_path = output / "turtle-soup-gbpjpy-specialist-raw-memory.jsonl"
    observation_path = output / "turtle-soup-gbpjpy-specialist-observations.jsonl"
    _write_jsonl(raw_path, raw_rows)
    _write_jsonl(observation_path, observations)
    (output / "turtle-soup-gbpjpy-specialist-cognitive-memory.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )

    report = {
        "schema": "qore.turtle_soup_gbpjpy.specialist_memory_report.v1",
        "identity": IDENTITY,
    ...
    (output / "turtle-soup-gbpjpy-specialist-memory-report.json").write_text(
