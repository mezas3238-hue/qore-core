"""Frozen XAUUSD Turtle Soup R1 and one-year sealed-holdout replay.
...
SYMBOL = "XAUUSD"
EVIDENCE_TIER = "E4_FROZEN_CANDIDATE_RULE"
ACQUISITION_OPEN = datetime(2015, 1, 1, 0, 0, tzinfo=UTC)
EVAL_OPEN = datetime(2015, 3, 1, 0, 0, tzinfo=UTC)
EVAL_CLOSE = datetime(2016, 3, 1, 0, 0, tzinfo=UTC)
...
def _collect_m5() -> Evidence:
    """Read the preregistered XAUUSD interval without probing another window."""
    ...
    try:
        ...
        if not bars:
            raise RuntimeError("no XAUUSD historical M5 evidence in frozen interval")
...
def evidence_payload(evidence: Evidence) -> dict[str, Any]:
    return {
        "schema": "qore.turtle_soup_xauusd_r1.holdout_evidence.v1",
        "identity": IDENTITY,
        "holdout_id": HOLDOUT_ID,
        "fresh_relative_to_documented_project_windows": True,
...
def load_evidence(path: Path) -> Evidence:
    ...
    if payload.get("identity") != IDENTITY or payload.get("holdout_id") != HOLDOUT_ID:
        raise ValueError("unexpected XAUUSD R1 evidence identity")
...
def _simulate(evidence: Evidence, signal: Signal) -> Trade:
    ...
    if not path:
        raise ValueError("empty execution path for frozen XAUUSD R1 signal")
...
def report(evidence: Evidence, output: Path) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema": "qore.turtle_soup_xauusd_r1.holdout_result.v1",
        "identity": IDENTITY,
        "holdout_id": HOLDOUT_ID,
        "symbol": SYMBOL,
