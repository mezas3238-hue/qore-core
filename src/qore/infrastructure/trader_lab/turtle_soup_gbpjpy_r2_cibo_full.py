"""GBPJPY Turtle Soup R2 modeled from CIBO pre-entry market intelligence.
...
IDENTITY = "TURTLE_SOUP_GBPJPY_R2_CIBO_FULL_CONTEXT"
SYMBOL = "GBPJPY"
SOURCE_RUN_ID = 35166210458
SOURCE_ARTIFACT_ID = 10475453293
...
def _fit_model(rows: Sequence[tuple[ContextSignal, r1.Trade]]) -> dict[str, Any]:
    ...
    return {
        "schema": "qore.turtle_soup_gbpjpy_r2.model.v1",
        "identity": IDENTITY,
...
def run(source_root: Path, output: Path) -> dict[str, Any]:
    ...
    if evidence.symbol != SYMBOL or provenance["retained_bars"] != 745478:
        raise ValueError("unexpected GBPJPY CIBO corpus")
    ...
    payload = {
        "schema": "qore.turtle_soup_gbpjpy_r2.cibo_full_context_replay.v1",
        "identity": IDENTITY,
        "symbol": SYMBOL,
