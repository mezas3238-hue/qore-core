"""R2-AD within-regime failure forensics for AUDUSD.

AUD_EFF1D_010_020 is positive across 2018-2026 and over 2016-2018 as a whole,
but the frozen annual gate failed 2016-2017 by -0.11573812R. This lab keeps the
exact EFF1D 0.10-0.20 regime and asks which already-frozen pre-parent variables
distinguish that losing annual slice from the other nine consumed years.

No filter is promoted automatically and the EFF1D bounds do not move.
"""
...
IDENTITY = "VT08_CRT_PURE_R2AD_AUDUSD_WITHIN_REGIME_FAILURE_001"
SCHEMA = "qore.vt08.crt_pure.r2ad_audusd_within_regime_failure.v1"
...
BAD_START = START
BAD_END = datetime(2017, 9, 21, 0, 0, tzinfo=UTC)
...
def _contrast(records: tuple[FailureRecord, ...]) -> dict[str, Any]:
    ...
    other = tuple(
        ...
        for record in records
        if not BAD_START
        <= datetime.fromisoformat(record.trade.entry_opened_at)
        < BAD_END
    )
    return {
        "full_10y": _summary_records(records),
        "bad_2016_17": _summary_records(bad),
        "other_9y": _summary_records(other),
...
def run_forensics() -> tuple[tuple[FailureRecord, ...], dict[str, Any]]:
    ...
    for parent in parents:
        ...
        if not EFF1_LOW <= efficiency_1d < EFF1_HIGH:
    ...
    for dimension, labels in dimensions.items():
        for label, result in labels.items():
            bad = result["bad_2016_17"]
            other = result["other_9y"]
            if (
                int(bad["trades"]) >= 3
                and float(bad["total_r"]) < 0
                and int(other["trades"]) >= 20
    ...
    report: dict[str, Any] = {
        ...
        "same_feature_family_as_r2q_r2x": True,
...
def main() -> None:
    ...
    print("CRT_R2AD_AUDUSD_FAILURE_JSON=" + json.dumps(report, sort_keys=True))
