"""XAUUSD Turtle Soup R2 modeled from CIBO pre-entry market intelligence.
...
SOURCE_ARTIFACT_DIGEST = "sha256:dcb905b11380e3d4e1a4dc621e269bb75d2886806dd662e260d21d1d15362e08"
...
def _fit_model(rows: Sequence[tuple[ContextSignal, r1.Trade]]) -> dict[str, Any]:
    ...
    return {
        "schema": "qore.turtle_soup_xauusd_r2.model.v1",
        "identity": IDENTITY,
...
def run(source_root: Path, output: Path) -> dict[str, Any]:
    ...
    if evidence.symbol != SYMBOL or provenance["retained_bars"] != 707716:
        raise ValueError("unexpected XAUUSD CIBO corpus")
    ...
    payload = {
        "schema": "qore.turtle_soup_xauusd_r2.cibo_full_context_replay.v1",
        "identity": IDENTITY,
        "symbol": SYMBOL,
