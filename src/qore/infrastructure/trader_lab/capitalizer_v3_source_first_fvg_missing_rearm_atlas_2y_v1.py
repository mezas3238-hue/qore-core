"""Outcome-free independent rearm atlas for frozen FVG_MISSING setups.

Population: the 322 SOURCE_FIRST funnel rows whose original MSS has no causal
M1 OB/FVG zone under the frozen V3 definition.

The previously tested same-displacement FVG_LAG2 path is not reused here.
This atlas only asks whether a completely NEW causal opportunity appears after
the original MSS, before the original H1 deadline:

new raid beyond the original sweep extreme
-> new M5 closeback
-> new SOURCE_FIRST M3 MSS
-> new causal M1 OB/FVG
-> new normal fill
-> valid frozen V3 stop geometry.
...
from qore.infrastructure.trader_lab.capitalizer_v3_source_first_no_fill_rearm_atlas_2y_v1 import (
    _new_closeback,
    _new_raid,
)

IDENTITY = "QORE_CAPITALIZER_V3_SOURCE_FIRST_FVG_MISSING_REARM_ATLAS_2Y_V1"
MATRIX_IDENTITY = (
...
def _load_fvg_missing(root: Path) -> tuple[dict[str, Any], ...]:
    ...
    if len(paths) != 1:
        raise ValueError("FVG-missing rearm requires one funnel ledger per market")
    ...
    with paths[0].open(encoding="utf-8") as handle:
        for line in handle:
            ...
            if raw.get("terminal_reason") == FVG_MISSING:
                rows.append(raw)
    return tuple(rows)
...
def _build_row(
    *,
    ...
    if original_mss is None or original_mss.confirmed_at != frozen_mss_at:
        raise ValueError("FVG-missing rearm original MSS reconstruction mismatch")
    if v3._m1_causal_zone(execution, event=original_mss) is not None:
        raise ValueError("FVG_MISSING row unexpectedly has original causal zone")
    ...
    if raid is None:
        return _terminal(raw=raw, terminal_stage="NO_NEW_RAID")
...
def build_market_report(
    ...
    *,
...
) -> tuple[dict[str, Any], tuple[FvgMissingRearmRow, ...]]:
    frozen = _load_fvg_missing(funnel_root)
    if not frozen:
        raise ValueError("FVG-missing rearm found no frozen rows")
    ...
    if not all_bars:
        raise ValueError("FVG-missing rearm found no native M1")
    symbol = all_bars[0].symbol
    if any(bar.symbol != symbol for bar in all_bars):
        raise ValueError("FVG-missing rearm requires one market per M1 root")
    if any(str(row["symbol"]) != symbol for row in frozen):
        raise ValueError("FVG-missing rearm funnel/M1 symbol mismatch")
    if any(str(row["session"]) != session.value for row in frozen):
        raise ValueError("FVG-missing rearm session mismatch")
    ...
    rows: list[FvgMissingRearmRow] = []
    for raw in frozen:
        closeback = closebacks.get(str(raw["closeback_at"]))
        if closeback is None:
            raise ValueError("FVG-missing rearm missing reconstructed closeback")
        execution = execution_by_day.get(str(raw["operating_date"]), ())
        if not execution:
            raise ValueError("FVG-missing rearm missing execution day")
        rows.append(
            _build_row(
                raw=raw,
                execution=execution,
    ...
    ordered = tuple(
        sorted(rows, key=lambda item: (item.original_mss_at, item.symbol))
    )
    ...
    return {
        ...
        "fvg_missing_population": len(ordered),
...
def write_market(
    report: dict[str, Any],
    rows: tuple[FvgMissingRearmRow, ...],
    output: Path,
) -> None:
    output.mkdir(parents=True, exist_ok=True)
    symbol = str(report["symbol"]).lower()
    stem = f"capitalizer-{symbol}-v3-source-first-fvg-missing-rearm-atlas-2y-v1"
...
def _load_reports(root: Path) -> list[dict[str, Any]]:
    paths = sorted(
        root.rglob(
            "capitalizer-*-v3-source-first-fvg-missing-rearm-atlas-2y-v1.json"
        )
    )
    if len(paths) != 9:
        raise ValueError(f"FVG-missing rearm matrix requires 9 reports, got {len(paths)}")
    return [dict(json.loads(path.read_text(encoding="utf-8"))) for path in paths]


def build_matrix(root: Path) -> dict[str, Any]:
    reports = _load_reports(root)
    total = sum(int(item["fvg_missing_population"]) for item in reports)
...
def write_matrix(report: dict[str, Any], output: Path) -> None:
    output.mkdir(parents=True, exist_ok=True)
    path = output / (
        "capitalizer-nine-market-v3-source-first-fvg-missing-rearm-atlas-2y-v1.json"
    )
    path.write_text(
        json.dumps(report, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
