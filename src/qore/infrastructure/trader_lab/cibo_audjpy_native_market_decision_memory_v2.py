...
IDENTITY = "CIBO_AUDJPY_NATIVE_MARKET_DECISION_MEMORY_V2"
SYMBOL = "AUDJPY"
...
def build(
    ...
    if evidence.symbol != SYMBOL or int(provenance["retained_bars"]) != 745468:
        raise ValueError("unexpected AUDJPY raw corpus")
    ...
    payload = {
        "schema": "qore.cibo.audjpy.native_market_decision_memory.v2",
        "identity": IDENTITY,
        "symbol": SYMBOL,
    ...
    (output / "cibo-audjpy-native-memory-v2-report.json").write_text(
        json.dumps(payload, indent=2, sort_keys=True) + "\n"
    )
    (output / "cibo-audjpy-native-memory-v2-observations.json").write_text(
        json.dumps(observations, indent=2, sort_keys=True) + "\n"
    )
    (output / "cibo-audjpy-native-memory-v2-hierarchy.json").write_text(
        json.dumps(hierarchy, indent=2, sort_keys=True) + "\n"
    )
    (output / "cibo-audjpy-native-memory-v2-decision-index.json").write_text(
