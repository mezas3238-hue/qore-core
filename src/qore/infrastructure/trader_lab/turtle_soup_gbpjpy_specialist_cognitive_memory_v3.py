"""Economic validation overlay for Turtle Soup GBPJPY Cognitive Memory V2.
...
IDENTITY = "TURTLE_SOUP_GBPJPY_SPECIALIST_COGNITIVE_MEMORY_V3"
SOURCE_V2_IDENTITY = v2.IDENTITY
SOURCE_V2_RUN_ID = 35349300927
SOURCE_V2_ARTIFACT_ID = 10549575964
SOURCE_V2_ARTIFACT_DIGEST = "sha256:9e9b57f3a108d4f26b3e0b3600705859dc2491dd735972d563f7a350fe0798d2"
...
def build(v2_root: Path, output: Path) -> dict[str, Any]:
    report_v2 = json.loads(
        _single(
            v2_root,
            "turtle-soup-gbpjpy-specialist-cognitive-memory-v2-report.json",
        ).read_text()
    )
    ...
    cognitive_v2 = json.loads(
        _single(
            v2_root,
            "turtle-soup-gbpjpy-specialist-cognitive-memory-v2.json",
        ).read_text()
    )
    ...
    (output / "turtle-soup-gbpjpy-specialist-cognitive-memory-v3.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )
    report = {
        "schema": "qore.turtle_soup_gbpjpy.specialist_cognitive_memory.v3",
        "identity": IDENTITY,
        "source_v2": {
    ...
    (output / "turtle-soup-gbpjpy-specialist-cognitive-memory-v3-report.json").write_text(
