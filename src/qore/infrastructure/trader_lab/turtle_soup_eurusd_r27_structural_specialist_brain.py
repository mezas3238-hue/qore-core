"""R27 Turtle Soup EURUSD — structural specialist-memory brain.
...
IDENTITY = "TURTLE_SOUP_EURUSD_R27_STRUCTURAL_SPECIALIST_BRAIN_V1"
...
def _load_cognitive(root: Path) -> dict[str, Any]:
    payload = json.loads(
        _single(root, "turtle-soup-eurusd-specialist-cognitive-memory-v2.json").read_text()
    )
...
def run(
    ...
    payload = {
        "schema": "qore.turtle_soup_eurusd_r27.structural_specialist_brain.v1",
        "identity": IDENTITY,
        "window": {
