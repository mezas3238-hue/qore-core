"""Turtle Soup XAUUSD R3: CIBO journey router, not a context filter.
...
TARGET_ARTIFACT_DIGEST = "sha256:61f60ee5df2507c3f7eca02fef1bc9252f0eaae0a428b47a6027882a7b74bfff"
...
def _load_targets(root: Path) -> tuple[dict[str, list[TargetCandidate]], dict[tuple[datetime, str, str, Decimal], str]]:
    ...
    if len(manifest_paths) != 1 or len(ledger_paths) != 1:
        raise ValueError("expected one XAUUSD Target Destination V2 artifact")
...
def _fit_router(
    ...
    return {
        "schema": "qore.turtle_soup_xauusd_r3.cibo_journey_router.v1",
        "identity": IDENTITY,
...
def run(source_root: Path, target_root: Path, output: Path) -> dict[str, Any]:
    ...
    if evidence.symbol != SYMBOL or int(provenance["retained_bars"]) != 707716:
        raise ValueError("unexpected XAUUSD 10Y source corpus")
    ...
    payload = {
        "schema": "qore.turtle_soup_xauusd_r3.cibo_journey_replay.v1",
        "identity": IDENTITY,
        "symbol": SYMBOL,
