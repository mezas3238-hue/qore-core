"""Economic validation overlay for Turtle Soup XAUUSD Cognitive Memory V2.
...
IDENTITY = "TURTLE_SOUP_XAUUSD_SPECIALIST_COGNITIVE_MEMORY_V3"
SOURCE_V2_IDENTITY = v2.IDENTITY
SOURCE_V2_RUN_ID = 35305072887
SOURCE_V2_ARTIFACT_ID = 10530513211
SOURCE_V2_ARTIFACT_DIGEST = (
    "sha256:99813bf2447afa8c410d0736100b3c693dea715c9e2109f18ed11261d790e0bb"
)
...
def build(v2_root: Path, output: Path) -> dict[str, Any]:
    ...
    cognitive_v2 = json.loads(
        _single(
            v2_root,
            "turtle-soup-xauusd-specialist-cognitive-memory-v2.json",
        ).read_text()
    )
    ...
    with _single(
        v2_root,
        "turtle-soup-xauusd-specialist-observations-v2.jsonl",
    ...
    (output / "turtle-soup-xauusd-specialist-cognitive-memory-v3.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )
    report = {
        "schema": "qore.turtle_soup_xauusd.specialist_cognitive_memory.v3",
        "identity": IDENTITY,
        "source_v2": {
    ...
    (output / "turtle-soup-xauusd-specialist-cognitive-memory-v3-report.json").write_text(
