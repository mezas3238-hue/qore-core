"""R2-CC AUDUSD cognitive WAIT-resolution walk-forward.

Frozen contract:
- preserve R2-BY initial EXECUTE / WAIT / ABSTAIN;
- ABSTAIN is terminal;
- WAIT observes exactly one completed M15;
- if structural stop is touched first -> terminal ABSTAIN;
- if original destination is consumed -> no chase;
- otherwise delayed candidate enters at the next M15 open;
- original structural stop is preserved;
- fixed 1.5R target is recalculated from delayed entry;
- original C3 expiry is preserved;
- STOP_FIRST is preserved;
- AUDUSD keeps BE_CLOSE_075.

A market-specific Journey Experience Memory is trained on the immediately prior
4Y only. Its state is the frozen first-M15 journey bucket and its labels are
delayed-entry Expected-R and delayed DOA rate.

The frozen CRT cognitive reasoner remains the second-decision sovereign.
Research only.
"""
...
def _record_key(record: ViabilityRecord) -> tuple[str, str, str, str, str]:
    trade = record.trade
    return (
    ...
    *,
    ...
    if first_bar is None:
        return DelayedRecord(
            source=record,
            journey_state="MISSING_FIRST_M15",
            delayed_trade=None,
            delayed_doa=None,
            terminal_reason="MISSING_FIRST_M15",
        )
    ...
    if stop_hit:
        return DelayedRecord(
            source=record,
            journey_state="INVALIDATED_BEFORE_REASSESS",
            delayed_trade=None,
            delayed_doa=None,
            terminal_reason="STRUCTURAL_STOP_TOUCHED",
        )
    if target_hit:
        return DelayedRecord(
            source=record,
            journey_state="DESTINATION_CONSUMED_BEFORE_REASSESS",
            delayed_trade=None,
            delayed_doa=None,
            terminal_reason="ORIGINAL_DESTINATION_CONSUMED",
        )

    close_r = r2ca._close_r(trade, first_bar)
    journey_state = r2ca._bucket(close_r)
    ...
    if delayed_bar is None:
        return DelayedRecord(
            ...
            delayed_trade=None,
            delayed_doa=None,
            terminal_reason="MISSING_DELAYED_ENTRY_M15",
        )

    entry = Decimal(delayed_bar.open_price)
    if bullish:
        if stop >= entry:
            terminal = "DELAYED_ENTRY_BELOW_STRUCTURAL_STOP"
            return DelayedRecord(record, journey_state, None, None, terminal)
        risk = entry - stop
        target = entry + TARGET_R * risk
    else:
        if stop <= entry:
            terminal = "DELAYED_ENTRY_ABOVE_STRUCTURAL_STOP"
            return DelayedRecord(record, journey_state, None, None, terminal)
        risk = stop - entry
        target = entry - TARGET_R * risk
    ...
    if not lifecycle:
        return DelayedRecord(
            ...
            delayed_trade=None,
            delayed_doa=None,
            terminal_reason="NO_DELAYED_LIFECYCLE",
        )
    managed = _simulate(
        trade=base,
        bars=lifecycle,
...
def run_walk_forward() -> tuple[
    ...
    for oos_year in range(START.year + TRAINING_YEARS, END.year):
        ...
        initial_model, initial_fit = r2by._fit(training)
        ...
        for row in oos:
            initial = r2by._decision(model=initial_model, record=row)
    ...
    report: dict[str, Any] = {
        "schema": SCHEMA,
        "identity": IDENTITY,
        "market": r2by.MARKET.value,
        "window_start": START.isoformat(),
        "window_end_exclusive": END.isoformat(),
        "initial_cognitive_identity": r2by.IDENTITY,
        "wait_journey_contract": {
            ...
            "protection": ProtectionPolicy.BE_CLOSE_075.value,
        },
        "journey_memory": {
...
def main() -> None:
    ...
    print(
        "CRT_R2CC_AUDUSD_COGNITIVE_WAIT_RESOLUTION_JSON="
