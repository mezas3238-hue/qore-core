"""Turtle Soup GBPUSD Specialist Cognitive Memory V2.
...
IDENTITY = "TURTLE_SOUP_GBPUSD_SPECIALIST_COGNITIVE_MEMORY_V2"
SOURCE_V1_IDENTITY = v1.IDENTITY
SOURCE_V1_RUN_ID = 35338839242
SOURCE_V1_ARTIFACT_ID = 10543594638
SOURCE_V1_ARTIFACT_DIGEST = (
    "sha256:27be69a68ee9b5daec2c43a087b986e649167e06510a5ab2ac18bf0ab62050d5"
)
SOURCE_V1_SHA = "de718176840b68d7b0418c6fedcd9aadb2eb8c7b"
...
def build(raw_root: Path, v1_root: Path, output: Path) -> dict[str, Any]:
    source_report = json.loads(
        _single(v1_root, "turtle-soup-gbpusd-specialist-memory-report.json").read_text()
    )
    ...
    if evidence.symbol != "GBPUSD" or int(provenance["retained_bars"]) != 745274:
        raise ValueError("unexpected GBPUSD master corpus")

    observations: list[dict[str, Any]] = []
    source_path = _single(v1_root, "turtle-soup-gbpusd-specialist-observations.jsonl")
    with source_path.open(encoding="utf-8") as handle:
        for line in handle:
            if line.strip():
                observations.append(json.loads(line))
    if len(observations) != 39112:
    ...
    v1._write_jsonl(
        output / "turtle-soup-gbpusd-specialist-observations-v2.jsonl",
        enriched,
    )
    (output / "turtle-soup-gbpusd-specialist-cognitive-memory-v2.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )

    report = {
        "schema": "qore.turtle_soup_gbpusd.specialist_cognitive_memory.v2",
        "identity": IDENTITY,
        "source_v1": {
    ...
    (output / "turtle-soup-gbpusd-specialist-cognitive-memory-v2-report.json").write_text(
