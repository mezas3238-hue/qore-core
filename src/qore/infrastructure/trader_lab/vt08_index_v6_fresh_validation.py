"""Frozen one-shot fresh-holdout adjudication for VT-08 Index V6."""
...
RULE_FINGERPRINT = "775746fdc6de11ca4cb7fdc6a664acbe0541c4f6befe8cbe4ca79336fccbc221"
HOLDOUT_START = "2020-09-15"
HOLDOUT_END_EXCLUSIVE = "2022-09-15"
MIN_SAMPLE = 30
...
class Vt08IndexV6FreshValidationError(InfrastructureError):
    __slots__ = ()


def _object(value: object, *, name: str) -> dict[str, object]:
    if type(value) is not dict:
        raise Vt08IndexV6FreshValidationError(f"{name} must be object")
    return cast(dict[str, object], value)


def _array(value: object, *, name: str) -> list[object]:
    if type(value) is not list:
        raise Vt08IndexV6FreshValidationError(f"{name} must be array")
    return cast(list[object], value)


def _integer(value: object, *, name: str) -> int:
    if type(value) is not int:
        raise Vt08IndexV6FreshValidationError(f"{name} must be integer")
    return value


def _decimal(value: object, *, name: str) -> Decimal:
    if type(value) is not str:
        raise Vt08IndexV6FreshValidationError(f"{name} must be decimal string")
    try:
        return Decimal(value)
    except Exception as error:
        raise Vt08IndexV6FreshValidationError(f"{name} is invalid decimal") from error
...
def validate_report(report: dict[str, object]) -> dict[str, object]:
    if report.get("candidate_id") != CANDIDATE_ID:
        raise Vt08IndexV6FreshValidationError("candidate identity drifted")
    if report.get("rule_fingerprint") != RULE_FINGERPRINT:
        raise Vt08IndexV6FreshValidationError("rule fingerprint drifted")
    partition = _object(report.get("partition"), name="partition")
    if partition != {
        "start_date": HOLDOUT_START,
        "end_date_exclusive": HOLDOUT_END_EXCLUSIVE,
    }:
        raise Vt08IndexV6FreshValidationError("fresh partition drifted")
    ...
    if len(halves) != 2:
        raise Vt08IndexV6FreshValidationError("fresh halves must contain two partitions")
    half_means = tuple(
        _decimal(
    ...
    if len(quartiles) != 4:
        raise Vt08IndexV6FreshValidationError("fresh quartiles must contain four partitions")
...
def main() -> None:
    ...
    try:
        ...
        raise Vt08IndexV6FreshValidationError("cannot read V6 fresh report") from error
