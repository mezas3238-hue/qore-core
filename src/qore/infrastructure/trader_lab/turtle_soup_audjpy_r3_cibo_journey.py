"""Turtle Soup AUDJPY R3: CIBO journey router, not a context filter.
...
IDENTITY = "TURTLE_SOUP_AUDJPY_R3_CIBO_JOURNEY_ROUTER"
SYMBOL = "AUDJPY"
...
TARGET_ARTIFACT_DIGEST = "sha256:01989d47b98688ad34fb3afadcd5826409a4c955b36fe133669cb288230f238a"
...
def _load_targets(root: Path) -> tuple[dict[str, list[TargetCandidate]], dict[tuple[datetime, str, str, Decimal], str]]:
    ...
    if len(manifest_paths) != 1 or len(ledger_paths) != 1:
        raise ValueError("expected one AUDJPY Target Destination V2 artifact")
...
def _fit_router(
    ...
    return {
        "schema": "qore.turtle_soup_audjpy_r3.cibo_journey_router.v1",
        "identity": IDENTITY,
...
def run(source_root: Path, target_root: Path, output: Path) -> dict[str, Any]:
    ...
    if evidence.symbol != SYMBOL or int(provenance["retained_bars"]) != 745468:
        raise ValueError("unexpected AUDJPY 10Y source corpus")
    ...
    payload = {
        "schema": "qore.turtle_soup_audjpy_r3.cibo_journey_replay.v1",
        "identity": IDENTITY,
        "symbol": SYMBOL,
