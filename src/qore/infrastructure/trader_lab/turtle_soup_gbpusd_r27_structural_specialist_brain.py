"""R27 Turtle Soup GBPUSD — structural specialist-memory brain.
...
IDENTITY = "TURTLE_SOUP_GBPUSD_R27_STRUCTURAL_SPECIALIST_BRAIN_V1"
...
def _load_cognitive(root: Path) -> dict[str, Any]:
    payload = json.loads(
        _single(root, "turtle-soup-gbpusd-specialist-cognitive-memory-v2.json").read_text()
    )
...
def run(
    ...
    payload = {
        "schema": "qore.turtle_soup_gbpusd_r27.structural_specialist_brain.v1",
        "identity": IDENTITY,
        "window": {
