...
IDENTITY = "CIBO_EURUSD_NATIVE_MARKET_DECISION_MEMORY_V2"
SYMBOL = "EURUSD"
...
def build(
    ...
    if evidence.symbol != SYMBOL or int(provenance["retained_bars"]) != 745458:
        raise ValueError("unexpected EURUSD raw corpus")
    ...
    payload = {
        "schema": "qore.cibo.eurusd.native_market_decision_memory.v2",
        "identity": IDENTITY,
        "symbol": SYMBOL,
    ...
    (output / "cibo-eurusd-native-memory-v2-report.json").write_text(
        json.dumps(payload, indent=2, sort_keys=True) + "\n"
    )
    (output / "cibo-eurusd-native-memory-v2-observations.json").write_text(
        json.dumps(observations, indent=2, sort_keys=True) + "\n"
    )
    (output / "cibo-eurusd-native-memory-v2-hierarchy.json").write_text(
        json.dumps(hierarchy, indent=2, sort_keys=True) + "\n"
    )
    (output / "cibo-eurusd-native-memory-v2-decision-index.json").write_text(
