"""VT08 Index R130 — targeted LOW-concurrency source-retest ablation.
...
The strongest B2 context is concurrent_pressure=LOW. In the frozen R46 causal
vocabulary this means at least one position is already open while total active
risk is <=0.25R at the candidate's canonical signal time.

R130 changes one source-authorized variable only:
- exact STANDARD CLOSE_BREAKOUT | NO_FAILED_ATTEMPT trades with LOW concurrent
  pressure use the already-frozen R89/R108 continuation retest when available;
...
SCHEMA = "qore.trader_lab.vt08_index_r130_low_concurrent_source_retest.v1"
IDENTITY = "VT08_INDEX_R130_LOW_CONCURRENT_SOURCE_RETEST_ABLATION_001"

SOURCE_R129_RUN_ID = 36076610569
SOURCE_R129_ARTIFACT_ID = 10839404006
SOURCE_R129_ARTIFACT_DIGEST = (
    "sha256:db94ff3d983c35e05aa71b3f9133fc8e"
    "cb4215d494cf06c8ed1784c6df25677d"
)

TARGET_COHORT = r128.COHORT_STATE
TARGET_CONCURRENT_PRESSURE = "LOW"
EXPECTED_TARGET = {"5Y": 217, "2Y": 95, "R66": 55}
...
def _feature_rows(
    *,
    ...
    if len(rows) != len(assigned):
        raise ValueError("R130 feature alignment drift")
    return tuple(rows)
...
def _window(
    *,
    ...
    if expected != EXPECTED_CANONICAL[window_id]:
        raise ValueError(f"R130 {window_id} canonical contract drift")
    ...
    if len(control) != expected:
        raise ValueError(f"R130 {window_id} control density drift")
    ...
    if len(standard_ids) != EXPECTED_STANDARD[window_id]:
        raise ValueError(f"R130 {window_id} STANDARD drift")
    ...
    for item, feature in zip(control, features, strict=True):
        ...
        target = (
            state == TARGET_COHORT
            and str(feature["concurrent_pressure"])
            == TARGET_CONCURRENT_PRESSURE
        )
        if not target:
            replayed.append(item)
            continue
        ...
        if continuation_index is None or continuation_index <= 0:
            raise ValueError("R130 continuation bar not found")
    ...
    if before_weights != after_weights:
        raise ValueError("R130 frozen R102 weights changed")
    ...
    return {
        ...
        "target_concurrent_pressure": TARGET_CONCURRENT_PRESSURE,
...
def build_report(
    *,
    ...
    if r129.IDENTITY != (
        "VT08_INDEX_R129_R128_CONTEXT_OVERLAP_RISK_SATURATION_001"
    ):
        raise ValueError("R130 R129 identity drift")
    ...
    return {
        ...
        "identity": IDENTITY,
        "source_r129": {
            "run_id": SOURCE_R129_RUN_ID,
            "artifact_id": SOURCE_R129_ARTIFACT_ID,
            "artifact_digest": SOURCE_R129_ARTIFACT_DIGEST,
            "decision": (
        ...
        "preregistered_contract": {
            "target_cohort": TARGET_COHORT,
            "concurrent_pressure": TARGET_CONCURRENT_PRESSURE,
            "concurrent_pressure_definition": (
                "ACTIVE_COUNT_GT_0_AND_ACTIVE_RISK_LE_0_25R"
            ),
