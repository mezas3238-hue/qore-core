"""Turtle Soup XAUUSD Specialist Cognitive Memory V2.
...
IDENTITY = "TURTLE_SOUP_XAUUSD_SPECIALIST_COGNITIVE_MEMORY_V2"
SOURCE_V1_IDENTITY = v1.IDENTITY
SOURCE_V1_RUN_ID = 35304421155
SOURCE_V1_ARTIFACT_ID = 10530462645
SOURCE_V1_ARTIFACT_DIGEST = (
    "sha256:77e0d4ad729f3c7a5a2e9ca124e3937c8ebb2f1e67ed7e462c82efe98826a64e"
)
SOURCE_V1_SHA = "8a5c174e854400c54d7849cc29ba3a4d357afdbb"
...
def build(raw_root: Path, v1_root: Path, output: Path) -> dict[str, Any]:
    source_report = json.loads(
        _single(v1_root, "turtle-soup-xauusd-specialist-memory-report.json").read_text()
    )
    ...
    source_path = _single(v1_root, "turtle-soup-xauusd-specialist-observations.jsonl")
    ...
    v1._write_jsonl(
        output / "turtle-soup-xauusd-specialist-observations-v2.jsonl",
        enriched,
    )
    (output / "turtle-soup-xauusd-specialist-cognitive-memory-v2.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )

    report = {
        "schema": "qore.turtle_soup_xauusd.specialist_cognitive_memory.v2",
        "identity": IDENTITY,
        "source_v1": {
    ...
    (output / "turtle-soup-xauusd-specialist-cognitive-memory-v2-report.json").write_text(
