"""Economic validation overlay for Turtle Soup GBPUSD Cognitive Memory V2.
...
IDENTITY = "TURTLE_SOUP_GBPUSD_SPECIALIST_COGNITIVE_MEMORY_V3"
SOURCE_V2_IDENTITY = v2.IDENTITY
SOURCE_V2_RUN_ID = 35339237432
SOURCE_V2_ARTIFACT_ID = 10544098544
...
def build(v2_root: Path, output: Path) -> dict[str, Any]:
    report_v2 = json.loads(
        _single(
            v2_root,
            "turtle-soup-gbpusd-specialist-cognitive-memory-v2-report.json",
        ).read_text()
    )
    ...
    cognitive_v2 = json.loads(
        _single(
            v2_root,
            "turtle-soup-gbpusd-specialist-cognitive-memory-v2.json",
        ).read_text()
    )
    ...
    (output / "turtle-soup-gbpusd-specialist-cognitive-memory-v3.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )
    report = {
        "schema": "qore.turtle_soup_gbpusd.specialist_cognitive_memory.v3",
        "identity": IDENTITY,
        "source_v2": {
    ...
    (output / "turtle-soup-gbpusd-specialist-cognitive-memory-v3-report.json").write_text(
