"""R2-AY AUDUSD confirmation-geometry suitability walk-forward.

Frozen before R2-AX outcome inspection.
...
Selection family:
Only confirmation/entry geometry buckets predeclared in R2-AX.

Walk-forward:
- train on immediately preceding 3 years;
- candidate state must be negative in all 3 training years;
- at least 8 removed trades in each training year;
- removing it retains >=75% of training trades;
- choose most negative aggregate qualifying state;
- otherwise exclude nothing;
- freeze before next 1Y OOS fold.

Advancement gate:
- >=170 retained trades/year;
- PF >=1.05;
- Total-R >0;
- >=60% OOS years positive;
- DD no worse than baseline OOS.
...
IDENTITY = "VT08_CRT_PURE_R2AY_AUDUSD_CONFIRMATION_SUITABILITY_WF_001"
SCHEMA = "qore.vt08.crt_pure.r2ay_audusd_confirmation_suitability_wf.v1"
TRAINING_YEARS = 3
...
def run_walk_forward() -> tuple[
    ...
    report: dict[str, Any] = {
        ...
        "selection_family_frozen_before_r2ax_results": True,
