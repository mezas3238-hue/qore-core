"""Economic validation overlay for Turtle Soup EURUSD Cognitive Memory V2.
...
IDENTITY = "TURTLE_SOUP_EURUSD_SPECIALIST_COGNITIVE_MEMORY_V3"
SOURCE_V2_IDENTITY = v2.IDENTITY
SOURCE_V2_RUN_ID = 35319185380
SOURCE_V2_ARTIFACT_ID = 10536571877
SOURCE_V2_ARTIFACT_DIGEST = (
    "sha256:f89b452390746f36480f8e32bad80df09de979d3c0d449b321cd8a99f21b5c9d"
)
...
def build(v2_root: Path, output: Path) -> dict[str, Any]:
    report_v2 = json.loads(
        _single(
            v2_root,
            "turtle-soup-eurusd-specialist-cognitive-memory-v2-report.json",
        ).read_text()
    )
    ...
    cognitive_v2 = json.loads(
        _single(
            v2_root,
            "turtle-soup-eurusd-specialist-cognitive-memory-v2.json",
        ).read_text()
    )
    ...
    with _single(
        v2_root,
        "turtle-soup-eurusd-specialist-observations-v2.jsonl",
    ...
    (output / "turtle-soup-eurusd-specialist-cognitive-memory-v3.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )
    report = {
        "schema": "qore.turtle_soup_eurusd.specialist_cognitive_memory.v3",
        "identity": IDENTITY,
        "source_v2": {
    ...
    (output / "turtle-soup-eurusd-specialist-cognitive-memory-v3-report.json").write_text(
