"""R36 GBPUSD F2 structural forensics on the R35 G25 near-gate ensemble.

R36 reproduces the R35 G25 ensemble exactly and adds pre-entry structural diagnostics for OTHER_SESSION_D1_BODY_OPPOSED (F2), the family that supplied most of the coverage but diluted net profit factor. It does not change execution, target, stop, lifecycle, or risk. The purpose is to identify stable structural subfamilies without using the sealed holdout.  Expansion always uses the actual nearest active rank-1 CIBO DOL,
...
IDENTITY = "TURTLE_SOUP_GBPUSD_R36_F2_STRUCTURAL_FORENSICS_V1"
...
def _choose(
    *,
    ...
    return Decision(
        ...
        classification="R36_FORENSICS_REPRODUCTION_SUBFAMILY",
        observations=int(FAMILY_DEV_EVIDENCE[family]["rank1_10y_n"]),
        source="R36_STRUCTURAL_EXPANSION",
        family=family,
    )
...
def _run_family_set(
    *,
    ...
    f2_rows = [
        ...
        if source == "R36_STRUCTURAL_EXPANSION" and family == F2
    ]
    ...
    return {
        ...
        "forensics": {
            "by_source": {
                ...
                for source in ("R32_REGIME_CORE", "R36_STRUCTURAL_EXPANSION")
            },
            "by_family": {
...
def run(
    ...
    payload = {
        "schema": "qore.turtle_soup_gbpusd.r36_f2_structural_forensics.v1",
        "identity": IDENTITY,
        "window": {
        ...
        "governance": {
            ...
            "r33_r34_r35_results_used_for_consumed_development_forensics_only": True,
            "fresh_holdout_consumed": False,
            "demo_eligible": False,
    ...
    (output / "r36-f2-structural-forensics-report.json").write_text(
