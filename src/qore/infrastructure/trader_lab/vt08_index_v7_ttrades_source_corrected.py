...
V7 repairs only source/session defects identified before its next fresh holdout:
- 18:00 NY is daily-open/context, not execution.
- 14:00 NY is not executable by Owner operating contract.
- SAME_C2 must transition to the H4 body side before entry.
- source-day reconstruction tolerates only the documented 16:15 NY broker
  maintenance gap and never synthesizes OHLC.
"""
...
def _source_day(
    ...
    return aggregate, tuple(missing)
...
def _signal_for_h4(
    *,
    ...
    local_hour = h4_opened_at.astimezone(_NY).hour
    if local_hour not in EXECUTABLE_H4_ANCHORS_NY:
        return None
...
def _session_diagnostics(
    ...
    *,
    ...
    return {
        "days_checked": complete + rejected,
        "accepted_source_days": complete,
    ...
    *,
    ...
    for opened in sorted(h4):
        ...
        signal = _signal_for_h4(
            symbol=symbol, indexed=indexed, h4=h4, h4_opened_at=opened
        )
    ...
    return {
        ...
        "session_reconstruction": _session_diagnostics(
            indexed, start_date=start_date, end_date_exclusive=end_date_exclusive
        ),
    }
...
def build_report(
    *,
    ...
    for symbol in AUTHORIZED_MARKETS:
        fingerprint, provider, checked_at, bars = v6._load_candidate_market(
            paths[symbol],
