"""Turtle Soup EURUSD Specialist Cognitive Memory V2.
...
IDENTITY = "TURTLE_SOUP_EURUSD_SPECIALIST_COGNITIVE_MEMORY_V2"
SOURCE_V1_IDENTITY = v1.IDENTITY
SOURCE_V1_RUN_ID = 35318206909
SOURCE_V1_ARTIFACT_ID = 10535839984
SOURCE_V1_ARTIFACT_DIGEST = (
    "sha256:310422faa64b41e50b88e7fd6657c9270d68ab75cc78f9a7db24eb9ad770635e"
)
SOURCE_V1_SHA = "b1151817a92c6a1c301fb983b06fba21a694802a"
...
def build(raw_root: Path, v1_root: Path, output: Path) -> dict[str, Any]:
    source_report = json.loads(
        _single(v1_root, "turtle-soup-eurusd-specialist-memory-report.json").read_text()
    )
    ...
    if evidence.symbol != "EURUSD" or int(provenance["retained_bars"]) != 745458:
        raise ValueError("unexpected EURUSD master corpus")

    observations: list[dict[str, Any]] = []
    source_path = _single(v1_root, "turtle-soup-eurusd-specialist-observations.jsonl")
    with source_path.open(encoding="utf-8") as handle:
        for line in handle:
            if line.strip():
                observations.append(json.loads(line))
    if len(observations) != 38440:
    ...
    v1._write_jsonl(
        output / "turtle-soup-eurusd-specialist-observations-v2.jsonl",
        enriched,
    )
    (output / "turtle-soup-eurusd-specialist-cognitive-memory-v2.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )

    report = {
        "schema": "qore.turtle_soup_eurusd.specialist_cognitive_memory.v2",
        "identity": IDENTITY,
        "source_v1": {
    ...
    (output / "turtle-soup-eurusd-specialist-cognitive-memory-v2-report.json").write_text(
