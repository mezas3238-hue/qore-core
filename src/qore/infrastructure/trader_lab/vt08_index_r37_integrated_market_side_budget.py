"""VT08 Index R37 — integrated causal market-side budget governor.

R36 showed that a post-allocation market/side health overlay was directionally
useful but too weak because most source-complete trades were already at the
minimum floor. R37 moves the same causal concept into the pre-allocation
request stage.

The R34 formation hierarchy and POI-family health remain intact. A rolling
(symbol, side) health multiplier is then applied BEFORE same-timestamp batch
allocation, allowing the fixed portfolio budget to favor currently healthy
requests instead of merely leaving risk unused. Only prior completed raw
outcomes may update health. Every structural signal remains present at a
strict 0.005R minimum; no calendar or year feature is available to runtime.
"""
...
SCHEMA = "qore.trader_lab.vt08_index_r37_integrated_market_side_budget.v1"
IDENTITY = "VT08_INDEX_R37_INTEGRATED_MARKET_SIDE_BUDGET_001"
...
def _profiles() -> tuple[MarketSideProfile, ...]:
    return tuple(
        MarketSideProfile(window, minimum, cold, weak, threshold)
...
def _assign(
    ...
    *,
    ...
    market_side_histories: dict[str, deque[Decimal]] = {}
    loss_streak = 0
    ...
    market_side_states = {"COLD": 0, "WEAK": 0, "HEALTHY": 0}

    def market_side_history(key: str) -> deque[Decimal]:
        if key not in market_side_histories:
            market_side_histories[key] = deque(
                maxlen=profile.rolling_group_trades
            )
        return market_side_histories[key]
    ...
    def settle(until: datetime) -> None:
        ...
        while exit_heap and exit_heap[0][0] <= until:
            ...
            market_side_history(group).append(raw_value)
    ...
    while cursor < len(stream):
        ...
        for opportunity, _outcome in batch:
            ...
            group_mult, group_state = r34._health(
                tuple(market_side_history(group)),
            ...
            weight *= group_mult
            market_side_states[group_state] += 1
    ...
    if len(assigned) != len(stream):
        raise ValueError("R37 changed source-complete trade count")
    ...
    return tuple(assigned), {
        ...
        "market_side_state_counts": market_side_states,
        "market_side_group_count": len(market_side_histories),
...
def build_report(
    *,
    ...
    if not contract.validates_trade_count(years=5, sample=len(stream)):
        raise ValueError(f"R37 density drift: {len(stream)}")
    ...
    return {
        ...
        "governance": {
            ...
            "new_dimension": (
                "preallocation_causal_market_side_completed_trade_health"
            ),
