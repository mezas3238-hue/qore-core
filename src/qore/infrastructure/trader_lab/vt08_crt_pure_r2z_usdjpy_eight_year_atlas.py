"""R2-Z eight-year multi-regime atlas for USDJPY VT08 CRT PURE.

R2-W falsified the only 2020-2026 coherent bucket on the consumed 2018-2020
validation window. R2-Z therefore reuses the exact R2-Q feature family and
requires coherence across four non-overlapping 2Y blocks from 2018 to 2026.

No new features, thresholds, execution rules or filters are introduced.
Research only.
"""
...
SCHEMA = "qore.vt08.crt_pure.r2z_usdjpy_eight_year_atlas.v1"
...
BLOCK_2 = datetime(2020, 9, 21, 0, 0, tzinfo=UTC)
BLOCK_3 = datetime(2022, 9, 21, 0, 0, tzinfo=UTC)
BLOCK_4 = datetime(2024, 9, 21, 0, 0, tzinfo=UTC)
...
def _windows(rows: tuple[AtlasRecord, ...]) -> dict[str, Any]:
    return {
        "full_8y": _block(rows, START, END),
        "block_2018_20": _block(rows, START, BLOCK_2),
        "block_2020_22": _block(rows, BLOCK_2, BLOCK_3),
        "block_2022_24": _block(rows, BLOCK_3, BLOCK_4),
        "block_2024_26": _block(rows, BLOCK_4, END),
    }
                ...
                coherent.append(
                    {
                        "dimension": dimension,
                        "label": label,
                        "full_8y": window["full_8y"],
                        "block_2018_20": blocks[0],
                        "block_2020_22": blocks[1],
                        "block_2022_24": blocks[2],
                        "block_2024_26": blocks[3],
                    }
                )
...
def main() -> None:
    ...
    print("CRT_R2Z_USDJPY_ATLAS_JSON=" + json.dumps(report, sort_keys=True))
