"""Fast-fractal 1Y replay for QORE Capitalizer.

Predeclared architecture B:
    latest completed H1 C2/C3 bias
    -> prior-session significant liquidity sweep
    -> M5 structural close / protected swing
...
MATRIX_IDENTITY = (
    "QORE_CAPITALIZER_NINE_MARKET_FAST_FRACTAL_H1_M5_M1_1Y_MATRIX_V1"
)
ENTRY_IDENTITY = "H1_C2C3__PRIOR_SESSION_SWEEP__M5_BREAK__M1_MSS_FVG_CE"
...
def build_market_report(
    ...
    *,
    ...
    if not all_bars:
        raise ValueError("fast-fractal replay found no native M1")
    symbol = all_bars[0].symbol
    if any(bar.symbol != symbol for bar in all_bars):
        raise ValueError("fast-fractal replay requires one symbol per M1 root")
...
def write_market(
    ...
    stem = (
        f"capitalizer-{report.symbol.lower()}-fast-fractal-h1-m5-m1-1y-v1"
    )
...
def _load_reports(root: Path) -> list[dict[str, Any]]:
    paths = sorted(
        root.rglob("capitalizer-*-fast-fractal-h1-m5-m1-1y-v1.json")
    )
    if len(paths) != 9:
        raise ValueError(
            f"fast-fractal matrix requires 9 reports, got {len(paths)}"
        )
    reports: list[dict[str, Any]] = []
    for path in paths:
        raw: Any = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(raw, dict) or raw.get("identity") != IDENTITY:
            raise ValueError("unexpected fast-fractal market report")
        reports.append(dict(raw))
    if {str(item["symbol"]) for item in reports} != EXPECTED_SYMBOLS:
        raise ValueError("fast-fractal universe mismatch")
...
def _load_trades(root: Path) -> tuple[FastFractalTrade, ...]:
    ...
    pattern = "capitalizer-*-fast-fractal-h1-m5-m1-1y-v1-trades.jsonl"
...
def write_matrix(report: dict[str, Any], output: Path) -> None:
    output.mkdir(parents=True, exist_ok=True)
    path = output / (
        "capitalizer-nine-market-fast-fractal-h1-m5-m1-1y-matrix-v1.json"
    )
    path.write_text(
