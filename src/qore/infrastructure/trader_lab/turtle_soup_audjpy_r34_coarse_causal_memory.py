"""R34 coarse causal memory coverage lab.

R34 does not relax Turtle Soup execution rules.  It tests whether the specialist
...
IDENTITY = "TURTLE_SOUP_AUDJPY_R34_CAUSAL_MEMORY_DEFRAGMENTATION_LAB_V1"
    ...
    for target in ladder:
        ...
        candidates.append(
            Decision(
                ...
                source="R34_POOLED_CAUSAL_MEMORY",
            )
        )
    if not candidates:
        return None
...
def _runtime_decision(decision: Decision) -> r26.SpecialistDecision:
    return r26.SpecialistDecision(
        ...
        memory_level="r34_pooled_causal_memory",
...
def run(
    ...
    payload = {
        "schema": "qore.turtle_soup_audjpy.r34_causal_memory_defragmentation.v1",
        "identity": IDENTITY,
        "window": {
    ...
    (output / "r34-causal-memory-defragmentation-report.json").write_text(
