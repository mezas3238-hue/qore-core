...
IDENTITY = "CIBO_GBPUSD_NATIVE_MARKET_DECISION_MEMORY_V2"
SYMBOL = "GBPUSD"
...
def build(
    ...
    if evidence.symbol != SYMBOL or int(provenance["retained_bars"]) != 745274:
        raise ValueError("unexpected GBPUSD raw corpus")
    ...
    payload = {
        "schema": "qore.cibo.gbpusd.native_market_decision_memory.v2",
        "identity": IDENTITY,
        "symbol": SYMBOL,
    ...
    (output / "cibo-gbpusd-native-memory-v2-report.json").write_text(
        json.dumps(payload, indent=2, sort_keys=True) + "\n"
    )
    (output / "cibo-gbpusd-native-memory-v2-observations.json").write_text(
        json.dumps(observations, indent=2, sort_keys=True) + "\n"
    )
    (output / "cibo-gbpusd-native-memory-v2-hierarchy.json").write_text(
        json.dumps(hierarchy, indent=2, sort_keys=True) + "\n"
    )
    (output / "cibo-gbpusd-native-memory-v2-decision-index.json").write_text(
