"""R27 Turtle Soup AUDJPY — structural specialist-memory brain.
...
IDENTITY = "TURTLE_SOUP_AUDJPY_R27_STRUCTURAL_SPECIALIST_BRAIN_V1"
...
def _load_cognitive(root: Path) -> dict[str, Any]:
    payload = json.loads(
        _single(root, "turtle-soup-audjpy-specialist-cognitive-memory-v2.json").read_text()
    )
...
def run(
    ...
    payload = {
        "schema": "qore.turtle_soup_audjpy_r27.structural_specialist_brain.v1",
        "identity": IDENTITY,
        "window": {
