"""Turtle Soup GBPJPY R3: CIBO journey router, not a context filter.
...
IDENTITY = "TURTLE_SOUP_GBPJPY_R3_CIBO_JOURNEY_ROUTER"
SYMBOL = "GBPJPY"
...
def _fit_router(
    ...
    return {
        "schema": "qore.turtle_soup_gbpjpy_r3.cibo_journey_router.v1",
        "identity": IDENTITY,
...
def run(source_root: Path, target_root: Path, output: Path) -> dict[str, Any]:
    ...
    if evidence.symbol != SYMBOL or int(provenance["retained_bars"]) != 745478:
        raise ValueError("unexpected GBPJPY 10Y source corpus")
    ...
    payload = {
        "schema": "qore.turtle_soup_gbpjpy_r3.cibo_journey_replay.v1",
        "identity": IDENTITY,
        "symbol": SYMBOL,
