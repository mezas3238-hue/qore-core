"""R26 Turtle Soup GBPJPY driven by the dedicated Specialist Cognitive Memory.

The strategy supplies its setup, actual NEXT_SOURCE_OPEN entry and exact
Protected Swing.  The dedicated Turtle Soup GBPJPY specialist memory supplies:
...
IDENTITY = "TURTLE_SOUP_GBPJPY_R26_SPECIALIST_MEMORY_BRAIN_V1"
...
def _load_cognitive(root: Path) -> dict[str, Any]:
    payload = json.loads(
        _single(
            root,
            "turtle-soup-gbpjpy-specialist-cognitive-memory.json",
        ).read_text()
    )
...
def run(
    ...
    report = json.loads(
        _single(
            specialist_root,
            "turtle-soup-gbpjpy-specialist-memory-report.json",
        ).read_text()
    )
    ...
    payload = {
        "schema": "qore.turtle_soup_gbpjpy_r26.specialist_memory_brain.v1",
        "identity": IDENTITY,
        "window": {
