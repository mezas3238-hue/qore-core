"""Turtle Soup GBPUSD R3: CIBO journey router, not a context filter.
...
IDENTITY = "TURTLE_SOUP_GBPUSD_R3_CIBO_JOURNEY_ROUTER"
SYMBOL = "GBPUSD"
...
TARGET_ARTIFACT_DIGEST = "sha256:f208010fedc5619599a1828ff1c243d7696ececdc27a0846b672559384bc9fc1"
...
def _load_targets(root: Path) -> tuple[dict[str, list[TargetCandidate]], dict[tuple[datetime, str, str, Decimal], str]]:
    ...
    if len(manifest_paths) != 1 or len(ledger_paths) != 1:
        raise ValueError("expected one GBPUSD Target Destination V2 artifact")
...
def _fit_router(
    ...
    return {
        "schema": "qore.turtle_soup_gbpusd_r3.cibo_journey_router.v1",
        "identity": IDENTITY,
...
def run(source_root: Path, target_root: Path, output: Path) -> dict[str, Any]:
    ...
    if evidence.symbol != SYMBOL or int(provenance["retained_bars"]) != 745274:
        raise ValueError("unexpected GBPUSD 10Y source corpus")
    ...
    payload = {
        "schema": "qore.turtle_soup_gbpusd_r3.cibo_journey_replay.v1",
        "identity": IDENTITY,
        "symbol": SYMBOL,
