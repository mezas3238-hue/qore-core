"""Turtle Soup XAUUSD Specialist Memory V1.

Creates a derived, trader-specific copy of the immutable CIBO XAUUSD 10Y
master memory.
...
1. SPECIALIST RAW MEMORY
   - one snapshot row for every CIBO XAUUSD behavior event;
...
IDENTITY = "TURTLE_SOUP_XAUUSD_SPECIALIST_MEMORY_V1"
RAW_IDENTITY = "TURTLE_SOUP_XAUUSD_SPECIALIST_RAW_MEMORY_V1"
COGNITIVE_IDENTITY = "TURTLE_SOUP_XAUUSD_SPECIALIST_COGNITIVE_MEMORY_V1"

MASTER_RAW_RUN_ID = 35166210458
MASTER_RAW_ARTIFACT_ID = 10476557530
MASTER_JOURNEY_RUN_ID = 35175979474
MASTER_TARGET_RUN_ID = 35204892665
MASTER_TARGET_ARTIFACT_ID = 10489343458
...
def build(
    ...
    for episode_id, setup in sorted(
        ...
        for target in ladder:
            ...
            row: dict[str, Any] = {
                "schema": "qore.turtle_soup_xauusd.specialist_decision_observation.v1",
    ...
    (output / "turtle-soup-xauusd-specialist-cognitive-memory.json").write_text(
        json.dumps(cognitive, indent=2, sort_keys=True) + "\n"
    )

    report = {
        "schema": "qore.turtle_soup_xauusd.specialist_memory_report.v1",
        "identity": IDENTITY,
    ...
    (output / "turtle-soup-xauusd-specialist-memory-report.json").write_text(
