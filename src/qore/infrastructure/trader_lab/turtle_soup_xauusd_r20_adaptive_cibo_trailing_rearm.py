"""R20 adaptive CIBO trailing + structural rearm for Turtle Soup XAUUSD.

Consumed-evidence research only.

R20 keeps the official CIBO XAUUSD dossier in governed CiboMemoryStore and
changes only position management/re-entry behavior identified by R19:

Trailing by CIBO market-memory state
------------------------------------
SUPPORTIVE:
    Give the trade room. No generic M5 swing trail. A farther target may still
    lock an already-conquered intermediate CIBO DOL from the next bar.
MIXED:
    Require two distinct confirmed protective M5 swings beyond entry before
    the swing-based stop can advance. Intermediate DOL locks remain valid.
CAUTIOUS:
    Protect early using the first confirmed protective M5 swing beyond entry.
    Intermediate DOL locks remain valid.

Structural rearm
----------------
After a trailing-stop exit, another setup is not admitted merely because the
position became free. It must belong to a new market event:
- its raid_at must be strictly after the trailing exit; and
- its C2 source candle must have opened strictly after the trailing exit.

There is no arbitrary minute/hour cooldown and no PnL-based re-entry filter.

Two variants isolate causality:
- ADAPTIVE_NO_REARM: adaptive trailing only.
- ADAPTIVE_REARM: identical trailing plus structural rearm.
"""
...
IDENTITY = "TURTLE_SOUP_XAUUSD_R20_ADAPTIVE_CIBO_TRAILING_REARM_V1"
EVAL_OPEN = datetime(2024, 9, 17, tzinfo=UTC)
EVAL_CLOSE = datetime(2026, 9, 17, tzinfo=UTC)

VARIANT_ADAPTIVE = "ADAPTIVE_NO_REARM"
VARIANT_REARM = "ADAPTIVE_REARM"

TRAIL_PROTECTED_SWING_CAUTION = "CAUTIOUS_FIRST_CONFIRMED_M5_SWING"
TRAIL_PROTECTED_SWING_MIXED = "MIXED_SECOND_CONFIRMED_M5_SWING"
TRAIL_DOL_LOCK = "CONQUERED_DOL_LOCK"

TRAIL_EXIT_REASONS = frozenset(
    {"TRAILING_STOP", "GAP_TRAILING_STOP", "TRAILING_STOP_FIRST"}
)
...
def _swing_trail_admissible(memory_state: str, confirmations: int) -> tuple[bool, str]:
    if memory_state == "SUPPORTIVE":
        return False, "SUPPORTIVE_LET_RUN"
    if memory_state == "CAUTIOUS":
        return confirmations >= 1, TRAIL_PROTECTED_SWING_CAUTION
    if memory_state == "MIXED":
        return confirmations >= 2, TRAIL_PROTECTED_SWING_MIXED
    raise ValueError(f"unsupported CIBO memory state: {memory_state}")
...
def _simulate_adaptive(
    ...
    *,
    ...
    for bar in path:
        # Prior-bar evidence becomes actionable only at this bar open.
        if (
        ...
        # Conquered intermediate DOL is a strong market event in every state.
        if target_rank > 1:
                ...
                if _target_touched(side=side, level=candidate.level, bar=bar):
                    ...
                    if _improves_stop(
                        ...
                        if pending_stop is None:
                            ...
                            better = _better_stop(
                                side, pending_stop, candidate_stop
                            )
        ...
        if (
            ...
            swing_confirmations += 1
            admitted, swing_reason = _swing_trail_admissible(
                memory_state,
                swing_confirmations,
            )
            if admitted:
...
def _variant_summary(executed: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    for state in ("SUPPORTIVE", "MIXED", "CAUTIOUS"):
        ...
        by_state[state] = {
            "gross": _stats(selected, "gross_r"),
            "primary_005r": _stats(selected, "primary_net_r"),
            "stress_010r": _stats(selected, "stress_net_r"),
        }
    return {
...
def _run_variant(
    *,
    ...
    for setup in setups:
        ...
        if use_rearm and not _structurally_rearmed(
            setup=setup,
        ...
        rank, reason = r19._memory_target_depth(
            setup=setup,
            regime=regime,
        ...
        counts["EXECUTE"] += 1
        if trade.exit_reason in TRAIL_EXIT_REASONS:
    ...
    converted.update(
        {
            ...
            "raid_at": item["raid_at"].isoformat(),
            "c2_opened_at": item["c2_opened_at"].isoformat(),
        }
    )
    return converted
...
def run(
    ...
    r19_report = json.loads(
        _find(r19_root, "r19-full-cibo-trailing-report.json").read_text()
    )
    if r19_report["identity"] != r19.IDENTITY:
        raise ValueError("unexpected R19 reference identity")
    ...
    adaptive, adaptive_counts = _run_variant(
        use_rearm=False,
        setups=setups,
        evidence=evidence,
        opens=opens,
    ...
    rearm, rearm_counts = _run_variant(
        use_rearm=True,
        setups=setups,
        evidence=evidence,
        opens=opens,
    ...
    payload = {
        "schema": "qore.turtle_soup_xauusd_r20.adaptive_cibo_trailing_rearm.v1",
        "identity": IDENTITY,
        "window": {
        ...
        "adaptive_trailing_contract": {
            "SUPPORTIVE": (
                "NO_GENERIC_M5_SWING_TRAIL_INTERMEDIATE_DOL_LOCK_ONLY"
            ),
            "MIXED": (
                "SECOND_DISTINCT_CONFIRMED_M5_PROTECTIVE_SWING_OR_INTERMEDIATE_DOL_LOCK"
            ),
            "CAUTIOUS": (
                "FIRST_CONFIRMED_M5_PROTECTIVE_SWING_OR_INTERMEDIATE_DOL_LOCK"
            ),
            "initial_stop": "EXACT_PROTECTED_SWING",
            "stop_widening_allowed": False,
            "new_stop_effective_same_bar": False,
            "pnl_used_to_select_trailing": False,
        },
        "structural_rearm_contract": {
            "trigger": "TRAILING_STOP_EXIT",
            "arbitrary_time_cooldown_used": False,
            "requires_raid_strictly_after_exit": True,
            "requires_c2_source_open_strictly_after_exit": True,
            "pnl_used_for_reentry": False,
        },
        "reproduction": {
        ...
        "reference_r19": {
            "run_id": 35292255680,
            "artifact_id": 10526916936,
            "full_cibo_static": r19_report["variants"]["FULL_CIBO_STATIC"],
            "full_cibo_trailing": r19_report["variants"]["FULL_CIBO_TRAILING"],
        },
        "variants": {
            VARIANT_ADAPTIVE: {
                "decision_counts": adaptive_counts,
                **_variant_summary(adaptive),
            },
            VARIANT_REARM: {
                "decision_counts": rearm_counts,
                **_variant_summary(rearm),
            },
        },
        "adaptive_vs_rearm_same_trade": _paired(adaptive, rearm),
        "governance": {
            "research_only": True,
    ...
    (output / "r20-adaptive-trailing-rearm-report.json").write_text(
        json.dumps(payload, indent=2, sort_keys=True) + "\n"
    )
    (output / "r20-adaptive-no-rearm-trades.json").write_text(
        json.dumps([_json_item(item) for item in adaptive], indent=2, sort_keys=True)
        + "\n"
    )
    (output / "r20-adaptive-rearm-trades.json").write_text(
        json.dumps([_json_item(item) for item in rearm], indent=2, sort_keys=True)
        + "\n"
    )
    return payload
...
def main() -> None:
    if len(sys.argv) != 6:
        raise SystemExit(
            "usage: module SOURCE_ROOT TARGET_ROOT DOSSIER_ROOT R19_ROOT OUTPUT_DIR"
        )
    print(
        json.dumps(
            run(
