"""Five-year validation of the exact frozen GBPJPY R36/R35 candidate.

This is consumed robustness validation, not a fresh holdout.
Window: 2021-09-17 <= entry < 2026-09-17 UTC.

The signal, entry, DOL, Protected Swing, authority-layer ordering and confidence
...
IDENTITY = "TURTLE_SOUP_GBPJPY_R37_FROZEN_R36_5Y_VALIDATION_V1"
...
def run(
    ...
    with (output / "r37-5y-scaled-trades.jsonl").open(
        "w",
        encoding="utf-8",
    ) as handle:
        for row in rows:
    ...
    report: dict[str, Any] = {
        "schema": "qore.turtle_soup_gbpjpy.r37_frozen_r36_5y_validation.v1",
        "identity": IDENTITY,
        "candidate": {
