...
class HostingOrchestrationDecision:
    """Deployment-control decision without lease or trading authority."""
...
def evaluate_hosting_orchestration(
    ...
    *,
    ...
    """Plan deployment control without acquiring/revoking execution authority."""
    ...
    if not isinstance(decision_id, HostingOrchestrationDecisionId):
        raise HostingOrchestratorValidationError(
            "decision_id must be HostingOrchestrationDecisionId"
        )
    if not isinstance(evidence_ref, HostingOrchestrationEvidenceReference):
        raise HostingOrchestratorValidationError(
            "evidence_ref must be HostingOrchestrationEvidenceReference"
        )
    _validate_timestamp(evaluated_at, field_name="orchestration evaluated_at")
    if record.observed_at > evaluated_at:
        raise HostingOrchestratorValidationError(
            "runtime observation cannot be newer than orchestration evaluation"
        )
    if health.evaluated_at != evaluated_at:
        raise HostingOrchestratorValidationError(
            "health assessment must bind exact orchestration evaluation time"
        )
    if evaluated_at > leases.captured_at:
        raise HostingOrchestratorValidationError(
            "lease state must be captured at or after orchestration evaluation"
        )
